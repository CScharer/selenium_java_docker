...
public class CompanyEnvironmentSetup extends Environment
{
	...
	public void setCompanyEnvironmentData(CompanyEnvironmentData companyEnvironmentData)
	{
		...
		if (!JavaHelpers.hasValue(getUserPassword()))
		{
			setUserName("");
			setUserPassword("");
			sysOut("The User Password has not been set so the User Name and User Password have been removed.");
		}
		...
		} else
		{
			...
			sysOut("The Browser has not been set so it will be set to use the ISelenium.BROWSER_DEFAULT.");
		}
		...
		switch (getCompanyEnvironmentData().getHubHost())
		{
			...
			default:
				...
				if (JavaHelpers.hasValue(getHubUserName()))
				{
					...
					if (getCompanyEnvironmentData().getHubUserName().equalsIgnoreCase(SauceUtils.DEFAULT_HOST))
					{
				...
				if (clearDataHub)
				{
					...
					sysOut("The Hub User Name or Hub User Access Key has not been set so the Hub Data has been removed.");
				}
