...
public class BTSCompanyEnvironmentData
{
	...
	public BTSCompanyEnvironmentData(String companyAbbreviation, String environment) throws Exception
	{
		...
		final String nodeEnvironment = nodeCompanyAbbreviation + NODE_ENVIRONMENT + getEnvironment() + "/";
	...
	private String getData(String xPath)
	{
		...
		{
			Environment
					.sysOut(JavaHelpers.getCurrentMethodName() + "xpathExpression = XML.XPATH.compile(" + xPath + ");");
		}
		...
		{
			Environment.sysOut(nodeList.getLength() + " Records Found For:[" + xPath + "] using Value[***Password***]");
		} else
		{
			Environment.sysOut(nodeList.getLength() + " Records Found For:[" + xPath + "] using Value[" + value + "]");
		}
		return value;
	}
}