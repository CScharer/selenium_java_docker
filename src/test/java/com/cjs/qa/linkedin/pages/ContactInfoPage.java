...
public class ContactInfoPage extends Page
{
	...
	public void setLinkedInConnectionContactInfoListMap(List<Map<String, String>> linkedInConnectionContactInfoListMap)
	{
	...
	{
		return webDriver
				.findElements(By.xpath(".//div[@class='pv-profile-section__section-info section-info']/section"));
	}
	...
	public void getContactInfoPageData(List<Map<String, String>> linkedInMapList)
	{
		...
		for (int mapIndex = 0; mapIndex < linkedInMapList.size(); mapIndex++)
		{
					...
					&& JavaHelpers.hasValue(map.get(Data.FIELD_LINKEDIN_URL)))
			{
				...
				String linkedInURLDetailContactInfo = LinkedIn.LINKEDIN_URL + linkedInURL + "/detail/contact-info";
		...
		try
		{
			...
			for (Entry entry : map.entrySet())
			{
				...
				if (!fieldName.equals(Data.FIELD_LINKEDIN_URL))
				{
					...
					stringBuilderData.append("[" + fieldName + "]='" + fieldValue.replaceAll("'", "''") + "'");
				}
			}
			if (!stringBuilderData.toString().equals(""))
			{
				...
				stringBuilder
						.append(" WHERE [" + Data.FIELD_LINKEDIN_URL + "]='" + map.get(Data.FIELD_LINKEDIN_URL) + "';");
	...
	public void updateLinkedInTable()
	{
		...
		try
		{
			...
			for (int mapIndex = 0; mapIndex < getLinkedInConnectionContactInfoListMap().size(); mapIndex++)
			{
				Map<String, String> contactInfoMap = getLinkedInConnectionContactInfoListMap().get(mapIndex);
				appendLinkedInRecord(contactInfoMap);
			}
			if (!getStringBuilderSQL().toString().equals(""))
			{
				FSO.fileWrite(LinkedInEnvironment.FILE_LOG,
						sqlStringBuilder.toString().replaceAll(";", Constants.NEWLINE + ';'), false);
				int recordsAffected = Data.getJdbc().executeUpdate(sqlStringBuilder.toString(), false);
				message = Constants.NEWLINE + sqlStringBuilder.toString().replaceAll(";", Constants.NEWLINE);
