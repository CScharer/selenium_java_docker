...
public class Main extends Page
{
	...
	public void getInmatesOnTheWeb(boolean captureNew)
	{
		...
		{
			load();
			String sTotalInmates = webDriver.findElement(By.xpath(".//*[@id='ctl00_ContentPlaceHolder1_lblTotal']"))
					.getText();
			...
			sqlStringBuilder = new StringBuilder();
			final List<WebElement> rowElements = webDriver
			...
			for (int indexRow = 0; indexRow < rowElements.size(); indexRow++)
			{
				...
				for (int indexColumn = 0; indexColumn < columnElements.size(); indexColumn++)
				{
					...
					switch (column)
					{
						...
						case 4:
							...
							{
								sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord("t_DOM_IOW_Arrests",
										sqlStringBuilder, mapRecord, true);
							} else
							{
				...
				if (!sqlStringBuilder.toString().equals(""))
				{
					try
					{
						Environment.sysOut("sqlStringBuilder.toString():[" + Constants.NEWLINE
	...
	private StringBuilder getArrestRecord(String url, StringBuilder sqlStringBuilder)
	{
		...
		if (objectExists(byNoResults, 3))
		{
			...
			sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord("t_DOM_IOW_Prisoners", sqlStringBuilder,
					inmateRecordMap, true);
		...
		for (final String key : inmateNameDateMap.keySet())
		{
			...
			{
				value = getLabel(
						By.xpath(".//*[@id='inmateNameDate']/tbody/tr[2]/th[text()[contains(.,'" + key + "')]]/../td"));
			} else
			{
				value = getLabel(
						By.xpath(".//*[@id='inmateNameDate']/tbody/tr/th[text()[contains(.,'" + key + "')]]/../td"));
			}
		...
		{
			// Environment.sysOut("key:[" + key + "]");
			final String value = getLabel(
					By.xpath(".//*[@id='inmateProfile']/tbody/tr/th[text()[contains(.,'" + key + "')]]/../td"));
		...
		{
			// Environment.sysOut("key:[" + key + "]");
			String value = getLabel(By.xpath(".//*[@id='inmateAddress']/strong[text()[contains(.,'" + key + "')]]/.."));
	...
	private StringBuilder getCaseRecord(String url, StringBuilder sqlStringBuilder, boolean refreshURL)
	{
		final By byInmateNameDate = By
				.xpath(".//*[@id='inmateNameDate']/tbody/tr[1]/th[text()[contains(.,'Offender/Name ID')]]/../td");
		if (refreshURL)
		{
			webDriver.get(url);
		}
		...
		if (objectExists(byNoResults, 3))
		{
			...
			Environment.sysOut("mapRecord:[" + mapRecord.toString() + "]");
			sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord("t_DOM_IOW_Cases", sqlStringBuilder, mapRecord,
					true);
			return sqlStringBuilder;
		}
		final List<WebElement> rowElements = webDriver
					...
					case 4:
						...
						if (!mapRecord.get("Case #").equals("Case #"))
						{
							...
							if (!listMapRecord.contains(mapRecord.toString()))
							{
								...
								sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord("t_DOM_IOW_Cases",
										sqlStringBuilder, mapRecord, true);
							}
							// }
						}
