...
public class SOAP
{
	...
	{
		final BufferedReader bufferedReader = new BufferedReader(new FileReader(new File(filePathName)));
		String input = "";
	...
	public String getSOAPMessageValue(SOAPMessage soapMessage, boolean maskPassword) throws QAException
	{
		setXml(null);
	...
	public void writeSOAPMessageToOutputStream(SOAPMessage soapMessage) throws QAException
	{
		...
		try
		{
			...
			soapMessageValue = soapMessageValue.replaceAll(Constants.NEWLINE + Constants.NEWLINE, Constants.NEWLINE);
	...
	{
		final List<String> charactersXML = Arrays.asList("&lt;", "&gt;", "&apos;", "&quot;", "&amp;", "&#xD;");
