...
public class SystemProcesses
{
	...
	public static final String						FILE_DATA_CSV					= Environment.FOLDER_DATA + "Data"
			+ IExtension.CSV;
	...
	public static final List<String>				LIST_PROCESS_TYPE				= Arrays
			.asList("M;SVC;APPS;V".split(Constants.DELIMETER_LIST));
	public static final List<String>				LIST_FIELDS_PROCESS_STANDARD	= Arrays
			.asList("Image Name;PID".split(Constants.DELIMETER_LIST));
	...
	public static void check()
	{
		try
		{
			...
			{
				final String fileData = Environment.FOLDER_DATA + "Process_" + processType + EXTENSION;
				String command = "CMD /C TASKLIST /S " + SYSTEM + "/U " + USERID + " /P PASSWORD /" + processType
						+ " /FO:" + FORMAT + ">" + fileData;
				...
				switch (processType)
				{
					case "M":
						...
						sqlStringBuilder.append(JDBCConstants.SELECT + "[DateTimeStamp],[PID],[Image Name],[Modules] ");
						sqlStringBuilder.append(JDBCConstants.FROM + "[t_Core_Processes_" + processType + "];");
						break;
					default:
						final List<Map<String, String>> records = jdbc.queryResultsString(
						...
						for (int recordIndex = 0; recordIndex < records.size(); recordIndex++)
						{
							...
							} else
							{
								...
								sqlStringBuilder.append(" " + JDBCConstants.WHERE + "[PID]="
										+ mapRecord.get(PRIMARY_KEY_PROCESS) + Constants.DELIMETER_LIST);
	...
	{
		final String tableNamePrevous = tableName.replace("t_Core_Processes", "t_Core_ProcessesPrevious");
		String sql = JDBCConstants.DROP_TABLE + JDBCConstants.IF_EXISTS + "[" + tableNamePrevous + "]";
		SQL.execute(sql);
		sql = JDBCConstants.CREATE_TABLE + "[" + tableNamePrevous + "] " + JDBCConstants.AS
				+ JDBCConstants.SELECT_ALL_FROM + "[" + tableName + "]";
		SQL.execute(sql);
	}
	...
	public static Map<String, List<String>> getProcessFields()
	{
		...
		for (final String processType : LIST_PROCESS_TYPE)
		{
			...
			switch (processType)
			{
				...
				case "SVC":
					...
					break;
				case "V":
					listFieldsTemp = Arrays
							.asList("Session Name;Session#;Mem Usage;Status;User Name;CPU Time;Window Title"
			...
			sqlStringBuilder.append(
					Constants.nlTab(0, 0) + JDBCConstants.CREATE_TABLE + "`t_Core_Processes_" + processType + "` (");
			sqlStringBuilder.append(
					Constants.nlTab(1, 1) + "`DateTimeStamp` TEXT " + JDBCConstants.NOT_NULL + "DEFAULT (null),");
			for (int listIndex = 0; listIndex < listFieldsProcess.size(); listIndex++)
			{
				...
				{
					sqlStringBuilder.append(Constants.nlTab(1, 1) + "`" + fieldName + "` NUMERIC "
							+ JDBCConstants.NOT_NULL + "DEFAULT -1");
				} else
				{
					sqlStringBuilder.append(Constants.nlTab(1, 1) + "`" + fieldName + "` TEXT " + JDBCConstants.NOT_NULL
							+ "DEFAULT (null)");
				}
		...
		sqlStringBuilder.append(Constants.nlTab(0, 0) + JDBCConstants.CREATE_TABLE + "`t_Core_Processes` (");
		sqlStringBuilder
				.append(Constants.nlTab(1, 1) + "`DateTimeStamp` TEXT " + JDBCConstants.NOT_NULL + "DEFAULT (null),");
		for (int listIndex = 0; listIndex < listFieldsProcessGrouped.size(); listIndex++)
		{
			...
			{
				sqlStringBuilder.append(
						Constants.nlTab(1, 1) + "`" + fieldName + "` NUMERIC " + JDBCConstants.NOT_NULL + "DEFAULT -1");
			} else
			{
	...
	private static void importData(String dateTimeStamp, String tableName, List<String> listHeadingsExpected)
	{
		...
		try
		{
			...
			for (final CSVRecord record : records)
			{
				...
				if (recordNumber >= (RECORD_START + 1))
				{
					...
					stringBuilder = SQL.appendStringBuilderSQLInsertRecord(tableName, stringBuilder, memberMap, true);
