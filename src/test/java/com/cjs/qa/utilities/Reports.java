...
{
	public static final String					JIRA_URL			= "http://jira-tst.wrberkley" + IExtension.COM
			+ "/browse/";
	public static final int						DEFAULT_SHEET_COUNT	= 2;
	public static final String					STRING_SUMMARY		= "Test Set;Test ID;Environment;Company;LOB;Test Name;Scenario Name;Status;Error(s);Started;Completed;Elapsed (seconds);Browser;Scenario ID;Session ID;API;DB;GUI";
	public static final String					STRING_COUNTS		= "Tests;Passed;Failed;Compare;Copy;Rate;Issue;Policy;Product;Forms";
	private static final List<String>			LIST_SUMMARY		= Arrays
			.asList(STRING_SUMMARY.split(Constants.DELIMETER_LIST));
	private static final List<String>			LIST_COUNTS			= Arrays
			.asList(STRING_COUNTS.split(Constants.DELIMETER_LIST));
	...
	private static synchronized void _updateReport(File filePathName, byte[] byteArray) throws IOException
	{
	...
	public static synchronized void createReportExcelLock(Scenario scenarioObject, Map<String, List> mapListTest)
			throws Exception, QAException
	{
		...
		try
		{
			...
			// Read in the excel file as Apache POI object
			final byte[] byteArray = (mapListTest.get("Summary").toString() + Constants.NEWLINE).getBytes();
	...
	private static void createReportExcel(Scenario scenarioObject, Map<String, List> mapListTest)
	{
		...
		try
		{
			...
			final String sheetNameFailure = Environment.getSheetFailure(filePathName, DEFAULT_SHEET_COUNT,
					mapSummary.get("Policy Control"));
			...
			{
				excel.addLink(sheetNameSummary, resultColumns.get("Test ID"), rowSummary, "URL",
						mapSummary.get("TestID"), hyperlinkTestID);
			}
			if (writeData)
			{
				if (JavaHelpers.hasValue(sheetNameFailure))
				{
					...
					excel.addLink(sheetNameSummary, MAP_INDEX_SUMMARY.get("Error(s)"), rowSummary, "DOCUMENT",
							mapSummary.get("Error(s)"), hyperlinkSheetFailure);
					// "See Sheet (" + sheetNameFailure + ")",
					excel.addLink(sheetNameFailure, 0, 0, "DOCUMENT", sheetNameSummary, hyperlinkSheetSummary);
					for (final Integer key : Environment.scenarioErrors.keySet())
					{
						final int rowSheetFailure = (excel.getRowCount(sheetNameFailure) + 1);
						excel.writeCell(sheetNameFailure, 0, rowSheetFailure, "Failure (" + String.valueOf(key) + ")");
						excel.autoSizeColumn(sheetNameFailure, 0);
						excel.setCellAlignment(sheetNameFailure, 0, 0);
						excel.writeCell(sheetNameFailure, 1, rowSheetFailure, Environment.scenarioErrors.get(key));
						...
						scenarioObject.log("Scenario Failure (" + key + "): " + Environment.scenarioErrors.get(key));
					}
			...
			{
				excel.writeCell(sheetNameSummary, MAP_INDEX_SUMMARY.get("Error(s)"), rowSummary,
	...
	private static void createResultFileDefaults(String filePathName) throws QAException, IOException
	{
		XLS excel = null;
		try
		{
	...
	{
		final String status = "dateTimeStamp:["
				+ DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME + ".SSS") + "], " + report
				+ Constants.NEWLINE;
		return status;
	}
	...
	private static void writeFailureData(XLS excel, String sheetName, Map<String, List> mapListTest, String section)
			throws QAException
	{
		try
		{
			...
			switch (section)
			{
				case "Policy":
					...
					break;
				case "Product":
					fields = "Action;Index;Node;Element;Product;UnitState;CategoryDescription;SubUnit;Total;Coverage;SubCoverage;Identifier;Premium";
					break;
				case "Forms":
