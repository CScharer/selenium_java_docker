...
public class ParameterHelper
{
	...
	public static String getParameters(List<List<Object>> parameterList, boolean includeMethodVariables)
	{
		...
		try
		{
			...
			parameterHelper = new ParameterHelper(className, methodName, JavaHelpers.getCallingLineNumber());
			...
			for (int methodIndex = 0; methodIndex < methodArray.length; methodIndex++)
			{
				...
				// ASM 7.1 is now used exclusively (old asm:asm:3.3.1 excluded from QuickBooks dependency)
				...
				try
				{
					...
					for (MethodNode methodNode : methodNodeList)
					{
						if (methodNode.name.equals(methodName) && methodNode.desc.equals(methodDescriptor))
						{
							...
							parameterHelper = getParameterData(parameterHelper, method, methodNode, parameterList);
			...
			if (!methodFound)
			{
				Environment.sysOut("Could not find method [" + parameterHelper.toString()
						+ "].  Please ensure that the method is a public method.");
			}
		} catch (Exception e)
		{
			// e.printStackTrace();
			Environment.sysOut("Error in " + JavaHelpers.getCurrentMethodName() + "[" + e.getMessage() + "]");
		}
		return parameterHelper.toString();
	}

	/**
	 * Gets the parameter data based on static/non-static, parameters and/or
	 * variables.
	 * 
	 ...
	 * @return
	 */
	private static ParameterHelper getParameterData(ParameterHelper parameterHelper, Method method,
			MethodNode methodNode, List<List<Object>> parameterList)
	{
		try
		{
			if (parameterList.size() != method.getParameterCount())
			{
				Environment.sysOut("The argumentList passed in with a count of (" + parameterList.size()
						+ ") does not match the method.getParameterCount of (" + method.getParameterCount() + ").");
			}
			...
			if (INCLUDE_LOCAL_VARIABLES)
			{
				...
				if (!parameterHelper.getIsStatic())
				{
					...
					Parameter parameter = new Parameter((parameterHelper.getParameterList().size()),
							parameterFieldMap.get("name").toString(), parameterFieldMap.get("type").toString(),
