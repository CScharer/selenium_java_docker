...
public class DateHelpers
{
	...
	public static final String	FORMAT_US_STANDARD_DATE			= FORMAT_MM + DELIMETER + FORMAT_dd + DELIMETER
			+ FORMAT_yyyy;
	public static final String	FORMAT_US_STANDARD_TIME			= FORMAT_HH + ":" + FORMAT_mm + ":" + FORMAT_ss;
	public static final String	FORMAT_US_STANDARD_DATE_TIME	= FORMAT_US_STANDARD_DATE + " "
			+ FORMAT_US_STANDARD_TIME;
	public static final String	FORMAT_DATE_TIME_STAMP			= FORMAT_yyyy + FORMAT_MM + FORMAT_dd + "_" + FORMAT_HH
			+ FORMAT_mm + FORMAT_ss + "." + FORMAT_SSS;
	...
	public static String getDatePlusMinus(String date, String changeType, Integer value, String format)
	{
		try
		{
			boolean add = true;
	...
	public void getDatePlusMinusTestCase()
	{
		...
		Environment.sysOut(FORMAT_y + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_y, 1, FORMAT_yyyy));
		Environment.sysOut(FORMAT_M + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_M, 1, FORMAT_MM));
		Environment.sysOut(FORMAT_d + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_d, 1, FORMAT_dd));
		Environment.sysOut(FORMAT_H + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_H, 1, FORMAT_HH));
		Environment.sysOut(FORMAT_m + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_m, 1, FORMAT_mm));
		Environment.sysOut(FORMAT_s + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_s, 1, FORMAT_ss));
		Environment.sysOut(FORMAT_y + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_y, -1, FORMAT_yyyy));
		Environment.sysOut(FORMAT_M + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_M, -1, FORMAT_MM));
		Environment.sysOut(FORMAT_d + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_d, -1, FORMAT_dd));
		Environment.sysOut(FORMAT_H + Constants.PIPE + DateHelpers.getDatePlusMinus(now, FORMAT_H, -1, FORMAT_HH));
	...
	public static String getDatePlusMinusDays(String date, Integer days, String format)
	{
		...
		final DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern(format);
		final LocalDateTime now = LocalDateTime.of(Integer.valueOf(iDate[2]), Integer.valueOf(iDate[0]),
				Integer.valueOf(iDate[1]), 0, 0);
		LocalDateTime then;
		if (days < 0)
		{
			days = (days * -1);
	 ...
	 * HH:MM:SSS Used for logging date time during execution.
	 */
	public static String getCurrentDateAndTime()
	{
		return getCurrentDateTime(FORMAT_US_STANDARD_DATE + " " + FORMAT_US_STANDARD_TIME + "." + FORMAT_SSS);
	}
	...
	{
		final String pattern = FORMAT_EEEE + ", " + FORMAT_dd_MMM_yyyy + " " + FORMAT_KK + ":" + FORMAT_mm + ":"
				+ FORMAT_ss + " a";
		return validateTimeFormat(date, pattern);
	}
	...
	public static GregorianCalendar parseDate(String date)
	{
		...
		gregorianCalendar.set(Integer.parseInt(parts.get(2)), stringToMonth(parts.get(1)),
				Integer.parseInt(parts.get(0)));
		return gregorianCalendar;
	}

	public static int getDateDifference(GregorianCalendar gregorianCalendar2, GregorianCalendar gregorianCalendar1)
	{
		return (int) ((gregorianCalendar1.getTime().getTime() - gregorianCalendar2.getTime().getTime())
				/ (1000 * 60 * 60 * 24));
	}
	...
	public static int getWorkingDays(String dateStart, String dateEnd)
	{
		...
		if (!dateStart.equals(dateEnd))
		{
			...
			calendarStart.set(Integer.valueOf(dateArray[2]), Integer.valueOf(dateArray[0]),
					Integer.valueOf(dateArray[1]));
			dateArray = dateEnd.split(DELIMETER);
			calendarEnd.set(Integer.valueOf(dateArray[2]), Integer.valueOf(dateArray[0]),
					Integer.valueOf(dateArray[1]));
			if (calendarStart.getTimeInMillis() > calendarEnd.getTimeInMillis())
			{// Swap the dates if they've been passed backwards.
			...
			do
			{
				...
				switch (calendarStart.get(Calendar.DAY_OF_WEEK))
				{
					...
					case Calendar.FRIDAY:
						workDays++;
						Environment.sysOut(
								calendarStart.get(Calendar.DAY_OF_WEEK) + " - " + calendarStart.get(Calendar.MONTH)
										+ DELIMETER + calendarStart.get(Calendar.DAY_OF_MONTH) + DELIMETER
