...
public class FSO
{
	...
	public void testFilesFolders()
	{
		...
		displayListPath("filesList(pathTest, false, fileFilter)", filesList(pathTest, false, extension));
	 ...
	 * @throws QAException
	 */
	public static void fileCopy(File filePathNameSource, File filePathNameDestination) throws QAException
	{
		try
		{
	 ...
	 * @throws QAException
	 */
	public static void fileCopy(String filePathNameSource, String filePathNameDestination) throws QAException
	{
	...
	public static void fileDownloadFromURL(String exportURI, String filePathName) throws QAException
	{
		...
		do
		{
			...
			{
				Environment.sysOut(
						QAException.ERROR + JavaHelpers.getCurrentClassMethodName() + ":Error Downloading File.");
	...
	{
		final String folderName = filePathName.substring(0, filePathName.lastIndexOf(Constants.DELIMETER_PATH));
	...
	private static List<Path> filesList(String path, List<Path> fileList, boolean recursive, FileFilter fileFilter)
	{
	 ...
	 * @throws QAException
	 */
	public static void folderCopy(File filePathNameSource, File filePathNameDestination) throws QAException
	{
	...
	private static List<Path> pathsList(String path, List<Path> pathsList, boolean recursive, FileFilter fileFilter)
	{
		final File root = new File(path);
		File[] files = null;
		if (fileFilter == null)
	...
	private static String replaceEscapedCharacter(String value, String replacee, String replacer)
	{
		...
		while (index > -1)
		{
			...
			{
				valueNew = valueNew.substring(0, index) + "_"
	...
	public static FileFilter setFileFilter(String extension)
	{
		final FileFilter fileFilter = new FileFilter()
		{
			...
			{
				return fileName.isFile() && fileName.getName().toLowerCase().endsWith(extension.toLowerCase());
			}
		};
		return fileFilter;
	}
