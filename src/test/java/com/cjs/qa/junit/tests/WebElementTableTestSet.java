...
public class WebElementTableTestSet
{
	...
	private final String		PATH_OUTPUT			= Constants.PATH_FILES_DATA + this.getClass().getName()
			+ Constants.DELIMETER_PATH;
	private final String		BOXOFFICE_MOJO_DATE	= DateHelpers.getCurrentDatePlusMinusDays("YYYY-MM-dd", -2);
	...
	public void testSetup() throws Throwable
	{
		...
		Environment.sysOut("Setup-Test Method:[" + getMethodName() + "], methodTest:[" + methodTest + "]");
	}
	...
	{
		return Convert
				.fromListToDataTable(Arrays.asList(Arrays.asList("#1", getWebElementTable().getCellText(record, "#1")),
	...
	private DataTable getTableDataSearchHardCoded(int record)
	{
		...
		if (getMethodName().equals("tBoxOfficeSearch"))
		{
			...
			{
				case 1:
					dataTable = Convert.fromListToDataTable(
							Arrays.asList(Arrays.asList(LABEL_TITLE, "Jigsaw"), Arrays.asList("Weekend", "$16.6M"),
									Arrays.asList(LABEL_GROSS, "$16.6M"), Arrays.asList(LABEL_WEEKS, "1")));
					break;
				case 2:
					dataTable = Convert.fromListToDataTable(
							Arrays.asList(Arrays.asList(LABEL_TITLE, "Tyler Perry's Boo 2! A Madea Halloween"),
									Arrays.asList("Weekend", "$10.1M"), Arrays.asList(LABEL_GROSS, "$35.6M"),
									Arrays.asList(LABEL_WEEKS, "2")));
					break;
				case 3:
					dataTable = Convert.fromListToDataTable(
							Arrays.asList(Arrays.asList(LABEL_TITLE, "Geostorm"), Arrays.asList("Weekend", "$5.9M"),
									Arrays.asList(LABEL_GROSS, "$23.8M"), Arrays.asList(LABEL_WEEKS, "2")));
					break;
				case 4:
					dataTable = Convert.fromListToDataTable(Arrays.asList(Arrays.asList(LABEL_TITLE, "Happy Death Day"),
							Arrays.asList("Weekend", "$5.1M"), Arrays.asList(LABEL_GROSS, "$48.4M"),
							Arrays.asList(LABEL_WEEKS, "3")));
					break;
				case 5:
					dataTable = Convert.fromListToDataTable(Arrays.asList(
							Arrays.asList(LABEL_TITLE, "Blade Runner 2049"), Arrays.asList("Weekend", "$4.1M"),
							Arrays.asList(LABEL_GROSS, "$81.5M"), Arrays.asList(LABEL_WEEKS, "4")));
					break;
				case 6:
					dataTable = Convert.fromListToDataTable(Arrays.asList(
							Arrays.asList(LABEL_TITLE, "Thank You for Your Service"), Arrays.asList("Weekend", "$3.8M"),
							Arrays.asList(LABEL_GROSS, "$3.8M"), Arrays.asList(LABEL_WEEKS, "1")));
					break;
				case 7:
					dataTable = Convert.fromListToDataTable(Arrays.asList(Arrays.asList(LABEL_TITLE, "Only the Brave"),
							Arrays.asList("Weekend", "$3.5M"), Arrays.asList(LABEL_GROSS, "$12.0M"),
							Arrays.asList(LABEL_WEEKS, "2")));
					break;
				case 8:
					dataTable = Convert.fromListToDataTable(Arrays.asList(Arrays.asList(LABEL_TITLE, "The Foreigner"),
							Arrays.asList("Weekend", "$3.4M"), Arrays.asList(LABEL_GROSS, "$29.1M"),
							Arrays.asList(LABEL_WEEKS, "3")));
					break;
				case 9:
					dataTable = Convert.fromListToDataTable(
							Arrays.asList(Arrays.asList(LABEL_TITLE, "Suburbicon"), Arrays.asList("Weekend", "$2.8M"),
									Arrays.asList(LABEL_GROSS, "$2.8M"), Arrays.asList(LABEL_WEEKS, "1")));
					break;
				case 10:
					dataTable = Convert.fromListToDataTable(
							Arrays.asList(Arrays.asList(LABEL_TITLE, "It"), Arrays.asList("Weekend", "$2.5M"),
									Arrays.asList(LABEL_GROSS, "$323.9M"), Arrays.asList(LABEL_WEEKS, "8")));
					break;
				default:
					break;
			}
	...
	private void readWriteSysOut(int startColumn, int startRow, int endColumn, int endRow)
	{
		...
		for (int rowIndex = startRow; rowIndex <= endRow; rowIndex++)
		{
			for (int columnIndex = startColumn; columnIndex <= (endColumn); columnIndex++)
			{
				...
				final String cellAttribute = getWebElementTable().getCellAttribute(rowIndex, columnName, "class");
				Environment.sysOut("Column Name By Index (Column Index:[" + columnIndex + "]):[" + columnName
						+ "], Column Index By Name (Column Name:[" + columnName + "]):[" + columnTableIndex
						+ "], Cell Value By Name:[" + cellValue + "], Cell Value By Index[" + cellValueIndex
						+ "], Cell 'class' Attribute By Index[" + cellAttribute + "]");
			}
		}
		Environment.sysOut("Name:[" + getWebElementTable().getColumnName(2) + "], Index:["
				+ getWebElementTable().getColumnIndex(LABEL_TITLE) + "], Value:["
				+ getWebElementTable().getCellText((rowCount + 1), 2) + "]");
		Environment.sysOut("Name:[" + getWebElementTable().getColumnName((columnCount + 1)) + "], Index:["
				+ getWebElementTable().getColumnIndex(LABEL_TITLE) + "], Value:["
				+ getWebElementTable().getCellText((rowCount + 1), 2) + "]");
	}
	...
	private void setTable()
	{
		...
		switch (getMethodName())
		{
			...
			case "tBoxOfficeSearch":
				url = "http://www.imdb" + IExtension.COM
			...
			case "tBoxOfficeMojo_34":
				...
				if (days.equals("0"))
				{
					url = "http://www.boxofficemojo" + IExtension.COM + "/daily/chart/?sortdate="
				...
				} else
				{
					...
					// getBOXOFFICE_MOJO_DATE() + "&p="+ IExtension.HTML
					url = "http://www.boxofficemojo" + IExtension.COM + "/daily/chart/?view=" + days + "day&sortdate="
							+ getBOXOFFICE_MOJO_DATE() + "&p=" + IExtension.HTML;
				}
				if (days.equals("1"))
				{
		...
		setWebElementTable(new WebElementTable(getWebDriver(), xPathTable, xPathHeadings, xPathRows, xPathCells));
	...
	public void tBoxOfficeMojo_00()
	{
		...
		getWebElementTable().highlightObjects(false);
		getWebElementTable()
				.exportTable(getPATH_OUTPUT() + getMethodName() + "_" + getBOXOFFICE_MOJO_DATE() + IExtension.CSV);
		getWebElementTable().reset();
	}
	...
	public void tBoxOfficeSearch()
	{
		...
		for (int searchRecord = 1; searchRecord <= rowsCount; searchRecord++)
		{
			...
			Environment.sysOut("matchingRecordsExist:[" + getWebElementTable().matchingRecordsExist(map) + "]");
			Environment.sysOut("matchingRecordsCount:[" + getWebElementTable().matchingRecordsCount(map) + "]");
			Environment.sysOut("matchingRecordsList:[" + getWebElementTable().matchingRecordsList(map) + "]");
		}
		...
		for (final String listWeek : listWeeks)
		{
			if (!listWeeksTested.contains(listWeek))
			{
				...
				Environment.sysOut("matchingRecordsExist (listWeek:[" + listWeek + "]):["
						+ getWebElementTable().matchingRecordsExist(map) + "]");
				Environment.sysOut("matchingRecordsCount (listWeek:[" + listWeek + "]):["
						+ getWebElementTable().matchingRecordsCount(map) + "]");
				Environment.sysOut("matchingRecordsList (listWeek:[" + listWeek + "]):["
