...
public class BitcoinTestSet
{
	...
	private String	mavenCommand	= new TestRunCommand(this.getClass().getName(), getTestName()).toString();
	...
	{
		Environment.sysOut("getCurrentMethodDebugName:[" + JavaHelpers.getCurrentMethodDebugName() + "]");
		...
		try
		{
			...
			do
			{
				...
				String dateTimeStamp = DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME);
	...
	private StringBuilder appendRecord(StringBuilder stringBuilderSQL, Bitcoin bitcoin) throws Throwable
	{
		...
		if (!stringBuilderSQL.toString().equals("") && stringBuilderSQL.toString().split(Constants.NEWLINE).length >= 1)
		{
