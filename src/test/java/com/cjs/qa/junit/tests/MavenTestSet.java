...
public class MavenTestSet
{
	...
	{
		final String projectAnalyze = "C:" + Constants.DELIMETER_PATH + "Workspace" + Constants.DELIMETER_PATH
				+ "Github" + Constants.DELIMETER_PATH + "cjs-app" + Constants.DELIMETER_PATH + "cjs-app-gui"
				+ Constants.DELIMETER_PATH + "pom" + IExtension.XML;
		final String PATH_WORKSPACE = "C:" + Constants.DELIMETER_PATH + "Workspace" + Constants.DELIMETER_PATH
				+ "Github" + Constants.DELIMETER_PATH;
		final String resultsPathFile = Constants.PATH_FILES_DATA + "MavenDependenciesResults" + IExtension.LOG;
	...
	private String runDependencyCheck(String batchPathFile, String projectPath, String resultsPathFile) throws Exception
	{
		...
		stringBuilderBatch.append(
				newLine("mvn dependency:analyze>" + Constants.QUOTE_DOUBLE + resultsPathFile + Constants.QUOTE_DOUBLE));
		...
		final String command = "cmd /C " + Constants.QUOTE_DOUBLE + batchPathFile + Constants.QUOTE_DOUBLE;
		...
		if (status.equals("0"))
		{
			final String projectFolder = projectPath.substring((projectPath.lastIndexOf(Constants.DELIMETER_PATH) + 1));
			...
			for (final String record : records)
			{
				...
				if (dependenciesSection)
				{
					...
					{ // Add Undeclared Dependency
						if (record.contains(WARNING_DEPENDENCY) && !record.equalsIgnoreCase(WARNING_UNDECLARED))
						{
	...
	private String sortAddDependenciesUndeclared(List<String> dependenciesList)
	{
		...
		for (final String dependency : dependenciesList)
		{
			...
			final String dependencyNew = dependencyOld.replace(dependencyObject.getVersion(),
					dependencyObject.getVersionTag());
			stringBuilderD.append(dependencyNew);
		}
		stringBuilder
				.append(newLine("Dependency Properties (Versions) Undeclared (" + dependenciesObjectList.size() + ")"));
		stringBuilder.append(stringBuilderP.toString());
		if (dependenciesObjectList.isEmpty())
		{
			stringBuilder.append(newLine("None"));
		}
		stringBuilder.append(newLine("Dependencies Undeclared (" + dependenciesObjectList.size() + ")"));
