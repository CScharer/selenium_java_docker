...
{
	private static void sendEmail(String from, String password, String subject, String body, String attachment)
			throws Throwable
	{
		body += " (from " + from + ")";
	...
	private String						mavenCommand	= new TestRunCommand(this.getClass().getName(), getTestName())
			.toString();
	...
	private static String[]				listStatus		= "failed;finished;skipped;starting;succeeded"
	...
	public TestWatcher					testWatcher		= new TestWatcher()
														{
															...
															private void updateTestWatcher(Description description,
																	String status)
															{
																testWatcherList
																		.add(Arrays.asList(description.getClassName(),
																				description.getMethodName(),
																				description.getDisplayName(), status));
	...
	{
		Environment.sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "], TestName"
				+ ":[" + getTestName() + "]");
		...
		switch (methodTest)
		{
			...
			case "marlboro":
				...
				{
					Assert.fail("The month day [" + monthDay + "] is not the expected day [" + dayExpect + "].");
					break;
				}
	...
	public void Core() throws Throwable
	{
		...
		try
		{
			...
			wmic.setTrace("ON");
			wmic.setRecord(Constants.QUOTE_DOUBLE + Constants.DELIMETER_PATH + "wrbts" + Constants.DELIMETER_PATH
					+ "shared" + Constants.DELIMETER_PATH + "bts QA" + Constants.DELIMETER_PATH + "QATools"
					+ Constants.DELIMETER_PATH + "Logs" + Constants.DELIMETER_PATH + wmic.getNode() + "_"
					+ dateTimeStamp + IExtension.XML + Constants.QUOTE_DOUBLE);
			wmic.setInteractive("OFF");
			wmic.setFailfast("ON");
			wmic.setUser("btsqa");
			wmic.setPassword("runb@byrun");
			wmic.setOutput(Constants.QUOTE_DOUBLE + Constants.DELIMETER_PATH + "wrbts" + Constants.DELIMETER_PATH
					+ "shared" + Constants.DELIMETER_PATH + "bts QA" + Constants.DELIMETER_PATH + "QATools"
					+ Constants.DELIMETER_PATH + "Logs" + Constants.DELIMETER_PATH + wmic.getNode() + "_"
					+ dateTimeStamp + IExtension.LOG + Constants.QUOTE_DOUBLE);
			wmic.setOutput("STDOUT");
			// wmic.setAppend("append")
			wmic.setAggregate("ON");
			// wmic.setAuthority("authority")
			wmic.wmicAlias.setAlias(null);
			wmic.setCommand("CMD /C " + Constants.QUOTE_DOUBLE + "C:" + Constants.DELIMETER_PATH + "Users"
					+ Constants.DELIMETER_PATH + "btsqa" + Constants.DELIMETER_PATH + "Desktop"
					+ Constants.DELIMETER_PATH + "TEST_WMIC" + IExtension.BAT + Constants.QUOTE_DOUBLE);
	...
	public void Dropbox() throws Throwable
	{
		...
		sqlStringBuilder.append(JDBCConstants.SELECT_ALL + JDBCConstants.FROM + "[v_Dropbox_ActiveAccounts]");
	...
	{
		Environment.sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		getEveryoneSocial().SignInPage.login(CJSConstants.EMAIL_ADDRESS_VIVIT, EPasswords.EVERYONE_SOCIAL.getValue());
	...
	public void Jenkins() throws Throwable
	{
		...
		for (int browserIndex = 0; browserIndex < browserList.size(); browserIndex++)
		{
			...
			final WebDriverValidation webDriverValidation = new WebDriverValidation(
					getSeleniumWebDriver().getWebDriver());
	...
	private void jenkins_WebDriverValidate(String browser, SoftAssert softAssert) throws Throwable
	{
		...
		{
			setSeleniumWebDriver(new SeleniumWebDriver(browser, Environment.isRunRemote(), null, null, null));
		}
		Environment.sysOut("Testing Browser:[" + getSeleniumWebDriver().getBrowser() + "]");
		try
		{
			List<String> urlList = Arrays.asList("https://www.google" + IExtension.COM
	...
	public void jenkins_Colors() throws Throwable
	{
		...
		getSeleniumWebDriver().initializeWebDriver();
		getSeleniumWebDriver().getWebDriver().get("https://htmlcolorcodes" + IExtension.COM + "/color-names/");
		final WebDriver webDriver = getSeleniumWebDriver().getWebDriver();
		// final Selenium selenium = new Selenium(webDriver)
		final List<WebElement> sectionsWebElement = webDriver.findElements(By.xpath("/html/body/main/article/section"));
		final List<ColorPalette> colorPaletteList = new ArrayList<>();
		for (final WebElement sectionWebElement : sectionsWebElement)
		{
			final List<WebElement> rowsWebElement = sectionWebElement.findElements(By.xpath("./table/tbody/tr"));
	...
	{
		final String FILE_PATHNAME = Constants.PATH_DESKTOP + "PluralSightSessionInformation" + IExtension.TXT;
		...
		listSessions.add(URL_PLURALSIGHT + IExtension.COM + "/library/courses/effective-email-communication");
		listSessions.add(URL_PLURALSIGHT + IExtension.COM
				+ "/library/courses/working-communicating-with-different-personalities");
		listSessions.add(URL_PLURALSIGHT + IExtension.COM + "/library/courses/project-management-beginners-guide");
		listSessions
				.add(URL_PLURALSIGHT + IExtension.COM + "/library/courses/project-scope-managing-resources-defining");
		listSessions
				.add(URL_PLURALSIGHT + IExtension.COM + "/library/courses/technical-writing-software-documentation");
		listSessions.add(URL_PLURALSIGHT + IExtension.COM + "/library/courses/test-driven-development-java");
		// final String url =
		...
		{
			final String sessionInformation = pluralSight.SessionPage.getSessionInformation(listSessions.get(index));
			FSO.fileWrite(FILE_PATHNAME, sessionInformation, true);
			final String tableOfContents = pluralSight.TableOfContentsPage.getTableOfContents(listSessions.get(index));
	...
	public void LinkedIn() throws Throwable
	{
		...
		getLinkedIn().LoginPage.login(getLinkedIn().LoginAlternatePage, CJSConstants.EMAIL_ADDRESS_MSN,
	...
	public void Marlboro() throws Throwable
	{
		...
		sqlStringBuilder.append(JDBCConstants.WHERE + "[UserName] " + JDBCConstants.NOT + "IN('ZScharer') ");
		sqlStringBuilder.append(JDBCConstants.ORDER_BY + "[UserName];");
		final List<Map<String, String>> listMapUsers = jdbc.queryResultsString(sqlStringBuilder.toString(), false);
		...
		for (final Map<String, String> mapUser : listMapUsers)
		{
			...
			{
				softAssert.assertEquals("THANK YOU FOR YOUR REQUEST.",
						getMarlboro().OffersAndActivityPage.getlabelMessage(), mapUser.get("UserName"));
				JavaHelpers.sleep(2);
			} else
			{
	...
	public void marlboro_EarnPoints() throws Throwable
	{
		Environment.sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		JDBC jdbc = new JDBC("", DATABASE_QA);
		String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + "[t_Marlboro] " + JDBCConstants.WHERE
				+ "[UserName] IN ('AScharer') " + JDBCConstants.ORDER_BY + "[UserName]";
	...
	public void testQueryAccess() throws Throwable
	{
		...
		String month = String.format("%02d", (Integer.valueOf(DateHelpers.getCurrentDateTime("MM")) - 1));
		if (month.equals("0"))
		{
			month = "12";
		}
	...
	public void United() throws Throwable
	{
		...
		getUnited().MemberAccountUpdatePage.validateMemberAccountInformation(
				"2211 Grand Ave Unit 3\nWest Des Moines, IA 502655628", CJSConstants.EMAIL_ADDRESS_MSN,
				"+1 (515) 681-7770");
