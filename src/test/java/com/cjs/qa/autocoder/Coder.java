...
public class Coder
{
	...
	protected static final List<String>	TYPES_INPUT				= Arrays
			.asList("Checkbox;Dropdown;Edit;Option;RadioButton".split(Constants.DELIMETER_LIST));
	...
	private static void getRecordsFromExcel(String pageName) throws QAException, Exception
	{
		...
		jdbc.executeUpdate(
				JDBCConstants.INSERT_INTO + "[tObjectPages] ([Page],[URL]) VALUES ('" + pageName + "','URL');", false);
		...
		for (int row = 1; row <= rows; row++)
		{
			...
			StringBuilder stringBuilder = SQL.appendStringBuilderSQLInsertRecord("tObjects", new StringBuilder(),
					recordMap, true);
	...
	public static String createCode(String currentPage) throws QAException, Exception
	{
		...
		Environment.sysOut("stringBuilder.toString():[" + stringBuilder.toString() + "]");
		List<Map<Integer, String>> recordsCodeListMap = jdbc.queryResultsIndex(stringBuilder.toString(), true);
		stringBuilder = new StringBuilder();
		stringBuilder.append(JDBCConstants.SELECT_DISTINCT + "[Type], [Name], [PagePopulate], [PageValidate] ");
		stringBuilder.append(JDBCConstants.FROM + "[vMethods_All] ");
		stringBuilder.append(JDBCConstants.WHERE + "[Page]='" + currentPage + "' ");
		stringBuilder.append(JDBCConstants.ORDER_BY + "[Name];");
		Environment.sysOut("stringBuilder.toString():[" + stringBuilder.toString() + "]");
		List<Map<Integer, String>> recordsOtherListMap = jdbc.queryResultsIndex(stringBuilder.toString(), true);
		stringBuilder = new StringBuilder();
		...
		for (Map<Integer, String> mapRecord : recordsCodeListMap)
		{
			...
			methodCode = methodCode
					.replaceAll(Constants.SYMBOL_TRADEMARK + "OBJECT_TYPE" + Constants.SYMBOL_TRADEMARK, type)
					.replaceAll(Constants.SYMBOL_TRADEMARK + "OBJECT_NAME" + Constants.SYMBOL_TRADEMARK, methodName)
	...
	private static String getDocumentation(List<Map<Integer, String>> recordsOtherListMap,
			Map<String, Integer> mapFields)
	{
		...
		for (Map<Integer, String> mapRecord : recordsOtherListMap)
		{
			...
			switch (type)
			{
				case LABEL_BUTTON:
				case "Link":
					stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "click" + type
							+ methodName + ":" + Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
							+ "Clicks the [" + name + "] " + type + ".");
					break;
				case LABEL_CHECKBOX:
					stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "get" + type
							+ methodName + ":" + Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
							+ "Gets the value of the [" + name + "] " + type + LABEL_FIELD);
					stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "set" + type
							+ methodName + ":" + Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
							+ "Sets the value of the [" + name + "] " + type + LABEL_FIELD);
					stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "toggle" + type
							+ methodName + ":" + Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
							+ "Toggles the value of the [" + name + "] Field regardless of what the current value is.");
					break;
				case LABEL_DROPDOWN:
				case LABEL_EDIT:
				case LABEL_OPTION:
		...
		stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + METHOD_NAME_POPULATE + ":"
				+ Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
				+ "Populates the value of the all of the fields.");
		stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + METHOD_NAME_VALIDATE + ":"
				+ Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1)
				+ "Validates the value of the all of the fields.");
		return stringBuilder.toString();
	}

	private static String getInputs(List<Map<Integer, String>> recordsOtherListMap, Map<String, Integer> mapFields)
	{
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(Constants.nlTab(1, 0));
		...
		for (Map<Integer, String> mapRecord : recordsOtherListMap)
		{
			...
			switch (type)
			{
				...
				case LABEL_CHECKBOX:
					stringBuilder
							.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "|" + name + "|checked|");
					break;
				default:
					stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + Constants.nlTab(0, 1) + "|" + name + "|"
							+ methodName + "|");
					break;
			}
		}
	...
	private static String getTable(List<Map<Integer, String>> recordsOtherListMap, Map<String, Integer> mapFields)
	{
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(Constants.nlTab(1, 0));
		stringBuilder.append(Constants.nlTab(1, 1) + COMMENT + "TABLE DEF");
		stringBuilder
				.append(Constants.nlTab(1, 1) + COMMENT + " DataTable dataTable = Convert.toDataTable(Arrays.asList(");
		for (Map<Integer, String> mapRecord : recordsOtherListMap)
		{
			String type = mapRecord.get(mapFields.get("Type"));
			String name = mapRecord.get(mapFields.get("Name"));
			switch (type)
			{
				...
				case "Label":
					break;
				case LABEL_CHECKBOX:
					stringBuilder.append(Constants.nlTab(1, 3) + COMMENT + "Arrays.asList(" + Constants.QUOTE_DOUBLE
							+ name + Constants.QUOTE_DOUBLE + ", " + Constants.QUOTE_DOUBLE + "checked"
							+ Constants.QUOTE_DOUBLE + "),");
					break;
				default:
					stringBuilder.append(Constants.nlTab(1, 3) + COMMENT + "Arrays.asList(" + Constants.QUOTE_DOUBLE
							+ name + Constants.QUOTE_DOUBLE + ", " + Constants.QUOTE_DOUBLE + name
							+ Constants.QUOTE_DOUBLE + "),");
					break;
			}
		}
	...
	private static String getPagePopulateValidate(List<Map<Integer, String>> recordsOtherListMap,
			Map<String, Integer> mapFields)
	{
		StringBuilder stringBuilder = new StringBuilder();
		// Populate
	...
	private static String getPageCasePopulate(String type, String name, String methodName)
	{
		...
		{
			stringBuilder.append(Constants.nlTab(1, 4) + "case " + Constants.QUOTE_DOUBLE + name.toLowerCase()
					+ Constants.QUOTE_DOUBLE + ":");
			String methodCall = "";
			switch (type)
			{
	...
	{
		List<String> cases = Arrays.asList("Checkbox;Dropdown;Edit;RadioButton".split(Constants.DELIMETER_LIST));
		StringBuilder stringBuilder = new StringBuilder();
	...
	private static String getPagePost(String methodName)
	{
		...
		stringBuilder.append(Constants.nlTab(1, 5) + "value = " + Constants.QUOTE_DOUBLE + "[" + Constants.QUOTE_DOUBLE
				+ " + field + " + Constants.QUOTE_DOUBLE + "]" + Constants.QUOTE_DOUBLE
				+ " + ISelenium.FIELD_NOT_CODED;");
		stringBuilder.append(Constants.nlTab(1, 5) + "Environment.sysOut(" + Constants.QUOTE_DOUBLE + "["
				+ Constants.QUOTE_DOUBLE + " + field + " + Constants.QUOTE_DOUBLE + "]" + Constants.QUOTE_DOUBLE
				+ " + ISelenium.FIELD_NOT_CODED);");
		stringBuilder.append(Constants.nlTab(1, 5) + LABEL_BREAK);
		...
		if (methodName.equals(METHOD_NAME_VALIDATE))
		{
			stringBuilder.append(
					Constants.nlTab(1, 2) + "Assert.assertSame(this.getClass().getName() + " + Constants.QUOTE_DOUBLE
							+ "validatePage" + Constants.QUOTE_DOUBLE + ", expected.toString(), actual.toString());");
		}
		stringBuilder.append(Constants.nlTab(1, 1) + "}");
		return stringBuilder.toString();
	}
	...
	private static String getPagePre(String methodName)
	{
		...
		stringBuilder.append(Constants.nlTab(1, 1) + "public void " + methodName + "(DataTable dataTable) {");
		if (methodName.equals(METHOD_NAME_VALIDATE))
		{
			stringBuilder.append(Constants.nlTab(1, 2) + "Map<String, String> expected = new HashMap<>();");
		...
		stringBuilder.append(Constants.nlTab(1, 3) + "if (!value.equals(" + Constants.QUOTE_DOUBLE
				+ Constants.QUOTE_DOUBLE + ")) {");
		stringBuilder.append(Constants.nlTab(1, 4) + "Environment.sysOut(" + Constants.QUOTE_DOUBLE + "{Field}"
				+ Constants.QUOTE_DOUBLE + " + field + " + Constants.QUOTE_DOUBLE + ", {Value}" + Constants.QUOTE_DOUBLE
				+ " + value" + ");");
		if (methodName.equals(METHOD_NAME_VALIDATE))
		{
