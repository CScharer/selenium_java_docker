...
public class Steps_Vivit extends Environment
{
	...
	{
		mapTest.put("Started", DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME + ".SSS"));
	...
	public void testTeardown() throws QAException
	{
		...
		mapTest.put("Completed", DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME + ".SSS"));
		timeCompleted = System.currentTimeMillis();
		mapTest.put("Elapsed (seconds)", String.valueOf((timeCompleted - timeStarted) / Constants.MILLISECONDS));
	...
	public void pGiven(String scenarioName) throws Throwable
	{
		...
		if (randomNumber.equals("1"))
		{
			...
			addScenarioError("Random Error Generator. randomNumber:[" + randomNumber + "]" + Constants.NEWLINE
	...
	public void board_of_Directors_Page(String scenarioName, String url) throws Throwable
	{
		...
		Environment.sysOut("board_of_Directors_Page-scenarioName;[" + scenarioName + "], url:[" + url + "]");
		webDriver.get(url);
	}

	@Then("^Board Member \"([^\"]*)\" Exists \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\", \"([^\"]*)\"$")
	public void board_Member_Exists(String name, String title, String eMail, String company, String address,
			String country, String duties) throws Throwable
	{
		Environment.sysOut(
				"board_Member_Exists-name:[" + name + "], title:[" + title + "], eMail:[" + eMail + "], company:["
						+ company + "], address:[" + address + "], country:[" + country + "], duties:[" + duties + "]");
		final DataTable table = Convert.fromListToDataTable(Arrays.asList(Arrays.asList("Name", name),
				Arrays.asList("Title", title), Arrays.asList("eMail", eMail), Arrays.asList("Company", company),
				Arrays.asList("Address", address), Arrays.asList("Country", country), Arrays.asList("Duties", duties)));
	...
	{
		Environment.sysOut(
				"staff_Member_Exists-nameTitle:[" + nameTitle + "], eMail:[" + eMail + "], phone:[" + phone + "]");
		final DataTable table = Convert.fromListToDataTable(Arrays.asList(Arrays.asList("Name-Title", nameTitle),
				Arrays.asList("eMail", eMail), Arrays.asList("Phone", phone)));
		Environment.sysOut("table:[" + table.toString() + "]");
