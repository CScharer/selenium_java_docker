...
public class GTWAPIMethods
{
	...
	public void createGTWNamespaceClasses()
	{
		...
		try
		{
			...
			final String gtwNamespaceClassPath = Constants.PATH_PROJECT + "src" + Constants.DELIMETER_PATH + "test"
					+ Constants.DELIMETER_PATH + "java" + Constants.DELIMETER_PATH + thisClassPath
			...
			for (String namespace : getGTWNamespaces(jdbc))
			{
				...
				List<Map<String, String>> gtwMethodMapList = jdbc.queryResultsString(sqlStringBuilder.toString(),
						false);
				StringBuilder stringBuilderClass = new StringBuilder();
				for (int gtwMethodMapListIndex = 0; gtwMethodMapListIndex < gtwMethodMapList
						.size(); gtwMethodMapListIndex++)
				{
	...
	private StringBuilder getMethod(StringBuilder stringBuilder, Map<String, String> gtwMethodMap)
	{
		...
		stringBuilder.append("\n\t\treturn getAPIJSONResponse(credentials, \"" + requestMethod
