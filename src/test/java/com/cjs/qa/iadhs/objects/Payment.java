...
{
	public static final String			HEADINGS							= "Payment Withheld;Payment Received;Payment Distrib;Payment Amount;Type of Payment";
	public static final List<String>	HEADING_LIST						= Arrays
			.asList(HEADINGS.split(Constants.DELIMETER_LIST));
	...
	public static final String			FILE_IADHS_PAYMENTS_CUMULATIVE		= IaDhsEnvironment.FOLDER_DATA + "Data"
			+ Constants.DELIMETER_PATH + DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_yyyyMMdd)
			+ "_IA DHS Child Support Payments" + IExtension.XLS;
	public static final String			REPORT_IADHS_PAYMENTS_CUMULATIVE	= FILE_IADHS_PAYMENTS_CUMULATIVE
			.replaceAll(IExtension.XLS, IExtension.HTM);
	...
	public Payment(String paymentWithheld, String paymentReceived, String paymentDistrib, Double paymentAmount,
	...
	private boolean append(Map<String, String> paymentMap) throws Throwable
	{
		...
		if (!exists(paymentMap))
		{
			...
			sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(TABLE_IADHS_PAYMENTS, sqlStringBuilder,
					paymentMap, true);
	...
	public void append(List<Payment> paymentList) throws Throwable
	{
		...
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + "[" + VIEW_IADHS_PAYMENTS_CUMULATIVE + "];";
		...
		String report = getPaymentTable(header, footer, fieldsList, listMapReport, appendedDistribDateList);
		FSO.fileWrite(REPORT_IADHS_PAYMENTS_CUMULATIVE, report, false);
		JDBC.exportDataFromTableView(VIEW_IADHS_PAYMENTS_CUMULATIVE, FILE_IADHS_PAYMENTS_CUMULATIVE, "IADHS-Payments",
				VivitData.DATABASE_DEFINITION, true);
		String body = toString() + Constants.NEWLINE + report + Constants.NEWLINE + IaDhsEnvironment.EMAIL_SIGNATURE;
		Email.sendEmail(CJSConstants.EMAIL_ADDRESS_GMAIL, EPasswords.EMAIL_GMAIL.getValue(),
				CJSConstants.EMAIL_ADDRESS_GMAIL, "", "", subject, body, Arrays.asList(FILE_IADHS_PAYMENTS_CUMULATIVE));
	}
	...
	public boolean exists(Map<String, String> paymentMap)
	{
		...
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.FROM + "[" + TABLE_IADHS_PAYMENTS + "] ");
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.WHERE + "[" + LBLHEADING_PAYMENT_WITHHELD + "]='"
				+ paymentMap.get(LBLHEADING_PAYMENT_WITHHELD) + "' ");
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.AND + "[" + LBLHEADING_PAYMENT_RECIEVED + "]='"
				+ paymentMap.get(LBLHEADING_PAYMENT_RECIEVED) + "' ");
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.AND + "[" + LBLHEADING_PAYMENT_AMOUNT + "]='"
				+ paymentMap.get(LBLHEADING_PAYMENT_AMOUNT) + "' ");
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.AND + "[" + LBLHEADING_TYPE_OF_PAYMENT + "]='"
				+ paymentMap.get(LBLHEADING_TYPE_OF_PAYMENT) + "' ");
		sqlStringBuilder.append(Constants.NEWLINE + JDBCConstants.AND + "[" + LBLHEADING_PAYMENT_DISTRIB + "]='"
				+ paymentMap.get(LBLHEADING_PAYMENT_DISTRIB) + "';");
		Environment.sysOut("stringBuilderSQL:" + sqlStringBuilder.toString());
		JDBC jdbc = new JDBC("", VivitData.DATABASE_DEFINITION);
		List<Map<String, String>> resultsMapList = jdbc.queryResultsString(sqlStringBuilder.toString(), true);
	...
	public static String getPaymentTable(String header, String footer, List<String> fields,
			List<Map<String, String>> listMapReport, List<String> appendedDistribDateList) throws Throwable
	{
		Environment.sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		Environment.sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("header", header),
				Arrays.asList("footer", footer), Arrays.asList("fields", fields))));
		int tab = 0;
			...
			for (final String field : fields)
			{
				...
				} else
				{
					if (lastPayment)
					{
						stringBuilderTable.append(Constants.nlTab(1, tab++) + "<td style=" + Constants.QUOTE_DOUBLE
								+ "border: 1px solid black; border-collapse: collapse; background-color: "
								+ ColorsHEX.YELLOW.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
					} else
					{
						stringBuilderTable.append(Constants.nlTab(1, tab++) + "<td style=" + Constants.QUOTE_DOUBLE
