...
public class CasePaymentsPage extends Page
{
	...
	public void getPreviousPayments() throws Throwable
	{
		...
		List<String> headingList = validateHeadingList();
		List<WebElement> previousPaymentRecordsWebElementList = webDriver.findElements(previousPaymentRecordsBy);
		...
		for (int previousPaymentRecordsWebElementListIndex = recordsCheck; previousPaymentRecordsWebElementListIndex > 0; previousPaymentRecordsWebElementListIndex--)
		{
			WebElement previousPaymentRecordsWebElement = previousPaymentRecordsWebElementList
					.get(previousPaymentRecordsWebElementListIndex);
			highlightCurrentElementBriefly(previousPaymentRecordsWebElement, FLASH_MILLISECONDS);
			List<WebElement> previousPaymentWebElementList = previousPaymentRecordsWebElement
					.findElements(By.xpath(".//a[2]"));
			getRecordWebElementsPayment(headingList, previousPaymentWebElementList);
		}
		getPayment().append(getPaymentList());
	}

	private void getRecordWebElementsPayment(List<String> headingList, List<WebElement> paymentWebElementList)
			throws Throwable
	{
		...
		for (int webElementListIndex = 0; webElementListIndex < paymentWebElementList.size(); webElementListIndex++)
		{
	...
	public List<String> validateHeadingList()
	{
		...
		for (int headingWebElementIndex = 0; headingWebElementIndex < headingWebElementList
				.size(); headingWebElementIndex++)
		{
		...
		String message = "The headings do not match!" + Constants.NEWLINE
