...
public class SecurityPage extends Page
{
	...
	final By	optionDontRememberThisDevice	= By
			.xpath(".//*[@id='authQuestionsForm']/div/div/label[@for='IsRememberDevice_False']");
	...
	public void answerSecurityQuestions() throws QAException
	{
		...
		final List<String> questionsList = Arrays
				.asList(EPasswords.UNITED_SECURITY_QUESTIONS.getValue().split(Constants.DELIMETER_LIST));
		final List<String> answersList = Arrays
				.asList(EPasswords.UNITED_SECURITY_ANSWERS.getValue().split(Constants.DELIMETER_LIST));
		...
		List<WebElement> questionsWebElementList = webDriver
				.findElements(By.xpath(".//*[@id='authQuestionsForm']/div/fieldset/legend"));
		for (int indexWebElement = 0; indexWebElement < questionsWebElementList.size(); indexWebElement++)
		{
