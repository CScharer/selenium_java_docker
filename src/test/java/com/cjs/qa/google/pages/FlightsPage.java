...
public class FlightsPage extends Page
{
	...
	public static final String	HEADINGS_FLIGHT			= "Airport;Preference;Sort;Airline;Operated By;Time Depart;Time Arrive;Duration;Stops;Price";
	public static final String	HEADINGS_SUMMARY		= "Traveler;Airport/Location Depart;Time Depart;Airport/Location Arrive;Time Arrive;Stops;Price;URL";
	public static final String	bestDepartingFlights	= ".//div[@class='gws-flights-results__best-flights']";
	public static final String	otherDepartingFlights	= ".//div[contains(@aria-labelledby,'gws-flights-results__other_flights_heading')]";
	public static final String	departingFlightsRecord	= "//div[contains(@class,'gws-flights-results__collapsed-itinerary')]";
	public static final By		bestDepartingFlightsBy	= By.xpath(bestDepartingFlights + departingFlightsRecord);
	public static final By		otherDepartingFlightsBy	= By.xpath(otherDepartingFlights + departingFlightsRecord);
	public static final By		errorPage				= By
			.xpath(".//p[@role='status'][.='Oops, something went wrong.']");
	...
	public void getFlightsOld(String filePathName, List<Map<String, String>> airportListMap, String airportTo,
			String dateDepartTo, String dateDepartFrom) throws Throwable
	{
		...
		for (Map<String, String> airportMap : airportListMap)
		{
			...
			if (airportFrom.equals(airportTo))
			{
				...
				excel.addLink(sheetSummary, summaryMap.get("Airport/Location Depart"), row, "URL", addressFrom, url);
			...
			} else
			{
				...
				for (int flightWebElementListIndex = 0; flightWebElementListIndex < flightWebElementList
						.size(); flightWebElementListIndex++)
				{
					WebElement flightWebElement = flightWebElementList.get(flightWebElementListIndex);
					flightList.add(setFlight(airportFrom, "Best", (flightWebElementListIndex + 1), flightWebElement));
				}
				...
				{
					WebElement flightWebElement = flightWebElementList.get(flightWebElementListIndex);
					flightList.add(setFlight(airportFrom, "Other", (flightWebElementListIndex + 1), flightWebElement));
				}
				for (int flightIndex = 0; flightIndex < flightList.size(); flightIndex++)
				{
					...
					if (flightIndex == 0)
					{
						excel.addLink(sheetSummary, summaryMap.get("Airport/Location Depart"), row, "DOCUMENT",
								airportFrom, "'" + sheetName + "'!A2");
						excel.writeCell(sheetSummary, summaryMap.get("Time Depart"), row, flight.getTimeDepart());
						excel.writeCell(sheetSummary, summaryMap.get("Airport/Location Arrive"), row, airportTo);
						excel.writeCell(sheetSummary, summaryMap.get("Time Arrive"), row, flight.getTimeArrive());
					...
					excel.writeCell(sheetName, flightMap.get("Airport"), (flightIndex + 1), flight.getAirport());
					excel.writeCell(sheetName, flightMap.get("Preference"), (flightIndex + 1), flight.getPreference());
					excel.writeCell(sheetName, flightMap.get("Sort"), (flightIndex + 1), flight.getSort());
					excel.writeCell(sheetName, flightMap.get("Airline"), (flightIndex + 1), flight.getAirline());
					excel.writeCell(sheetName, flightMap.get("Operated By"), (flightIndex + 1), flight.getOperatedBy());
					excel.writeCell(sheetName, flightMap.get("Time Depart"), (flightIndex + 1), flight.getTimeDepart());
					excel.writeCell(sheetName, flightMap.get("Time Arrive"), (flightIndex + 1), flight.getTimeArrive());
					excel.writeCell(sheetName, flightMap.get("Duration"), (flightIndex + 1), flight.getDuration());
	...
	public Flight getDriveData(Map<String, String> driverMap, String airportTo)
	{
		...
		return new Flight(airportFrom, "Driving", 1, addressFrom, traveler, "Up to you", duration, duration,
				"Up to you", Double.valueOf(price));
	}

	public List<Flight> getFlights(String airportFrom, String airportTo, String dateDepartTo, String dateDepartFrom)
			throws Throwable
	{
		String url = getURLFlight(airportFrom, airportTo, dateDepartTo, dateDepartFrom);
		Environment.sysOut("url:[" + url + "]");
	...
	public Flight setFlight(String airportFrom, String preference, int index, WebElement flightRecordWebElement)
	{
		...
		price = price.replaceAll(",", "");
		Flight flight = new Flight(airportFrom, preference, index, airline, airlingOperatedBy, timeDepart, duration,
				timeArrive, stops, Double.valueOf(price));
	...
	private static String getURLFlight(String airportFrom, String airportTo, String dateDepartTo, String dateDepartFrom)
