...
public class EventsNamespace extends YMService
{
	...
	{
		return Arrays.asList("RegisterID;RsvpID;DateRegister;LastName;FirstName;WebsiteID;ID;RsvpResponse"
				.split(Constants.DELIMETER_LIST));
	}

	public static List<String> customList() throws Throwable
	{
		return Arrays.asList("Custom_JobLevel;Custom_JobFunction;Custom_MayHPEemailyouofferssupportdatesandeventnews"
				.split(Constants.DELIMETER_LIST));
	}
	...
	{
		return Arrays
				.asList("strEmail;strWorkTitle;strEmployerName;strAddress1;strAddress2;strCity;strLocation;strPostalCode;strCountry;strPhone;strRSVPComments;Custom_AdditionalComments"
						.split(Constants.DELIMETER_LIST));
	}
	...
	{
		return Arrays
				.asList("Name;EventID;StartDateTime;EndDateTime;LocationName;LocationAddress1;LocationAddress2;LocationCity;LocationLocation;LocationPostalCode;LocationCountry;ContactName;ContactEmail;ContactPhone;ShortDescription"
						.split(Constants.DELIMETER_LIST));
	}
	...
	{
		return Arrays
				.asList("Status;EventName;FirstName;LastName;BadgeNumber;RegistrationID;DateRegistered;Attended;OrderID;MemberID;InvoiceID;IsPrimaryRegistrant;PrimaryRegistrantRegistrationID;PrimaryRegistrantMemberID;AttendeeType"
						.split(Constants.DELIMETER_LIST));
	}
	...
	public Map<String, String> allSearch(String searchText, int pageSize, long startRecord) throws Throwable
	{
		// Returns a list of Community Events based on the supplied search term.
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Events_All_Search"
				+ IExtension.HTM);
		...
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Events.All.Search" + Constants.QUOTE_DOUBLE + ">");
		if (searchText != null)
		{
	...
	public Map<String, String> eventAttendeesGet(int eventID) throws Throwable
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC
				+ "Events_Event_Attendees_Get" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Events.Event.Attendees.Get" + Constants.QUOTE_DOUBLE + ">");
	...
	public Map<String, String> eventGet(int eventID) throws Throwable
	{
		...
		Environment.sysOut(
				JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Events_Event_Get" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Events.Event.Get" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 2) + "<EventID>" + eventID + "</EventID>");
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_SUFFIX);
		return getAPIXMLResponse("POST", stringBuilder.toString());
	}
