...
public class YMService
{
	...
	public static final String	API_HEADER		= Constants.nlTab(1, 0) + XML.HEADING_INFO + Constants.nlTab(1, 0)
			+ "<YourMembership>";
	...
	public YMService()
	{
		if (!serviceActive)
		{
			try
			{
				final HttpURLConnection httpUrlConnection = (HttpURLConnection) new URL(URL_YM).openConnection();
				httpUrlConnection.setRequestMethod("HEAD");
				final int responseCode = httpUrlConnection.getResponseCode();
				if (!((responseCode >= HttpURLConnection.HTTP_OK)
						&& (responseCode < HttpURLConnection.HTTP_BAD_REQUEST)))
				{
					Environment.sysOut(CONNECTED_TO + URL_YM + "] unsuccessful with response of [" + responseCode + ":"
				...
				Assert.assertTrue(
						CONNECTED_TO + URL_YM + "] unsuccessful with response of [" + responseCode + ":"
								+ IHTTP.getResponseValue(responseCode) + "].",
						((responseCode >= HttpURLConnection.HTTP_OK)
								&& (responseCode < HttpURLConnection.HTTP_BAD_REQUEST)));
				Environment.sysOut(CONNECTED_TO + URL_YM + "] successfull with response of [" + responseCode + ":"
						+ IHTTP.getResponseValue(responseCode) + "].");
				serviceActive = true;
	...
	public static Map<String, String> getAPIXMLResponse(String requestMethod, String apiRequest) throws Throwable
	{
		...
		{
			Environment.sysOut(JavaHelpers.getCurrentMethodName() + " Parameters:" + Constants.nlTab(1, 1)
					+ "requestMethod:[" + requestMethod + "]" + Constants.nlTab(1, 1) + "URL_YM:[" + URL_YM + "]"
