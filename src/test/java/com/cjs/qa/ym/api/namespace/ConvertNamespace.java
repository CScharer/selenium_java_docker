...
public class ConvertNamespace extends YMService
{
	public Map<String, String> toEasternTime(String localTime, String localGmtBias) throws Throwable
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Convert_ToEasternTime"
				+ IExtension.HTM);
		...
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Convert.ToEasternTime" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 2) + "<LocalTime>" + localTime + "</LocalTime>");
		stringBuilder.append(Constants.nlTab(1, 2) + "<LocalGmtBias>" + localGmtBias + "</LocalGmtBias>");
