...
public class SessionNamespace extends YMService
{
	...
	public static String getSessionID() throws Throwable
	{
		...
		VivitEnvironment.sysOut("timeSessionStart:[" + timeSessionStart + "], timeSessionCurrent:[" + timeSessionCurrent
				+ "], timeSessionElapsed:[" + timeSessionElapsed + "]");
	...
	public Map<String, String> abandon() throws Throwable
	{
		...
		Environment.sysOut(
				JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Session_Abandon" + IExtension.HTM);
		final String apiRequest = getSessionID() + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Session.Abandon" + Constants.QUOTE_DOUBLE + ">" + YMAPI.LABEL_CALL_METHOD_SUFFIX;
	...
	public static Map<String, String> create(boolean refreshSession) throws Throwable
	{
		...
		Environment.sysOut(
				JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Session_Create" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Session.Create" + Constants.QUOTE_DOUBLE + ">");
	...
	public Map<String, String> ping() throws Throwable
	{
		...
		Environment.sysOut(
				JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Session_Ping" + IExtension.HTM);
		final String apiRequest = getSessionID() + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Session.Ping" + Constants.QUOTE_DOUBLE + ">" + YMAPI.LABEL_CALL_METHOD_SUFFIX;
		return getAPIXMLResponse("POST", apiRequest);
	}
}