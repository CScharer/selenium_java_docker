...
public class Sa_NonMembersNamespace extends YMService
{
	public Map<String, String> allGetIDs(String timestamp, int websiteID, String groups) throws Throwable
	{
		...
		// <ServerGmtBias>.
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Sa_NonMembers_All_GetIDs"
				+ IExtension.HTM);
		...
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.NonMembers.All.GetIDs" + Constants.QUOTE_DOUBLE + ">");
	...
	public Map<String, String> profileCreate() throws Throwable
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC
				+ "Sa_NonMembers_Profile_Create" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID() + SessionNamespace.getSAPasscode());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.NonMembers.Profile.Create" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_SUFFIX);
		return getAPIXMLResponse("POST", stringBuilder.toString());
	}
}