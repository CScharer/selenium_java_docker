...
public class Sa_EventsNamespace extends YMService
{
	public Map<String, String> eventMap() throws Throwable
	{
		final List<String> fields = Arrays
				.asList("Name;EventID;StartDateTime;EndDateTime;LocationName;LocationAddress1;LocationAddress2;LocationCity;LocationLocation;LocationPostalCode;LocationCountry;ContactName;ContactEmail;ContactPhone;ShortDescription;Active"
	...
	{
		final List<String> fields = Arrays
				.asList("ID;Name;Description;Presenter;SeatLimit;StartDateTime;EndDateTime;CreatedDate;UpdatedDate"
						.split(Constants.DELIMETER_LIST));
		...
		if (!arrayEventIDs.isEmpty())
		{
			...
			for (int index = 0; index < (arrayEventIDs.size() - 1); index++)
			{
				...
				for (int indexEvent = 0; indexEvent < nodesEvent.getLength(); indexEvent++)
				{
					...
					for (int indexSessionGroup = 0; indexSessionGroup < nodesSessionGroup
							.getLength(); indexSessionGroup++)
					{
						...
						sXpath = XPATH_ROOT + "/SessionGroups/SessionGroup" + "[@ID='" + groupID + "']" + "[@Name='"
								+ groupName + "']";
						final NodeList nodesGroupSession = XML.getNodeList(sXML, sXpath);
						for (int indexGroupSession = 0; indexGroupSession < nodesGroupSession
								.getLength(); indexGroupSession++)
						{
							...
							for (final String key : sessionMap.keySet())
							{
								...
								final String value = elementGroupSession.getElementsByTagName(key).item(0)
										.getTextContent();
					...
					for (int indexUngroupedSession = 0; indexUngroupedSession < nodesUngroupedSession
							.getLength(); indexUngroupedSession++)
					{
						final Map<String, String> sessionMap = sessionMap();
						for (final String key : sessionMap.keySet())
						{
							...
							final String value = elementUngroupedSession.getElementsByTagName(key).item(0)
									.getTextContent();
							sessionMap.put(key, value);
						}
						Environment.sysOut("sessionMap:[" + sessionMap.toString() + "]");
				...
				sXML = mapResults.get("xml");
				final List<String> arrayRegistrationIDs = XML.getNodes(sXML, "Sa.Events.Event.Registrations.GetIDs",
						"RegistrationID");
	...
	public Map<String, String> allGetIDs(String startDate, String endDate, String name, String status,
			String lastModifiedDate) throws Throwable
	{
		// Returns a list of Events for the community optionally filtered by
		// date or event name.
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Sa_Events_All_GetIDs"
				+ IExtension.HTM);
		...
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.Events.All.GetIDs" + Constants.QUOTE_DOUBLE + ">");
		if (startDate != null)
		...
		if (lastModifiedDate != null)
			stringBuilder
					.append(Constants.nlTab(1, 2) + "<LastModifiedDate>" + lastModifiedDate + "</LastModifiedDate>");
	...
	public Map<String, String> eventGet(int eventID) throws Throwable
	{
		...
		// information.
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC + "Sa_Events_Event_Get"
				+ IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID() + SessionNamespace.getSAPasscode());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.Events.Event.Get" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 2) + "<EventID>" + eventID + "</EventID>");
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_SUFFIX);
		return getAPIXMLResponse("POST", stringBuilder.toString());
	}

	public Map<String, String> eventRegistrationGet(String registrationID, String badgeNumber) throws Throwable
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC
				+ "Sa_Events_Event_Registration_Get" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID() + SessionNamespace.getSAPasscode());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.Events.Event.Registration.Get" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 2) + "<RegistrationID>" + registrationID + "</RegistrationID>");
	...
	public Map<String, String> eventRegistrationsGetIDs(int eventID, String status) throws Throwable
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentClassMethodName() + " - " + URL_YM_API_DOC
				+ "Sa_Events_Event_Registrations_GetIDs" + IExtension.HTM);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(SessionNamespace.getSessionID() + SessionNamespace.getSAPasscode());
		stringBuilder.append(Constants.nlTab(1, 1) + YMAPI.LABEL_CALL_METHOD_PREFIX + Constants.QUOTE_DOUBLE
				+ "Sa.Events.Event.Registrations.GetIDs" + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append(Constants.nlTab(1, 2) + "<EventID>" + eventID + "</EventID>");
