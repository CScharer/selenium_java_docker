...
public class UnmarshallYourMembership_Response
{
	...
	private YourMembership_Response get(SOAPMessage soapMessage) throws QAException
	{
		...
		try
		{
			for (final Iterator<jakarta.xml.soap.Node> bodyIterator = soapMessage.getSOAPBody().getChildElements(); bodyIterator
					.hasNext();)
			{
				final SOAPElement soapBody = (SOAPElement) bodyIterator.next();
				for (final Iterator<jakarta.xml.soap.Node> dataIterator = soapBody.getChildElements(); dataIterator.hasNext();)
				{
					...
					} catch (final JAXBException e)
					{
						throw new QAException(
								"Error Creating (context = JAXBContext.newInstance(YourMembership_Response.class);).",
								e);
					}
					...
					{
						throw new QAException("Error Creating (unmarshaller = context.createUnmarshaller();).", e);
					}
					try
					{
					...
					{
						throw new QAException(
								"Error Creating (yourMembership_Response = (YourMembership_Response) unmarshaller.unmarshal(node);).",
								e);
					}
				}
			}
		...
		{
			throw new QAException(
					"Error In (for (final Iterator<SOAPElement> bodyIterator = soapMessage.getSOAPBody().getChildElements(); bodyIterator.hasNext();)).",
					e);
		}
