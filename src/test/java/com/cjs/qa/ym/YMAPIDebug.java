...
public class YMAPIDebug
{
	...
	{
		final String dateTimeCurrent = DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME);
		...
		// Auth Namespace
		mapResults = ymAPI.AuthNamespace.authenticate(mapCScharer.get("userID"), mapCScharer.get("password"));
		mapResults = ymAPI.AuthNamespace.createToken(mapCScharer.get("userID"), mapCScharer.get("password"), true,
		...
		mapResults = ymAPI.MemberNamespace.certificationsJournalGet(true, "startDate", "entryID", "certificationID",
				100, 1);
		...
		mapResults = ymAPI.MemberNamespace.connectionApprove(Integer.valueOf(mapCScharer.get("webSiteMemberID")), true);
		...
		mapResults = ymAPI.MemberNamespace.passwordInitializeReset(mapCScharer.get("userID"),
				mapCScharer.get("emailAddress"));
		mapResults = ymAPI.MemberNamespace.passwordUpdate("resetToken", mapCScharer.get("password"),
		...
		// Namespace
		mapResults = ymAPI.Sa_AuthNamespace.authenticate(mapCScharer.get("userID"), mapCScharer.get("password"),
		...
		// Sa.Events Namespace
		mapResults = ymAPI.Sa_EventsNamespace.allGetIDs(ymInceptionDate, dateTimeCurrent, "name", null,
				ymInceptionDate);
		...
		mapResults = ymAPI.Sa_ExportNamespace.eventRegistrations(false, "eventID", "sessionIDs", "productID", "All",
				"lastName", true);
		...
		mapResults = ymAPI.Sa_FinanceNamespace.invoicePaymentCreate("invoiceNo", "amount", "type", "refNo",
				dateTimeCurrent, "payerName", "payerEmail", "billOrganization", false, false, false, "billAddress1",
				"billAddress2", "billCity", "billLocation", "billPostalCode", "billCountry", "billPhone",
				"paymentNotes", "internalNotes");
		// Sa.Groups Namespace
		mapResults = ymAPI.Sa_GroupsNamespace.groupCreate("groupCode", "typeID", "name", false, true, "accessibility",
				"membership", "messaging", false, false, true, false, "emailApproval", "emailOptionsAdmin",
				"emailOptionsMember", "shortDescription", "welcomeContent");
		mapResults = ymAPI.Sa_GroupsNamespace.groupGetMembershipLog("groupID", ymInceptionDate, "itemID");
		mapResults = ymAPI.Sa_GroupsNamespace.groupUpdate("groupID", "groupCode", "typeID", "name", false, true,
				"accessibility", "membership", "messaging", false, false, true, false, "emailApproval",
				"emailOptionsAdmin", "emailOptionsMember", "shortDescription", "welcomeContent");
		mapResults = ymAPI.Sa_GroupsNamespace.groupTypesGet();
		// Sa.Member Namespace
		mapResults = ymAPI.Sa_MemberNamespace.certificationsGet("iD", true);
		mapResults = ymAPI.Sa_MemberNamespace.certificationsJournalGet("iD", true, ymInceptionDate, "entryID",
				"certificationID", 100, 1);
		...
		mapResults = ymAPI.Sa_MembersNamespace.certificationsJournalEntryCreate("iD", "cEUsEarned", "",
				"certificationID", "certificationName", "description", "", "scorePercent", "creditTypeCode");
		...
		mapResults = ymAPI.Sa_MembersNamespace.groupsAdd("iD", "groupCode", (Boolean) null, (Boolean) null,
				(Boolean) null, "");
		mapResults = ymAPI.Sa_MembersNamespace.groupsRemove("iD", "groupCode", (Boolean) null, (Boolean) null, "");
		...
		mapResults = ymAPI.Sa_PeopleNamespace.profileFindID("importID", "constituentID", 0, "username", "email");
