...
import com.cjs.qa.ym.api.dataobjects.UnmarshallYourMembership_Response;
import com.cjs.qa.ym.api.dataobjects.YourMembership_Response;
...
public class YMData extends Environment
{
	 ...
	 * @throws Throwable
	 */
	public static boolean addMissingFields(String apiNamespace, String tableName, List<String> fieldsAddList)
			throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(VivitData.LABEL_API_NAMESPACE, apiNamespace),
				Arrays.asList(VivitData.LABEL_TABLE_NAME, tableName), Arrays.asList("fieldsAddList", fieldsAddList))));
		...
		{
			sysOut("Added apiNamespace[" + apiNamespace + "], table[" + tableName + "], fieldName[" + fieldName + "]");
			...
			sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(VivitTables.VIVIT_DATABASE_CHANGES,
					sqlStringBuilder, fieldNameMap);
		}
	...
	public static void exportGTWEventAttendees() throws Throwable
	{
		...
		do
		{
			...
			{
				Environment.sysOut(QAException.ERROR + JavaHelpers.getCurrentClassMethodName()
	...
	public static void exportGTWEvents() throws Throwable
	{
		...
		do
		{
			...
			{
				Environment.sysOut(
						QAException.ERROR + JavaHelpers.getCurrentClassMethodName() + ":Error Extracting Event Data.");
				if (attempt > ATTEMPTS_MAX)
				{
					throw new QAException(QAException.ERROR + "[" + e.getMessage() + "]", e);
				}
	...
	{
		if (FSO.fileExists(VivitFoldersFiles.DATA_YMAPI_DATA))
		{ // The file has already been created.
			return;
		}
		JDBC.exportDataFromTableView(VivitTables.VIVIT_EVENTS_CURRENT, VivitFoldersFiles.DATA_YMAPI_DATA, "Events",
				VivitData.DATABASE_DEFINITION, true);
		JDBC.exportDataFromTableView(VivitTables.VIVIT_EVENTINFORMATION_CURRENT, VivitFoldersFiles.DATA_YMAPI_DATA,
				"EventInformation", VivitData.DATABASE_DEFINITION, false);
		JDBC.exportDataFromTableView(VivitViews.VIVIT_EVENT_ATTENDEES_CURRENT, VivitFoldersFiles.DATA_YMAPI_DATA,
				"EventAttendees", VivitData.DATABASE_DEFINITION, false);
		JDBC.exportDataFromTableView(VivitTables.VIVIT_YMGTW_LINKS_CURRENT, VivitFoldersFiles.DATA_YMAPI_DATA,
				"YMGTWLinks", VivitData.DATABASE_DEFINITION, false);
		JDBC.exportDataFromTableView(VivitTables.VIVIT_EVENT_REGISTRATION_IDS_CURRENT,
				VivitFoldersFiles.DATA_YMAPI_DATA, "EventRegistrationIDs", VivitData.DATABASE_DEFINITION, false);
		JDBC.exportDataFromTableView(VivitTables.VIVIT_EVENT_REGISTRATION_CURRENT, VivitFoldersFiles.DATA_YMAPI_DATA,
				"EventRegistration", VivitData.DATABASE_DEFINITION, false);
	}
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper
				.getParameters(Arrays.asList(Arrays.asList(VivitData.LABEL_DATE_TIME_FROM, dateTimeFrom))));
		if (getYmapi() == null)
		{
			setYmApi(new YMAPI());
		}
		Map<String, String> resultsMap = getYmApi().Sa_ExportNamespace.exportDataMemberGroups(dateTimeFrom);
	...
	public static void getAllEventAttendees() throws Throwable
	{
		...
		try
		{
			...
			{
				String filePathName = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_ATTENDEES + eventID + IExtension.XML;
				String xml;
				...
				{ // Create the Event Attendee information.
					...
					Map<String, String> eventAttendeeMap = getYmapi().EventsNamespace
							.eventAttendeesGet(Integer.parseInt(eventID));
	...
	public static void getAllEventInformation() throws Throwable
	{
		...
		try
		{
			...
			for (String eventID : eventIDList)
			{
				String filePathName = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_INFORMATION + eventID + IExtension.XML;
				String xml;
				if (!FSO.fileExists(filePathName))
				{ // Create the Event Information information.
					...
					Map<String, String> eventInformationMap = getYmapi().EventsNamespace
							.eventGet(Integer.parseInt(eventID));
	...
	public static void getAllEventRegistrationIDs() throws Throwable
	{
		...
		try
		{
			...
			for (String eventID : eventIDList)
			{
				String filePathName = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION_IDS + eventID
						+ IExtension.XML;
				String xml;
				if (!FSO.fileExists(filePathName))
				{ // Create the Event RegistrationIDs information.
					...
					Map<String, String> eventInformationMap = getYmApi().Sa_EventsNamespace
	...
	public static void getAllEventRegistration() throws Throwable
	{
		...
		try
		{
			...
			{
				String xml = "";
				String filePathName = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION + registrationID
						+ IExtension.XML;
				if (!FSO.fileExists(filePathName))
				{ // Create the Event Registration information.
					...
					Map<String, String> eventInformationMap = getYmapi().Sa_EventsNamespace
							.eventRegistrationGet(registrationID, "");
					xml = eventInformationMap.get("xml");
					FSO.fileWrite(filePathName, xml, false);
				}
			}
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(pageRecordStart, "pageRecordStart"))));
		try
		{
			boolean recordsExist = false;
			do
			{
				String filePathName = VivitFoldersFiles.PATH_API_DATA_YM_EVENTS + pageRecordStart + IExtension.XML;
				String xml;
				if (!FSO.fileExists(filePathName))
				{ // Create the Event information.
					...
					Map<String, String> mapResults = getYmapi().EventsNamespace.allSearch(getEventNameSearch(),
							EventsNamespace.PAGES_MAX, pageRecordStart);
	...
	public static List<String> getEventRegistrationIDListAll() throws Throwable
	{
		...
		List<String> registrationIDFileList = FSO.filesList(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION_IDS);
	 ...
	 * @throws Throwable
	 */
	public static List<Map<String, String>> getEventInformationMapListAll(String xml) throws Throwable
	{
		...
		for (int eventInformationIndex = 0; eventInformationIndex < eventInformationNodeList
				.getLength(); eventInformationIndex++)
		{
			...
			for (final String key : sessionMap.keySet())
			{
				...
				final String value = eventInformationElement.getElementsByTagName(key).item(0).getTextContent();
	...
	public static List<Map<String, String>> getEventMapListAll(String xml) throws Throwable
	{
		...
		for (int resultIndex = 0; resultIndex < resultNodeList.getLength(); resultIndex++)
		{
			...
			for (int eventIndex = 0; eventIndex < eventNodeList.getLength(); eventIndex++)
			{
				...
				getResults().getItem().add(
						new Item(sessionMap.get("eventName"), sessionMap.get("eventID"), sessionMap.get("eventDate")));
				getEventList().add(
						new Event(sessionMap.get("eventDate"), sessionMap.get("eventID"), sessionMap.get("eventName")));
			}
		}
	...
	public static List<Map<String, String>> getEventRegistrationIDsMapListAll(String xml) throws Throwable
	{
		...
		for (int registrationIDIndex = 0; registrationIDIndex < registrationIDNodeList
				.getLength(); registrationIDIndex++)
		{
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(tableBackupName, "tableBackupName"))));
	...
	public static StringBuilder importGoToWebinarEventAttendees() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList())));
		StringBuilder sqlStringBuilder = new StringBuilder();
		List<String> eventAttendeeFileList = FSO.filesList(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_ATTENDEES);
		for (String filePathName : eventAttendeeFileList)
		{
			...
			sysOut("filePathName:[" + filePathName + "], eventID:[" + eventID + "], attendeeNodeList.getLength():["
					+ attendeeNodeList.getLength() + "], xml:[" + Constants.NEWLINE + xml + "]");
			if (attendeeNodeList.getLength() > 0)
			{
				for (int attendeeNodeIndex = 0; attendeeNodeIndex < attendeeNodeList.getLength(); attendeeNodeIndex++)
				{
					...
					for (int attendeeChildNodeIndex = 0; attendeeChildNodeIndex < attendeeChildrenNodeList
							.getLength(); attendeeChildNodeIndex++)
					{
						...
						if (attendeeChildNode.getNodeType() == Node.ELEMENT_NODE)
						{
							...
							final String attendeeNodeValue = attendeeElement.getElementsByTagName(attendeeNodeName)
							...
							} else
							{
								...
								for (int dataSetChildNodeIndex = 0; dataSetChildNodeIndex < dataSetChildrenNodeList
										.getLength(); dataSetChildNodeIndex++)
								{
									...
									if (dataSetChildNode.getNodeType() == Node.ELEMENT_NODE)
									{
										...
										String valueNodeValue = dataSetChildNode.getAttributes()
												.getNamedItem("ExportValue").getNodeValue();
					...
					sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(VivitTables.VIVIT_EVENT_ATTENDEES_CURRENT,
							sqlStringBuilder, attendeeMap, true);
				}
			}
		}
	...
	public static StringBuilder importGoToWebinarEventInformation() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		StringBuilder sqlStringBuilder = new StringBuilder();
		List<String> eventFileList = FSO.filesList(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_INFORMATION);
		for (String filePathName : eventFileList)
		{
			String xml = FSO.fileReadAll(filePathName);
			for (Map<String, String> sessionMap : getEventInformationMapListAll(xml))
			{
				sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(VivitTables.VIVIT_EVENTINFORMATION_CURRENT,
						sqlStringBuilder, sessionMap, true);
			}
		}
		return sqlStringBuilder;
	}
			...
			{ // Add the IDs for YM & GTW
				if (isValidEventGTW(sessionMap))
				{
					...
					if (JavaHelpers.isValidStringNumber(gtwID))
					{
						...
						SQL.appendStringBuilderSQLInsertRecord(VivitTables.VIVIT_YMGTW_LINKS_CURRENT, sqlStringBuilder,
								linkMap, true);
					}
				}
			}
	...
	public static StringBuilder importGoToWebinarEvents() throws Throwable
	{
		...
		for (String filePathName : eventFileList)
		{
			...
			{
				sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(VivitTables.VIVIT_EVENTS_CURRENT,
						sqlStringBuilder, sessionMap, true);
			}
		}
		return sqlStringBuilder;
	...
	public static StringBuilder importGoToWebinarEventRegistration() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList())));
		StringBuilder sqlStringBuilder = new StringBuilder();
		List<String> eventRegistrationFileList = FSO.filesList(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION);
		for (String filePathName : eventRegistrationFileList)
		{
			String xml = FSO.fileReadAll(filePathName);
			Document document = XML.createDocument(xml);
			// //Sa.Events.Event.Registration.Get/Registration
			NodeList eventRegistrationNodeList = document.getElementsByTagName("Sa.Events.Event.Registration.Get");
			sysOut("filePathName:[" + filePathName + "], eventRegistrationNodeList.getLength():["
					+ eventRegistrationNodeList.getLength() + "], xml:[" + Constants.NEWLINE + xml + "]");
			if (eventRegistrationNodeList.getLength() > 0)
			{
				for (int eventRegistrationNodeIndex = 0; eventRegistrationNodeIndex < eventRegistrationNodeList
						.getLength(); eventRegistrationNodeIndex++)
				{
					final Node eventRegistrationNode = eventRegistrationNodeList.item(eventRegistrationNodeIndex);
					...
					Map<String, String> eventRegistrationMap = EventsNamespace
							.eventRegistrationWithDataSetAndCustomMap();
					for (int eventRegistrationChildNodeIndex = 0; eventRegistrationChildNodeIndex < eventRegistrationChildrenNodeList
							.getLength(); eventRegistrationChildNodeIndex++)
					{
						Node eventRegistrationChildNode = eventRegistrationChildrenNodeList
								.item(eventRegistrationChildNodeIndex);
						if (eventRegistrationChildNode.getNodeType() == Node.ELEMENT_NODE)
						{
							...
							final String eventRegistrationNodeValue = eventRegistrationElement
									.getElementsByTagName(eventRegistrationNodeName).item(0).getTextContent();
							if (!eventRegistrationNodeName.equals("DataSet"))
							{
								...
								{
									sysOut("The [" + eventRegistrationNodeName
											+ "] item does not exist in the eventRegistrationMap!!!");
								}
							} else
							{
					...
					sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(
							VivitTables.VIVIT_EVENT_REGISTRATION_CURRENT, sqlStringBuilder, eventRegistrationMap, true);
				}
			}
		}
	...
	public static StringBuilder importGoToWebinarEventRegistrationIDs() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList())));
		StringBuilder sqlStringBuilder = new StringBuilder();
		List<String> eventRegistrationFileList = FSO
				.filesList(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION_IDS);
		for (String filePathName : eventRegistrationFileList)
		{
			String eventID = filePathName.replace(VivitFoldersFiles.PATH_API_DATA_YM_EVENT_REGISTRATION_IDS, "");
			eventID = eventID.replace(IExtension.XML, "");
			String xml = FSO.fileReadAll(filePathName);
			...
			for (Map registrationIDMap : registrationIDMapList)
			{
				...
				sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(
						VivitTables.VIVIT_EVENT_REGISTRATION_IDS_CURRENT, sqlStringBuilder, registrationIDMap, true);
			}
		}
		Environment.sysOut(sqlStringBuilder.toString().split(Constants.NEWLINE).length);
		return sqlStringBuilder;
	 ...
	 * @throws Throwable
	 */
	public static List<StringBuilder> namespaceImport(String filePathName, String apiNamespace, String tableName)
			throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		Environment.sysOut(
				ParameterHelper.getParameters(Arrays.asList(Arrays.asList(VivitData.LABEL_FILE_PATH_NAME, filePathName),
				...
				if (recordNumber >= (RECORD_START + 1))
				{
					if (!headingsMapped)
					{
						...
						if (!headingsCSVList.equals(headingsExpectedList))
						{
							...
							{
								Assert.fail("Expected headings (headingsExpectedMissingList["
										+ headingsExpectedMissingList.toString()
										+ "]) do not match CSV headings and could not be removed from tableName["
										+ tableName + "]");
							}
							if (!headingsCSVMissingList.isEmpty()
									&& !addMissingFields(apiNamespace, tableName, headingsCSVMissingList))
							{
								Assert.fail("CSV headings (headingsCSVMissingList[" + headingsCSVMissingList.toString()
										+ "]) do not match Expected DB headings and could not be added to tableName["
										+ tableName + "]");
							}
						}
					...
					sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(tableName, sqlStringBuilder, mapMember,
							true);
	...
	public static void pullFromYM(String dateTimeFrom) throws Throwable
	{
		...
		do
		{
			try
			{
				...
				if (!FSO.fileExists(VivitFoldersFiles.DATA_YMAPI_DATA_MEMBERS))
				{ // Export Members from YMAPI
					exportMembers(dateTimeFrom);
				}
				if (!FSO.fileExists(VivitFoldersFiles.DATA_YMAPI_DATA_MEMBER_GROUPS))
			...
			{
				Environment
						.sysOut(QAException.ERROR + JavaHelpers.getCurrentClassMethodName() + ":Pulling data from YM.");
				if (attempt > ATTEMPTS_MAX)
				{
					throw new QAException(QAException.ERROR + "[" + e.getMessage() + "]", e);
				}
	...
	{
		List<String> apiNamespaceListTrue = Arrays.asList("Sa_ExportNamespace.exportDataMembers",
				"Sa_ExportNamespace.exportDataMemberGroups");
		...
		SQL.executeVivit(LABEL_DROP_TABLE,
		...
		SQL.executeVivit(LABEL_CREATE_TABLE,
				VivitTables.DOM_VIVIT_MEMBERS_PREVIOUS + "-" + VivitTables.DOM_VIVIT_MEMBERS_PREVIOUS,
				sqlStringBuilder);
		for (int apiNamespaceIndex = 0; apiNamespaceIndex < apiNamespaceListAll.size(); apiNamespaceIndex++)
		{
			...
			if (apiNamespaceListTrue.contains(apiNamespace))
			{
				if (apiNamespaceIndex == 0)
				{
					sqlStringBuilder = new StringBuilder();
					sqlStringBuilder
							.append(JDBCConstants.DELETE_FROM + "[" + VivitTables.VIVIT_DATABASE_CHANGES + "];");
				...
				sqlStringBuilder.append(JDBCConstants.AND + "[" + VivitData.LABEL_TABLE_NAME + "] = '"
						+ VivitTables.DOM_VIVIT_MEMBERS + "';");
				sqlStringBuilder.append(Constants.NEWLINE);
				SQL.executeVivit(LABEL_DELETE_FROM, VivitTables.VIVIT_DATABASE_CHANGES + "-" + apiNamespace,
						sqlStringBuilder);
			}
			List<StringBuilder> sqlStringBuilderList = new ArrayList<>();
			sqlStringBuilder = new StringBuilder();
			switch (apiNamespace)
			{
				case "Sa_ExportNamespace.exportDataMembers":
					sqlStringBuilder.append(
							JDBCConstants.DELETE_FROM + "[" + VivitTables.DOM_VIVIT_MEMBERS + "];" + Constants.NEWLINE);
					SQL.executeVivit(LABEL_DELETE_FROM, VivitTables.DOM_VIVIT_MEMBERS, sqlStringBuilder);
					sqlStringBuilderList.addAll(namespaceImport(VivitFoldersFiles.DATA_YMAPI_DATA_MEMBERS, apiNamespace,
							VivitTables.DOM_VIVIT_MEMBERS));
					break;
				case "Sa_ExportNamespace.exportDataMemberGroups":
					sqlStringBuilder.append(JDBCConstants.DELETE_FROM + "[" + VivitTables.DOM_VIVIT_MEMBER_GROUPS + "];"
							+ Constants.NEWLINE);
					SQL.executeVivit(LABEL_DELETE_FROM, VivitTables.DOM_VIVIT_MEMBER_GROUPS, sqlStringBuilder);
					sqlStringBuilderList.addAll(namespaceImport(VivitFoldersFiles.DATA_YMAPI_DATA_MEMBER_GROUPS,
							apiNamespace, VivitTables.DOM_VIVIT_MEMBER_GROUPS));
					break;
				case "MemberProducts":
					sqlStringBuilder.append(JDBCConstants.DELETE_FROM + "[" + VivitTables.DOM_VIVIT_MEMBER_PRODUCTS
							+ "];" + Constants.NEWLINE);
					SQL.executeVivit(LABEL_DELETE_FROM, VivitTables.DOM_VIVIT_MEMBER_PRODUCTS, sqlStringBuilder);
	 ...
	 * @throws Throwable
	 */
	public static boolean removeExtraFields(String apiNamespace, String tableName, List<String> fieldsRemoveList)
			throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(VivitData.LABEL_API_NAMESPACE, apiNamespace),
		...
		{
			sysOut("Removed apiNamespace[" + apiNamespace + "], table[" + tableName + "], fieldName[" + fieldName
					+ "]");
			Map<String, String> fieldNameMap = fieldNameMapTemp;
			fieldNameMap.put("ApiMethod", apiNamespace);
			fieldNameMap.put("TableName", tableName);
	...
	public static List<StringBuilder> updateMemberProducts() throws Throwable
	{
		...
		final List<String> delimitedFieldList = Arrays
				.asList("Micro_Focus_Product_Centers;Micro_Focus_Products_Used;Big_Data_Software_Products_Used;Micro_Focus_Backup_&_Governance_Products_Used;Micro_Focus_ITOM_Products_Used;Micro_Focus_Application_Delivery_Products_Used"
						.split(Constants.DELIMETER_LIST));// Other_Micro_Focus_Products_Used
		for (final String delimitedField : delimitedFieldList)
		{
			sqlStringBuilder = new StringBuilder();
			sqlStringBuilder.append(
					JDBCConstants.SELECT + "[" + VivitData.LABEL_WEB_SITE_MEMBER_ID + "],[" + delimitedField + "] ");
			...
			for (final String record : records)
			{
				...
				for (final String product : products)
				{
					...
					sqlStringBuilder = SQL.appendStringBuilderSQLInsertRecord(VivitTables.DOM_VIVIT_MEMBER_PRODUCTS,
							sqlStringBuilder, mapProduct, true);
				}
			}
			sqlStringBuilderList.add(sqlStringBuilder);
		}
	...
	public static void stubAttendeeUpdateMe() throws Throwable
	{
		...
		String filePathNameSource = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_ATTENDEES.replace(today, otherday)
				+ EVENTID_TEST + IExtension.XML;
		String filePathNameDestination = VivitFoldersFiles.PATH_API_DATA_YM_EVENT_ATTENDEES + EVENTID_TEST
				+ IExtension.XML;
	...
	public void testApiMethod() throws Throwable
	{
		...
		// "");
		apiMap = getYmApi().Sa_EventsNamespace.eventRegistrationGet("A53E0C69-6554-49C5-87A1-EED72921F115", "");
	...
	public void Verification() throws Throwable
	{
		...
		String xml = "";
		final UnmarshallYourMembership_Response unmarshallYourMembership_Response = new UnmarshallYourMembership_Response();
		final YourMembership_Response yourMembership_Response = unmarshallYourMembership_Response.getFromFile(fileName);
