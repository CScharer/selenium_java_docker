...
public class YMAPIMethods
{
	...
	public static final String	VIEW_VIVIT_YMAPI_METHODWITHPARAMETERS	= "v_Vivit_YMAPI_MethodWithParameters";
	// CJSConstants.PATH_FILES_TEMPLATES
	static final String			PATH_TEMPLATES							= Environment.FOLDER_DATA + "Templates"
			+ Constants.DELIMETER_PATH;
	private static JDBC			jdbc									= new JDBC("", "QAAuto");

	private YMAPIMethods()
	{ // Empty
	}
	...
	public static void createClasses() throws Throwable
	{
		try
		{
			...
			final String namespaces = jdbc.queryResults(JDBCConstants.SELECT_ALL
					+ JDBCConstants.FROM + "[" + VIEW_VIVIT_YMAPI_NAMESPACES + "]", "", false);
			...
			for (final String namespace : arrayNamespaces)
			{
				...
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[CLASS_NAMESPACE]", newNamespace + "Namespace");
				...
				newTemplateYMAPIClass = newTemplateYMAPIClass.replaceAll(Constants.NEWLINE + Constants.NEWLINE,
						Constants.NEWLINE);
	...
	private static String getMethods(String namespace) throws QAException
	{
		...
		final String methods = jdbc.queryResults(JDBCConstants.SELECT_ALL + JDBCConstants.FROM
				+ "[" + VIEW_VIVIT_YMAPI_NAMESPACESMETHODS + "] " + JDBCConstants.WHERE + " [Namespace] = '"
				+ namespace + "'", Constants.DELIMETER_LIST, false);
	...
	private static String getMethod(String newTemplateYMAPIMethod, String namespace, String method) throws QAException
	{
		...
		final List<Map<String, String>> listMapYMAPIMethods = jdbc.queryResultsString(sqlStringBuilder.toString(),
				false);
		for (final Map<String, String> mapMethod : listMapYMAPIMethods)
		{
			...
			newTemplateYMAPIMethod = newTemplateYMAPIMethod.replace("[METHOD_DESCRIPTION]", methodDescription);
			...
			sqlStringBuilder.append(JDBCConstants.SELECT_ALL);
			sqlStringBuilder
					.append(JDBCConstants.FROM + "[" + VIEW_VIVIT_YMAPI_METHODWITHPARAMETERS + "] ");
			sqlStringBuilder.append(JDBCConstants.WHERE + "[" + LABEL_NAME + "]='" + callMethod + "'");
			final List<Map<String, String>> listMapMethodParameters = jdbc
					.queryResultsString(sqlStringBuilder.toString(), false);
			...
			for (final Map<String, String> mapMethodParameters : listMapMethodParameters)
			{
				...
				if (parameterPType.equals("Call Arguments"))
				{
					...
					String paramterName = paramterCall.substring(0, 1).toLowerCase() + paramterCall.substring(1);
					paramterName = paramterName.replace(" ", "");
					stringBuilderArguments.append(parameterDeclarationType + " " + paramterName);
					stringBuilderCallParameters.append(Constants.nlTab(1, 2)
							+ "stringBuilder.append(Constants.nlTab(1, 2) + " + Constants.QUOTE_DOUBLE + "<"
							+ paramterCall + ">" + Constants.QUOTE_DOUBLE + " + " + paramterName + " + "
							+ Constants.QUOTE_DOUBLE + "</" + paramterCall + ">" + Constants.QUOTE_DOUBLE + ");");
				}
			}
			newTemplateYMAPIMethod = newTemplateYMAPIMethod.replace("[METHOD_PARAMETERS]",
					stringBuilderArguments.toString());
			newTemplateYMAPIMethod = newTemplateYMAPIMethod.replace("[METHOD_CALL_PARAMETERS]",
					stringBuilderCallParameters.toString());
		}
	...
	public static void createClasses2() throws QAException
	{
		...
		try
		{
			...
			for (final Map<String, String> mapMethod : listMapYMAPIMethods)
			{
				...
				final List<String> listMethod = Arrays.asList(callMethod.split(Constants.DELIMETER_PATH + "."));
				...
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[METHOD_NAMESPACE]", methodNameSpace);
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[METHOD_NAME]", methodName);
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[METHOD_DESCRIPTION]", methodDescription);
				...
				for (final Map<String, String> mapMethodParameters : listMapMethodParameters)
				{
					...
					if (parameterPType.equals("Call Arguments"))
					{
						...
						final String paramterName = paramterCall.substring(0, 1).toLowerCase()
				...
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[METHOD_PARAMETERS]",
						stringBuilderArguments.toString());
				newTemplateYMAPIClass = newTemplateYMAPIClass.replace("[METHOD_CALL_PARAMETERS]",
						stringBuilderCallParameters.toString());
				Environment.sysOut("newTemplateYMAPIClass:[" + newTemplateYMAPIClass + "]");
	...
	public static void createMapping(WebDriver webDriver) throws QAException
	{
		...
		for (final Map<String, String> mapMethodLinks : listMapMethodLinks)
		{
			for (Entry entry : mapMethodLinks.entrySet())
			{
				...
				stringBuilder.append("[Namespace],[ID],[URL],[Name],[Description],[ExampleRequest],[ExampleResponse]");
				...
				int parameterID = 0;
				final List<WebElement> webElementRows = webDriver.findElements(By.xpath("html/body/table/tbody/tr"));
							...
							switch (field)
							{
								...
								case 5:
									...
									stringBuilder.append(JDBCConstants.INSERT_INTO
											+ "[t_DOM_Vivit_YMAPIMethodParameters] (");
									...
									stringBuilder.append("'" + mapValues.get("MaxLength").replaceAll("'", "''") + "',");
									stringBuilder.append("'" + mapValues.get("Required").replaceAll("'", "''") + "',");
									stringBuilder.append(
	...
	private static String getNamespace(String methodName, List<String> listNamespaces) throws QAException
	{
	...
	private static Map<String, String> getMapValues(String methodName, Map<String, String> mapValues, String pageText)
			throws QAException
	{
		...
		try
		{
			String description = pageText.substring((pageText.indexOf(methodName) + methodName.length()),
					pageText.indexOf("Parameters"));
		...
		try
		{
			String exampleRequest = pageText.substring((pageText.indexOf("Example XML Request")),
					pageText.indexOf("Example XML Response"));
		...
		try
		{
			String exampleResponse = pageText.substring((pageText.indexOf("Example XML Response")),
					pageText.indexOf("See Also"));
