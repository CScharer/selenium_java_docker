...
public class DBConnections
{
	private static DBParameters Access()
	{
		...
		dbParameters.setServer("Driver={Microsoft Access Driver (*" + IExtension.MDB + ", *" + IExtension.ACCDB + ")}");
		dbParameters.setName("C:" + Constants.DELIMETER_PATH + "Temp" + Constants.DELIMETER_PATH
				+ "Vivit-Operating_Budget" + IExtension.ACCDB);
		dbParameters.setMemory("false");
		dbParameters.setConnectionString(dbParameters.getUrlPrefix() + dbParameters.getName() + Constants.DELIMETER_LIST
				+ "memory=" + dbParameters.getMemory());
		return dbParameters;
	}
	...
	private static DBParameters MSExcel()
	{
		final DBParameters dbParameters = new DBParameters("UCANACCESS");
		dbParameters
				.setServer("Driver={Driver do Microsoft Excel (*" + IExtension.XLS + ", *" + IExtension.XLSX + ")}");
		dbParameters.setServer("Driver={Driver do Microsoft Excel (*" + IExtension.XLS + ", *" + IExtension.XLSX
				+ ", *.xlsm, *.xlsb)}");
		dbParameters.setServer("Driver={Microsoft Excel Driver (*" + IExtension.XLS + ")}");
		dbParameters.setServer("Driver={Microsoft Excel Driver (*" + IExtension.XLS + ", *" + IExtension.XLSX + ", *"
				+ IExtension.XLSM + ", *" + IExtension.XLSB + ")}");
	...
	private static DBParameters SQLServer()
	{
		...
		dbParameters.setConnectionString(dbParameters.getUrlPrefix() + CJSConstants.JIRA_TEST_DATABASE_SERVER + ":"
				+ CJSConstants.JIRA_TEST_DATABASE_PORT + ";DatabaseName=" + CJSConstants.JIRA_TEST_DATABASE_NAME
				+ ";user=" + CJSConstants.QA_SERVICE_ACCOUNT_NAME + ";password="
	...
	public static DBParameters getDBParametersAutoCoderExcel(String environment)
	{
		switch (environment.toUpperCase())
		{
			default:
				final DBParameters parameters = SQLite();
				parameters.setName(Constants.PATH_FILES_DATA_DATABASES + "AutoCoderExcel" + IExtension.SQLITE);
				parameters.setConnectionString(parameters.getUrlPrefix() + parameters.getName());
				return parameters;
		}
	}
