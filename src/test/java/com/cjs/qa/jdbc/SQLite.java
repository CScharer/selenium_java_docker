...
public class SQLite
{
	...
	public static void createProductSummaryTableAndViews(String tableNameControl, String tableNameTest) throws Throwable
	{
		final List<String> fields = Arrays.asList("Product", "UnitState", "SubUnit", "Total", "Coverage", "SubCoverage",
				"Premium");
		final String dateTimeStamp = DateHelpers.getCurrentDateAndTime();
		Environment.sysOut("dateTimeStamp:[" + dateTimeStamp + "]");
		String fileName = DateHelpers.convertDate(dateTimeStamp, "MM/dd/yyyy HH:mm:ss.SSS", "yyyyMMdd_HHmmss.SSS");
		...
		stringBuilder.append(");");
		SQL.createTableView(connection, stringBuilder.toString().replace("<TABLE_NAME>", tableNameControl),
				tableNameControl);
		SQL.createTableView(connection, stringBuilder.toString().replace("<TABLE_NAME>", tableNameTest), tableNameTest);
		final String viewNameAdded = "V_ADDED";
		final String viewNameChanged = "V_CHANGED";
		final String viewNameDeleted = "V_DELETE";
		stringBuilder = new StringBuilder();
		stringBuilder.append(JDBCConstants.CREATE_VIEW + Constants.QUOTE_DOUBLE + "<VIEW_NAME>" + Constants.QUOTE_DOUBLE
				+ " " + JDBCConstants.AS + JDBCConstants.SELECT + "tl.* ");
		...
		String sql = stringBuilder.toString().replace("<VIEW_NAME>", viewNameDeleted)
				.replace("<TABLE_LEFT>", tableNameControl).replace("<TABLE_RIGHT>", tableNameTest)
				.replace("<PREMIUM_VALUE>", "=''");
		SQL.createTableView(connection, sql, viewNameDeleted);
		sql = stringBuilder.toString().replace("<VIEW_NAME>", viewNameAdded).replace("<TABLE_LEFT>", tableNameTest)
				.replace("<TABLE_RIGHT>", tableNameControl).replace("<PREMIUM_VALUE>", "=''");
		SQL.createTableView(connection, sql, viewNameAdded);
		sql = stringBuilder.toString().replace("<VIEW_NAME>", viewNameChanged).replace("<TABLE_LEFT>", tableNameControl)
				.replace("<TABLE_RIGHT>", tableNameTest).replace("<PREMIUM_VALUE>", "!=tl.[Premium]");
