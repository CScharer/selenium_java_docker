...
{
	public static final String	URL_MICROSOFT_API_BASE	= "https://login.microsoftonline.com/extSTS.srf";
	public static final String	URL_MICROSOFT			= URL_MICROSOFT_API_BASE;
	public static final String	URL_SHAREPOINT_API_BASE	= "https://login.microsoftonline.com/extSTS.srf";
	public static final String	URL_SHAREPOINT			= URL_SHAREPOINT_API_BASE;
	public static final String	URL_SIGNIN				= "https://vivitworldwide.sharepoint.com/_forms/default.aspx?wa=wsignin1.0";
	...
	public SharepointService()
	{
		try
		{
			final HttpURLConnection httpUrlConnection = (HttpURLConnection) new URL(URL_SHAREPOINT).openConnection();
			httpUrlConnection.setRequestMethod("HEAD");
			final int responseCode = httpUrlConnection.getResponseCode();
			if (!((responseCode >= HttpURLConnection.HTTP_OK) && (responseCode < HttpURLConnection.HTTP_BAD_REQUEST)))
			{
				Environment.sysOut(CONNECTED_TO + URL_SHAREPOINT + "] unsuccessful with response of [" + responseCode
			...
			Assert.assertTrue(
					CONNECTED_TO + URL_SHAREPOINT + "] unsuccessful with response of [" + responseCode + ":"
							+ IHTTP.getResponseValue(responseCode) + "].",
					((responseCode >= HttpURLConnection.HTTP_OK)
							&& (responseCode < HttpURLConnection.HTTP_BAD_REQUEST)));
			Environment.sysOut(CONNECTED_TO + URL_SHAREPOINT + "] successfull with response of [" + responseCode + ":"
					+ IHTTP.getResponseValue(responseCode) + "].");
	...
	{
		String microsoftLoginRequest = getMicrosoftLoginRequest(VivitData.getEmailFrom(), VivitData.getEmailPassword());
		Environment.sysOut("microsoftLoginRequest:" + microsoftLoginRequest);
		Map<String, String> resultsMap = SharepointService.getAPIXMLResponse("POST", microsoftLoginRequest,
				URL_MICROSOFT);
	...
	public static Map<String, String> getAPIXMLResponse(String requestMethod, String apiRequest, String url)
			throws QAException
	{
		...
		{
			VivitEnvironment.sysOut(JavaHelpers.getCurrentMethodName() + " Parameters:" + Constants.nlTab(1, 1)
					+ "requestMethod:[" + requestMethod + "]" + Constants.nlTab(1, 1) + "url:[" + url + "]"
					+ Constants.nlTab(1, 1) + "request[" + request + "]");
		}
		int responseCode = -1;
		...
		try
		{
			...
			httpURLConnection.setRequestProperty("Content-Length", "" + Integer.toString(request.getBytes().length));
	...
	private String getMicrosoftLoginRequest(String userName, String password) throws Throwable
	{
		...
		stringBuilder.append(nlTab
				+ "xmlns:u=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\">");
		stringBuilder.append(nlTab + "<s:Header>");
		stringBuilder.append(nlTab + "<a:Action");
		stringBuilder.append(
				nlTab + "s:mustUnderstand=\"1\">http://schemas.xmlsoap.org/ws/2005/02/trust/RST/Issue</a:Action>");
		stringBuilder.append(nlTab + "<a:ReplyTo>");
		stringBuilder.append(nlTab + "<a:Address>http://www.w3.org/2005/08/addressing/anonymous</a:Address>");
		stringBuilder.append(nlTab + "</a:ReplyTo>");
		...
		stringBuilder
				.append(nlTab + "<a:Address>https://vivitworldwide.sharepoint.com/SitePages/Home.aspx</a:Address>");
		stringBuilder.append(nlTab + "</a:EndpointReference>");
		stringBuilder.append(nlTab + "</wsp:AppliesTo>");
		stringBuilder
				.append(nlTab + "<t:KeyType>http://schemas.xmlsoap.org/ws/2005/05/identity/NoProofKey</t:KeyType>");
		stringBuilder
				.append(nlTab + "<t:RequestType>http://schemas.xmlsoap.org/ws/2005/02/trust/Issue</t:RequestType>");
		stringBuilder.append(nlTab + "<t:TokenType>urn:oasis:names:tc:SAML:1.0:assertion</t:TokenType>");
		stringBuilder.append(nlTab + "</t:RequestSecurityToken>");
		stringBuilder.append(nlTab + "</s:Body>");
