...
{
	public static final List<String>	reportFieldList	= Arrays.asList("Method", "xPath", "Index", "Collectable",
			"Collected", "Points Top", "Group", "Description", "Points Bottom");
	...
	public static String toReport()
	{
		...
		for (final PointsCard pointsCard : getPointsCardList())
		{
			...
			stringBuilder.append(" style=" + Constants.QUOTE_DOUBLE + "background-color:" + backgroundColor + Constants.QUOTE_DOUBLE);
			...
			for (final String field : getReportFieldList())
			{
				...
				switch (field)
				{
					...
					case "Points Bottom":
						...
						break;
					case "Points Top":
						value = MicrosoftReport.getFieldHTML(JavaHelpers.formatNumber(
								Double.valueOf(pointsCard.getPointsTop()), MicrosoftReport.getFORMAT_NUMBER()));
						break;
					case "xPath":
						value = MicrosoftReport.getFieldHTML(pointsCard.getxPath().replaceAll("By.xpath: ", ""));
						break;
					default:
						break;
				}
