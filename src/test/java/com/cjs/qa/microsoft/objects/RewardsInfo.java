...
{
	public static final List<String>	reportFieldList	= Arrays.asList("Method", "xPath", "Field", "Value");
			...
			for (final String field : getReportFieldList())
			{
				...
				switch (field)
				{
					case "Method":
						...
						break;
					case "xPath":
						value = MicrosoftReport.getFieldHTML(rewardsInfo.getxPath().replaceAll("By.xpath: ", ""));
						break;
					case "Field":
					...
					case "Value":
						...
						{
							value = JavaHelpers.formatNumber(Double.valueOf(rewardsInfo.getValue()),
									MicrosoftReport.getFORMAT_NUMBER());
						}
						if (fieldName.contains("Needed") && Double.valueOf(rewardsInfo.getValue()) > 0)
						{
							value = "<div style=" + Constants.QUOTE_DOUBLE + "background-color: "
									+ ColorsHEX.LIGHTYELLOW.getValue() + ";" + Constants.QUOTE_DOUBLE + ">"
									+ JavaHelpers.formatNumber(Double.valueOf(rewardsInfo.getValue()),
											MicrosoftReport.getFORMAT_NUMBER());
						}
