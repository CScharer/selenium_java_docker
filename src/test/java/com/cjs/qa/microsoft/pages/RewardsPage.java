...
public class RewardsPage extends Page
{
	...
	public static final String			URL_POINTS_BREAKDOWN		= "https://account.microsoft" + IExtension.COM
			+ "/rewards/pointsbreakdown";
	public static final String			FILE_REPORT					= MicrosoftEnvironment.FOLDER_DATA
			+ "MicrosoftRewardsReport" + IExtension.HTML;
	public static final String			FILE_SIGNATURE				= Constants.PATH_OUTLOOK_SIGNATURES + "MSN"
			+ IExtension.HTML;
	public static final String			POINT_GROUPS				= "Microsoft Edge bonus;PC search;Mobile search;Shop & Earn;Other activities";
	public static final List<String>	POINT_GROUPS_LIST			= Arrays
			.asList(POINT_GROUPS.split(Constants.DELIMETER_LIST));
	public static final String			SEARCHES_NEEDED_PC			= "Microsoft Edge bonus;PC search";
	public static final List<String>	SEARCHES_NEEDED_PC_LIST		= Arrays
			.asList(SEARCHES_NEEDED_PC.split(Constants.DELIMETER_LIST));
	...
	public final By						byPointsDailySet			= By
			.xpath(".//*[@id='daily-sets']/mee-card-group[1]/div/mee-card");
	public final By						byPointsMoreActivities		= By
			.xpath(".//*[@id='more-activities']/div/mee-card");
	public final By						byPointsAvailableSneakPeek	= By
			.xpath(".//*[@id='daily-sets']/mee-card-group[2]/div/mee-card");
	...
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		// getWebDriver().get(URL_POINTS_BREAKDOWN);
		// refresh();
		final By byPointsBreakdown = By.xpath(XPAPTH_USER_BANNER + "//span/ng-transclude[.='Points breakdown']/..");
	...
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		String htmlMicrosoftReport = XML.HEADING_INFO + "<xsl:stylesheet version=" + Constants.QUOTE_DOUBLE + "1.0"
				+ Constants.QUOTE_DOUBLE + "><html>" + getMicrosoftReport().printReports() + "</html>";
		htmlMicrosoftReport = htmlMicrosoftReport.replace("</html>", "<h4>Report Genereated at "
				+ DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_US_STANDARD_DATE_TIME) + "</h4></html>");
	...
	private void getDashboard10PointsMonthRewardLevel(String group) throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Points Required (Message)", pointsEarnedWebElement.getText()));
		...
		final List<WebElement> webElementChildren = pointsEarnedWebElement.findElements(By.xpath(".//b"));
		final String[] pointsArray = webElementChildren.get(0).getText().split(" ");
		xPath += "//b";
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Points Required (Month)", removeCommas(pointsArray[0])));
	}
	...
	private void getDashboard20PointsAvailable(String group) throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList().add(
				new RewardsInfo(group, xPath, "Available Points", removeCommas(availablePointsWebElement.getText())));
		final WebElement availablePointsLifetimeWebElement = webElementList.get(1);
		RewardsInfo.getRewardsInfoList().add(new RewardsInfo(group, xPath, "Lifetime Points (Message)",
				availablePointsLifetimeWebElement.getText()));
		RewardsInfo.getRewardsInfoList().add(new RewardsInfo(group, xPath, "Lifetime Points",
				removeCommas(availablePointsLifetimeWebElement.getText().replace(" lifetime points", ""))));
	}
	...
	private void getDashboard30PointsStreak(String group) throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Streak Days", removeCommas(streakDaysWebElement.getText())));
		...
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Streak Bonus (Message)", streakBonusWebElement.getText()));
	}
	...
	private void getDashboard40PointsEarned(String group) throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Today (Points Available)", removeCommas(pointsArray[1])));
	}
	...
	private void getDashboard50PointsCurrentStreak(String group) throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList()
				.add(new RewardsInfo(group, xPath, "Current Streak (Message)", webElementCurrentStreak.getText()));
	}
	...
	private void getPointBreakdownInfo10() throws QAException
	{
		...
		final String xPathMessage = ISelenium.getWebElementXPath(Arrays.asList(1), webElementMessage, "xpath");
	...
	private void getPointsCardsAvailable10DailySet() throws QAException
	{
		...
		for (int cardIndex = 0; cardIndex < cardList.size(); cardIndex++)
		{
			...
			final PointsCard pointsCard = getPointsCardsAvailableCard(method, parentWebElement, cardIndex, true);
	...
	private void getPointsCardsAvailable30SneakPeek() throws QAException
	{
		...
		final By bySneakPeekAtTomorrowsSet = By.xpath(
				".//span[.=" + Constants.QUOTE_DOUBLE + "SNEAK PEEK AT TOMORROW'S SET" + Constants.QUOTE_DOUBLE + "]");
		final WebElement webElementSneakPeekAtTomorrowsSet = getWebDriver().findElement(bySneakPeekAtTomorrowsSet);
		...
		{
			final WebElement parentWebElement = cardList.get(cardIndex);
			PointsCard.getPointsCardList().add(getPointsCardsAvailableCard(method, parentWebElement, cardIndex, false));
		}
		final By byGoBackToTodaysSet = By
				.xpath(".//span[.=" + Constants.QUOTE_DOUBLE + "GO BACK TO TODAY'S SET" + Constants.QUOTE_DOUBLE + "]");
	 ...
	 * @throws QAException
	 */
	private void getPointsCardsAvailable40Uneared(List<PointsCard> pointsCardList) throws QAException
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		for (int pointsCardIndex = 0; pointsCardIndex < pointsCardList.size(); pointsCardIndex++)
		{
			...
			// if (pointsCard.isCollected())
			{
				...
				{ // Ignore the points message for now, but code to collect the
					// message maybe later.
					case -1:
						break;
					case 5:
	 ...
	 * @throws QAException
	 */
	private PointsCard getPointsCardsAvailableCard(String method, WebElement parentWebElement, int cardIndex,
			boolean collectable) throws QAException
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
	...
	private void getPointsCardsCollected05(PointsCard pointsCard) throws QAException
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		final String method = JavaHelpers.getCurrentMethodName();
		final String message = pointsCard.getMethod() + Constants.NEWLINE + method + ":" + pointsCard.toMessage();
	...
	private boolean isSearchesNeeded() throws QAException
	{
		...
		RewardsInfo.getRewardsInfoList().add(new RewardsInfo(method, method + " (pointsAvailable)", "Points Available",
				String.valueOf(pointsAvailable)));
		RewardsInfo.getRewardsInfoList().add(
				new RewardsInfo(method, method + " (pointsEarned)", "Points Earned", String.valueOf(pointsEarned)));
		RewardsInfo.getRewardsInfoList().add(
				new RewardsInfo(method, method + " (pointsNeeded)", "Points Needed", String.valueOf(pointsNeeded)));
		RewardsInfo.getRewardsInfoList().add(new RewardsInfo(method, method + " (searchesNeeded)", "Searches Needed",
