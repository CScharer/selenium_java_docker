...
public class DailyPollQuizPages extends RewardsPage
{
	...
	private final By		byPointsAvailableLabel		= By
			.xpath(".//*[@id='quizWelcomeContainer']//span[@class='rqWcCredits']");
	private final By		byPointsEarnedLabel			= By
			.xpath(".//*[@id='btoHeadPanel']//span[@class='rqECredits']");
	private final By		byWelcomeLabel				= By
			.xpath(".//*[@id='quizWelcomeContainer']/div[@class='rqText']");
	...
	public void completeQuizByOrder() throws QAException
	{
		...
		for (int answerIndex = 0; answerIndex < (answersList.size() - 1); answerIndex++)
		{
			...
			{
				final WebElement moveFromWebElement = scrollToElementAndHighlightBriefly(
						By.xpath(".//div[@class='rq_button']//*[@type='button'][@data-optionnumber='"
								+ answerActualIndex + "']"));
				final WebElement moveToWebElement = scrollToElementAndHighlightBriefly(
						By.xpath(".//div[@class='rq_button']//*[@type='button'][@data-optionnumber='"
								+ answerExpectedIndex + "']"));
	...
	public void completeQuizBySelection() throws QAException
	{
		...
		{
			final String xPath = ".//div[@class='rq_button']//*[@type='button'][@data-option='"
	...
	private void getAnswers(boolean ordering) throws QAException
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		final By byXPath = By
				.xpath(".//div[@id='currentQuestionContainer']//div[@class='rq_button']//*[@type='button']");
		...
		for (int index = 0; index < webElements.size(); index++)
		{
			...
			{ // Update correct state.
				if ((Integer.valueOf(answers.getDataID()) - 1) == Integer.valueOf(answers.getDataOptionNumber()))
				{
					answers.setCorrect(true);
				}
			}
	...
	private String getQuizWelcomeValue() throws QAException
	{
		...
		if (objectExists(byPointsAvailableLabel, 5))
		{
			...
			Environment.sysOut(
					"Points Available: [" + getPointsAvailable() + "], Answers Needed: [" + getAnswersNeeded() + "]");
	...
	public void getTestYourSmartsQuizPoints(PointsCard pointsCard) throws QAException
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		// (1 of x)
		WebElement webElement = scrollToElementAndHighlightBriefly(By.xpath(".//div[contains(@class,'b_footnote')]"));
		...
		{
			webElement = scrollToElementAndHighlightBriefly(By.xpath(".//*[contains(@id,'wk_question_text')]"));
			final String questionValue = webElement.getText();
			Environment.sysOut("question:[" + questionValue + "]");
			// Options
			final List<WebElement> webElementList = getWebDriver().findElements(
					By.xpath(".//*[@id='QuestionPane" + questionNumber + "']//div[@class='wk_paddingBtm']"));
			for (int webElementListIndex = 0; webElementListIndex < webElementList.size(); webElementListIndex++)
			{
				...
				final WebElement webElementAnswer = webElement.findElement(
						By.xpath("//div[@id='ChoiceText_" + questionNumber + "_" + answerNumber + "']/div"));
				final String answerValue = webElementAnswer.getText();
				Environment.sysOut("answerValue:[" + answerValue + "]");
			}
			final int randomSelection = Integer
					.valueOf(JavaHelpers.generateRandomInteger(0, (webElementList.size() - 1)));
			webElement = scrollToElementAndHighlightBriefly(By.xpath("//div[@id='ChoiceText_" + questionNumber + "_"
					+ randomSelection + "']/../..//span[@class='wk_Circle']"));
			clickObject(webElement);
			JavaHelpers.sleep(3);
			if (questionNumber == answers)
			{
				// Next question
				webElement = scrollToElementAndHighlightBriefly(By.xpath(".//*[@id='check'][@value='Get your score']"));
				clickObject(webElement);
				JavaHelpers.sleep(1);
			} else
			{
				// Next question
				webElement = scrollToElementAndHighlightBriefly(By.xpath(".//*[@id='check'][@value='Next question']"));
				clickObject(webElement);
				JavaHelpers.sleep(1);
			}
		}
	...
	{
		Environment.sysOut("***ClassMethodDebug***:[" + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		final WebElement webElement = scrollToElementAndHighlightBriefly(
		...
		Environment.sysOut("Points Available: [" + getPointsAvailable() + "], Points Earned:[" + pointEarned
				+ "], Answers Needed: [" + answersNeeded + "]");
