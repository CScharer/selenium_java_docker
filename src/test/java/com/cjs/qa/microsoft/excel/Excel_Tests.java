...
public class Excel_Tests
{
	...
	private static void testExcelType(XLSX excel) throws QAException, IOException
	{
		...
		Environment
				.sysOut("Sheet Exists(" + IExcel.SHEET_DEFAULT + "):[" + excel.sheetExists(IExcel.SHEET_DEFAULT) + "]");
		Environment.sysOut("Sheet Exists(1):[" + excel.sheetExists(1) + "]");
		Environment
				.sysOut("Sheet Exists(" + Excel_Tests.SHEET_MISSING + "):[" + excel.sheetExists(SHEET_MISSING) + "]");
		Environment.sysOut("Sheet Exists(5):[" + excel.sheetExists(5) + "]");
		//
		Environment.sysOut("Row Exists(" + Constants.QUOTE_DOUBLE + IExcel.SHEET_DEFAULT + Constants.QUOTE_DOUBLE
				+ ", 0):[" + excel.rowExists(IExcel.SHEET_DEFAULT, 0) + "]");
		Environment.sysOut("Row Exists(0, 0):[" + excel.rowExists(0, 0) + "]");
		Environment.sysOut("Row Exists(" + Constants.QUOTE_DOUBLE + IExcel.SHEET_DEFAULT + Constants.QUOTE_DOUBLE
				+ ", 1):[" + excel.rowExists(IExcel.SHEET_DEFAULT, 1) + "]");
		Environment.sysOut("Row Exists(0, 1):[" + excel.rowExists(0, 1) + "]");
		//
		Environment.sysOut("Column Exists(" + Constants.QUOTE_DOUBLE + IExcel.SHEET_DEFAULT + Constants.QUOTE_DOUBLE
				+ ", " + Constants.QUOTE_DOUBLE + "New Sheet 1" + Constants.QUOTE_DOUBLE + ", 0):["
				+ excel.columnExists(IExcel.SHEET_DEFAULT, "New Sheet 1", 0) + "]");
		Environment.sysOut("Column Exists(" + Constants.QUOTE_DOUBLE + IExcel.SHEET_DEFAULT + Constants.QUOTE_DOUBLE
				+ ", 0, 0):[" + excel.columnExists(IExcel.SHEET_DEFAULT, 0, 0) + "]");
		Environment.sysOut("Column Exists(0, " + Constants.QUOTE_DOUBLE + "Testing" + Constants.QUOTE_DOUBLE + ", 0):["
				+ excel.columnExists(0, "Testing", 0) + "]");
		Environment.sysOut("Column Exists(0, 0, 0):[" + excel.columnExists(0, 0, 0) + "]");
		//
		Environment.sysOut("Column Exists(" + SHEET_MISSING + ", " + Constants.QUOTE_DOUBLE + "New Sheet 1"
				+ Constants.QUOTE_DOUBLE + ", 0):[" + excel.columnExists(SHEET_MISSING, "New Sheet 1", 0) + "]");
		Environment.sysOut("Column Exists(" + Constants.QUOTE_DOUBLE + IExcel.SHEET_DEFAULT + Constants.QUOTE_DOUBLE
				+ ", 0, 0):[" + excel.columnExists(IExcel.SHEET_DEFAULT, 1, 0) + "]");
		Environment.sysOut("Column Exists(0, " + Constants.QUOTE_DOUBLE + "New Sheet 1" + Constants.QUOTE_DOUBLE
				+ ", 0):[" + excel.columnExists(0, "New Sheet 1", 1) + "]");
		...
		Environment.sysOut("renameSheet(0, " + Constants.QUOTE_DOUBLE + "NewSheet" + Constants.QUOTE_DOUBLE + ")"
				+ excel.renameSheet(0, "NewSheet"));
		Environment.sysOut("renameSheet(" + Constants.QUOTE_DOUBLE + "NewSheet" + Constants.QUOTE_DOUBLE + ", "
				+ IExcel.SHEET_DEFAULT + ")" + excel.renameSheet("NewSheet", IExcel.SHEET_DEFAULT));
		// Environment.sysOut("renameSheet(9, " + Constants.QUOTE_DOUBLE +
		// "NewSheet" +
		// Constants.QUOTE_DOUBLE + ")" +
		// excel.renameSheet(9, "NewSheet"));
		Environment.sysOut("renameSheet(1, " + Constants.QUOTE_DOUBLE + "NoNewSheet" + Constants.QUOTE_DOUBLE + ")"
				+ excel.renameSheet(1, "NoNewSheet"));
		Environment.sysOut("renameSheet(" + Constants.QUOTE_DOUBLE + "NoNewSheet" + Constants.QUOTE_DOUBLE + ", "
				+ Constants.QUOTE_DOUBLE + "NoSheet" + Constants.QUOTE_DOUBLE + ")"
				+ excel.renameSheet("NoNewSheet", "NoSheet"));
		Environment.sysOut("createSheet(" + Constants.QUOTE_DOUBLE + "NoSheet" + Constants.QUOTE_DOUBLE + ")"
