...
public class GroupPage extends Page
{
	...
	public void getGroupPageData() throws Throwable
	{
		...
		{
			Environment
					.sysOut("Retrieving Data:[" + JavaHelpers.getCurrentClassMethodName() + "], [" + groupType + "]");
	...
	private void getGroupData(String group) throws Throwable
	{
		...
		try
		{
			...
			for (int indexChapterURL = 0; indexChapterURL < chapterURLList.size(); indexChapterURL++)
			{
				...
				Environment.sysOut("ElapsedTime:[" + elapsedTime + "], ElapsedTime:["
						+ stopwatch.toString().replaceAll(" ", "") + "]");
				stopwatch.reset();
				final List<WebElement> wallPostElements = webDriver.findElements(By.xpath("//div[@class='WallPost']"));
				...
				{// Broken Group URL.
					...
					// groupType = groupType;
					// pageID = pageID;
					groupName = groupType + ": " + pageID.substring((pageID.lastIndexOf("/") + 1), pageID.length());
					elapsedTime = "N/A";
					wallPosts = "N/A";
				...
				for (WebElement webElement : webElements)
				{
					...
					{
						webSiteMemberID = webSiteMemberID.substring(
								(webSiteMemberID.indexOf(SYMBOL_ID) + SYMBOL_ID.length()), webSiteMemberID.length());
					}
					Environment.sysOut("Leader Type:[" + leaderType + "], Leader:[" + leaderName + "], Href:[" + href
	...
	private Map<String, By> getGroupURLs()
	{
		...
		mapByGroupURLsBy.put("GroupDirectoryURL", By.xpath(ROOT_NODE_PRE + "Group Directory" + ROOT_NODE_POST));
		...
		mapByGroupURLsBy.put("PhotoGalleryURL", By.xpath(ROOT_NODE_PRE + "Photo Gallery" + ROOT_NODE_POST));
