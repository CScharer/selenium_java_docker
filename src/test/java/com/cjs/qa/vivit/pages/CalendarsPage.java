...
public class CalendarsPage extends Page
{
		...
		for (final Groups groups : Groups.getGroupsList())
		{
			if (JavaHelpers.hasValue(groups.getCalendarURL()))
			{
				Environment.sysOut("Retrieving Data:[" + JavaHelpers.getCurrentClassMethodName() + "], ["
						+ groups.getGroupName() + "], [" + groups.getCalendarURL() + "]");
	...
	{
		webDriver.get(url);
		final List<WebElement> listCalendarRecords = webDriver.findElements(byTableCalendars);
		for (int record = 0; record < listCalendarRecords.size(); record++)
		{
			...
			Calendars.getCalendarsList()
					.add(new Calendars(calendarID, calendarPage, eventName, eventDate, eventDetails, calendarID));
		}
	}
}