...
public class VivitDataFactory
{
	...
	public DataTable getBio(String columnName, String columnValue)
	{
		...
		sqlStringBuilder.append(JDBCConstants.SELECT_ALL_FROM + "[" + VivitTables.DOM_VIVIT_MEMBERS + "] ");
		switch (columnName)
		{
			...
			case "Email_Address_Alternate":
				sqlStringBuilder
						.append(JDBCConstants.WHERE + "LOWER([" + columnName + "])=LOWER('" + columnValue + "');");
				break;
			default:
				sqlStringBuilder
						.append(JDBCConstants.WHERE + "[" + VivitData.LABEL_WEB_SITE_MEMBER_ID + "]='10969660';");
				break;
		}
		...
		final DataTable dataTable = Convert.fromListToDataTable(Arrays.asList(
				...
				Arrays.asList("Alternate IM Username",
						CJSConstants.EMAIL_ADDRESS_MSN.substring(0, CJSConstants.EMAIL_ADDRESS_MSN.indexOf('@'))),
				...
				Arrays.asList("Employer URL", "https://www.linkedin" + IExtension.COM + "/in/cscharer"),
				...
				Arrays.asList("Used HP Years", "// " + mapData.get("Years_Micro_Focus_Products_Used"))));
		jdbc.close();
