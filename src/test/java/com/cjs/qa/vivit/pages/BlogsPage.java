...
public class BlogsPage extends Page
{
		...
		for (final Groups groups : Groups.getGroupsList())
		{
			if (JavaHelpers.hasValue(groups.getBlogsURL()))
			{
				Environment.sysOut("Retrieving Data:[" + JavaHelpers.getCurrentClassMethodName() + "], ["
						+ groups.getGroupName() + "], [" + groups.getBlogsURL() + "]");
	...
	private void getBlogs(String url, String blogPage)
	{
		...
		{
			Environment.sysOut("blogPage:[" + blogPage + "], [" + url + "] DOES NOT EQUAL [" + urlActual + "]");
		}
		...
		for (int record = 0; record < listBlogRecords.size(); record++)
		{
			...
			if (record > 0)
			{
				...
				Blogs.getBlogsList()
						.add(new Blogs(groupName, blogID, name, Integer.valueOf(numberOfPosts), lastActivity, blogURL));
			}
		}
	}
}