...
public class ForumsPage extends Page
{
	...
	final By byTableForums = By.xpath(".//*[@id='SpContent_Container']/table/tbody/tr[@class!='header']");
	...
	public void getForumData() throws Throwable
	{
		...
		for (final Groups groups : Groups.getGroupsList())
		{
			...
			{
				Environment.sysOut("Retrieving Data:[" + JavaHelpers.getCurrentClassMethodName() + "], ["
						+ groups.getGroupName() + "], [" + groups.getForumsURL() + "]");
				getForums(groups.getForumsURL(), groups.getGroupName());
			} else
			{
				Environment.sysOut("Retrieving Null:[" + JavaHelpers.getCurrentClassMethodName() + "], ["
						+ groups.getGroupName() + "], [" + groups.getForumsURL() + "]");
			}
		}
	...
	private void getForums(String url, String forumPage)
	{
		...
		for (int record = 0; record < listForumRecords.size(); record++)
		{
			...
			Forums.getForumsList().add(new Forums(groupName, forumID, name, Integer.valueOf(topics),
					Integer.valueOf(posts), lastActivity, forumURL));
		}
	}
}