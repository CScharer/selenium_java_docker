...
public class VivitData extends Environment
{
	...
	public static final String			EMAIL_ADDRESS_DIRECTORS				= "Directors"
			+ CJSConstants.MAILDOMAIN_VIVIT;
	...
	public static final String			LOGO_SOURCE							= FSO
			.fileReadAll(VivitFoldersFiles.LOGO_SOURCE);
	public static final String			STYLE_BORDER						= "border: 1px solid black; border-collapse: collapse;width: 100%;";
	...
	public static final String			QUERY_REPORT_AUTOMATION_EMAIL		= JDBCConstants.SELECT + "[EMail] "
			+ JDBCConstants.FROM + "[" + VivitViews.VIVIT_AUTOMATION_REPORT + "]";
	public static final String			QUERY_REPORT_BILLABLE_HOURS_EMAIL	= JDBCConstants.SELECT + "[EMail] "
			+ JDBCConstants.FROM + "[" + VivitViews.VIVIT_CONTRACTORS_BILLABLE_HOURS + "]";
	public static final List<String>	DATABASE_ONLY_FIELDS				= Arrays.asList(LABEL_RECORD_COMPLETE,
			LABEL_RECORD_NUMBER);

	@Test
	public void temporaryTest()
	{
		List<String> sqlFileList = FSO.filesList(VivitFoldersFiles.PATH_API_DATA_YM_SQL, IExtension.SQL);
	...
	public static void checkStatus(String classMethodType) throws Throwable
	{
		...
		if (!success)
		{
			...
			throw new QAException(JavaHelpers.getCurrentClassMethodDebugName() + "- The success file ["
					+ successFilePathName + "] was either missing or showed the last run failed!");
		}
	}
	...
	{
		final String fileStandardTable = FOLDER_DATA + "StandardTable" + IExtension.HTML;
		HTML html = new HTML(fileStandardTable, ColorsHEX.VIVIT_GOLD.getValue(), ColorsHEX.VIVIT_LIGHTBLUE.getValue());
		final List<String> viewsList = Arrays
				.asList("v_Vivit_Broken_Links;v_Vivit_GroupLeaders_Current_By_Group;v_Vivit_GroupLeaders_Current_By_Role;v_Vivit_Contractors_Billable_Hours"
		...
		for (int viewIndex = 0; viewIndex < viewsList.size(); viewIndex++)
		{
			...
			String sql = JDBCConstants.SELECT_ALL + JDBCConstants.SELECT_COUNT_FROM + "[" + view + "] "
					+ JDBCConstants.LIMIT + "0";
	...
	{ // Delete backup files older than 3 days old.
		...
		final String date = DateHelpers.getCurrentDatePlusMinusDays(formatDate, (DAYS_TO_KEEP_BACKUPS * -1));
		...
		final String filePathName = file.getAbsolutePath();
			...
			try
			{
				...
				{
					sysOut("Deleting filePathName:[" + filePathName + "], [" + dateFormat.format(fileTime.toMillis()) + "]");
					if (lastModified < deleteDaysAgo)
					{
						FSO.fileDelete(filePathName);
					}
				} else {
					sysOut("Preserved filePathName:[" + filePathName + "], [" + dateFormat.format(fileTime.toMillis()) + "]");
				}
	...
	public static void dropAndCreateBackUpTables(String tableGroupName)
	{
		...
		String datePreviousBackUp = DateHelpers.getCurrentDatePlusMinusDays(DateHelpers.FORMAT_yyyyMMdd,
				(DAYS_TO_KEEP_BACKUPS * -1));
	 ...
	 * @throws Throwable
	 */
	public static String getBillableHoursTable(String header, String footer, List<String> fieldList,
			List<Map<String, String>> reportListMap) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("header", header),
				Arrays.asList("footer", footer), Arrays.asList("fields", fieldList))));
		int tab = 0;
		...
		stringBuilderReport.append(Constants.nlTab(1, tab--) + "<link rel=" + Constants.QUOTE_DOUBLE + "stylesheet"
				+ Constants.QUOTE_DOUBLE + " type=" + Constants.QUOTE_DOUBLE + "text/css" + Constants.QUOTE_DOUBLE
				+ " href=" + Constants.QUOTE_DOUBLE + "TableReport.css" + Constants.QUOTE_DOUBLE + "></link>");
		stringBuilderReport.append(Constants.nlTab(1, tab) + "</head>");
		...
		for (Map<String, String> mapReport : reportListMap)
		{
			if (!firstRecord)
			{
				...
				if (header != null)
				{
					...
					stringBuilderHeader.append(Constants.nlTab(1, tab++) + "<table class=" + Constants.QUOTE_DOUBLE
							+ "scroll" + Constants.QUOTE_DOUBLE + " style=" + Constants.QUOTE_DOUBLE + STYLE_BORDER
							+ Constants.QUOTE_DOUBLE + ">");
					stringBuilderHeader.append(Constants.nlTab(1, tab++) + "<tr>");
					stringBuilderHeader.append(Constants.nlTab(1, tab++) + "<th style=" + Constants.QUOTE_DOUBLE
							+ "border: 1px solid black; border-collapse: collapse; background-color: "
							+ ColorsHEX.VIVIT_GRAY.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
					stringBuilderHeader.append(Constants.nlTab(1, tab++) + "<div style=" + Constants.QUOTE_DOUBLE
							+ "height: 100px" + Constants.QUOTE_DOUBLE + ">");
					stringBuilderHeader.append(Constants.nlTab(1, tab--) + "<img href=" + Constants.QUOTE_DOUBLE
							+ "https://www.vivit-worldwide.org/default.asp?" + Constants.QUOTE_DOUBLE + " src="
							+ Constants.QUOTE_DOUBLE + "https://www.vivit-worldwide.org/graphics/logo.png"
							+ Constants.QUOTE_DOUBLE + " alt=" + Constants.QUOTE_DOUBLE + "Error"
							+ Constants.QUOTE_DOUBLE + " style=" + Constants.QUOTE_DOUBLE
							+ "max-width: 100%; max-height: 100%; display:block; margin:auto;" + Constants.QUOTE_DOUBLE
							+ " />");
					...
					stringBuilderHeader.append(Constants.nlTab(1, tab++) + "<th style=" + Constants.QUOTE_DOUBLE
							+ "border: 1px solid black; border-collapse: collapse; background-color: "
							+ ColorsHEX.VIVIT_GOLD.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
				...
				stringBuilderTable.append(Constants.nlTab(1, tab++) + "<table class=" + Constants.QUOTE_DOUBLE
						+ "scroll" + Constants.QUOTE_DOUBLE + " style=" + Constants.QUOTE_DOUBLE + STYLE_BORDER
						+ Constants.QUOTE_DOUBLE + ">");
				...
				{
					stringBuilderTable.append(Constants.nlTab(1, tab++) + "<th style=" + Constants.QUOTE_DOUBLE
							+ "border: 1px solid black; border-collapse: collapse; background-color: "
							+ ColorsHEX.VIVIT_LIGHTBLUE.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
			...
			for (final String field : fieldList)
			{
				...
				stringBuilderTable.append(Constants.nlTab(1, tab++) + "<td style=" + Constants.QUOTE_DOUBLE
						+ "border: 1px solid black; border-collapse: collapse;" + Constants.QUOTE_DOUBLE + ">");
		...
		if (footer != null)
		{
			...
			stringBuilderFooter.append(Constants.nlTab(1, tab++) + "<table class=" + Constants.QUOTE_DOUBLE + "scroll"
					+ Constants.QUOTE_DOUBLE + " style=" + Constants.QUOTE_DOUBLE + STYLE_BORDER
					+ Constants.QUOTE_DOUBLE + ">");
			stringBuilderFooter.append(Constants.nlTab(1, tab++) + "<tr>");
			stringBuilderFooter.append(Constants.nlTab(1, tab++) + "<td style=" + Constants.QUOTE_DOUBLE
					+ "border: 1px solid black; border-collapse: collapse; background-color: "
					+ ColorsHEX.VIVIT_GOLD.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
			stringBuilderFooter.append(Constants.nlTab(1, tab--) + "<font color=" + Constants.QUOTE_DOUBLE + "red"
					+ Constants.QUOTE_DOUBLE + "><b>Note:</b> </font><i>" + footer + "</i>");
	 ...
	 * @throws Throwable
	 */
	public static String getUpdatedQuery(StringBuilder sb, String tableName, String fieldName) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("sb", sb),
				Arrays.asList(LABEL_TABLE_NAME, tableName), Arrays.asList("fieldName", fieldName))));
		...
		sb.append(
				Constants.nlTab(1, 2) + "(" + JDBCConstants.SELECT_COUNT_FROM + "[" + tableName + "])) AS [Percent] ");
		sb.append(Constants.nlTab(1, 0) + JDBCConstants.FROM + "[" + tableName + "] ");
		sb.append(Constants.nlTab(1, 0) + JDBCConstants.WHERE + "[" + fieldName + "] " + JDBCConstants.NOT_NULL);
		...
		sb.append(Constants.nlTab(1, 0) + "(" + JDBCConstants.SELECT_COUNT_FROM + "[" + tableName + "])) AS [Count],");
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_ATTACHMENT, attachment))));
		...
		Email.sendEmail(getEmailFrom(), getEmailPassword(), getEmailTo(QUERY_REPORT_AUTOMATION_EMAIL), "", "", subject,
				body, attachment);
	}
	 ...
	 * @throws Throwable
	 */
	public static void sendAutomationReport(String embeddedReport, String attachment) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("embeddedReport", embeddedReport),
	...
	public void sendAutomationReportsTest() throws Throwable
	{
		...
		List<String> listReports = Arrays.asList(VivitFoldersFiles.REPORT_XLS_AUTOMATION_A,
				VivitFoldersFiles.REPORT_XLS_AUTOMATION_S);
	 ...
	 * @throws Throwable
	 */
	public static void sendAutomationReports(String embeddedReport, List<String> listReports) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("embeddedReport", embeddedReport),
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(
				Arrays.asList(Arrays.asList("body", body), Arrays.asList(LABEL_ATTACHMENT, attachment))));
		final String date = new Date().toString();
		final String subject = "Vivit - Treasurer (Monthly Report) " + date;
		body += VivitEnvironment.EMAIL_SIGNATURE;
		Email.sendEmail(getEmailFrom(), getEmailPassword(), EMAIL_ADDRESS_DIRECTORS, "", "", subject, body, attachment);
	}
	...
	public static void compactDatabase() throws Throwable
	{
		...
		final String command = "CMD /C " + Constants.QUOTE_DOUBLE + Constants.QUOTE_DOUBLE + Constants.PATH_SQLITE3_FILE
				+ Constants.QUOTE_DOUBLE + " " + Constants.QUOTE_DOUBLE + VivitFoldersFiles.DATABASE
				+ Constants.QUOTE_DOUBLE + " " + Constants.QUOTE_DOUBLE + "" + compactCommand + Constants.QUOTE_DOUBLE
				+ Constants.QUOTE_DOUBLE + "";
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_SEND_EMAIL, sendEmail))));
		...
		String embeddedReport = createReportAutomation(VivitFoldersFiles.REPORT_XLS_AUTOMATION_A, false);
		getHTMLMemberChanges();
		final String htmlMemberChanges = FSO.fileReadAll(VivitFoldersFiles.REPORT_HTM_AUTOMATION_MEMBER_CHANGES);
		...
		getHTMLBrokenLinks();
		final String htmlBrokenLinks = FSO.fileReadAll(VivitFoldersFiles.REPORT_HTM_AUTOMATION_BROKEN_LINKS);
		embeddedReport = embeddedReport.replace("LABEL_REPLACE_BROKEN_LINKS", htmlBrokenLinks);
		getHTMLDatabaseChanges();
		final String htmlDatabaseChanges = FSO.fileReadAll(VivitFoldersFiles.REPORT_HTM_AUTOMATION_DATABASE_CHANGES);
		String embeddedReportTemp = embeddedReport;
		embeddedReportTemp = embeddedReportTemp.replace("LABEL_REPLACE_DATABASE_CHANGES", htmlDatabaseChanges);
		embeddedReportTemp = embeddedReportTemp.replace("LABEL_REPLACE_API_VERSION_YM", YMService.API_VERSION);
		embeddedReportTemp = embeddedReportTemp.replace("LABEL_REPLACE_API_VERSION_GTW", GTWebinarService.API_VERSION);
		//
		String htmlTestInformation = FSO.fileReadAll(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TEST_INFORMATION);
		...
		{
			VivitData.createReportAutomationStatisticsForPeriod(VivitViews.VIVIT_MEMBER_CHANGE_MONTH,
					VivitFoldersFiles.REPORT_XLS_AUTOMATION_S_MONTH);
		...
		{
			VivitData.createReportAutomationStatisticsForPeriod(VivitViews.VIVIT_MEMBER_CHANGE_YEAR,
					VivitFoldersFiles.REPORT_XLS_AUTOMATION_S_YEAR);
	 ...
	 * @throws Throwable
	 */
	public static String createReportAutomation(String filePathName, boolean sendEmail) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		Environment
				.sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_FILE_PATH_NAME, filePathName),
		...
		for (final String groupType : groupTypes)
		{
			for (final String groupSort : groupSorts)
			{
				...
				stringBuilder.append(JDBCConstants.FROM + "[" + VivitViews.VIVIT_GROUPPAGE_TIMINGS_SECONDS + "] ");
		...
		final List<Map<String, String>> reportListMap = jdbc.queryResultsString(stringBuilder.toString(), false);
		...
		for (final Map<String, String> tableMap : reportListMap)
		{
			...
			sysOut("viewName:[" + viewName + "], labelName:[" + labelName + "], sheetLinkName:[" + sheetLinkName + "]");
	 ...
	 * @throws Throwable
	 */
	public static void createReportAutomationStatistics(String filePathName, boolean sendEmail) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		Environment
				.sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_FILE_PATH_NAME, filePathName),
		...
		for (final Map<String, String> tableMap : reportListMap)
		{
			...
			if (dynamicQuery.equals("1"))
			{
				...
				final int recordsUpdated = SQL.executeVivit("Update", VivitTables.VIVIT_QUERIES + "-" + queryName,
						sqlStringBuilder);
				if (recordsUpdated != 1)
				{
			...
			Environment
					.sysOut("sql:[" + sql + "], queryName:[" + queryName + "], sheetLinkName:[" + sheetLinkName + "]");
			final String data = jdbc.queryResults(sql, Constants.DELIMETER_LIST, true);
			if (!data.equals(""))
			{
				final String[] records = data.split(Constants.NEWLINE);
	 ...
	 * @throws Throwable
	 */
	public static void createReportAutomationStatisticsForPeriod(String view, String filePathName) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_FILE_PATH_NAME, filePathName))));
		FSO.fileDelete(filePathName);
		...
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(JDBCConstants.SELECT_ALL);
		stringBuilder.append(JDBCConstants.FROM + "[" + VivitTables.VIVIT_REPORT_FIELDS_STATISTIICAL + "] ");
		stringBuilder.append(JDBCConstants.ORDER_BY + "[Order];");
		final List<Map<String, String>> reportListMap = jdbc.queryResultsString(stringBuilder.toString(), false);
		int newMemberCount = Integer.parseInt(jdbc.queryResults("select count() from [" + view + "];", "", false));
		...
		for (final Map<String, String> tableMap : reportListMap)
		{
			...
			Environment
					.sysOut("sql:[" + sql + "], fieldName:[" + fieldName + "], sheetLinkName:[" + sheetLinkName + "]");
			...
			if (recordListMap.size() > 1)
			{
				...
				// int row = (excel.getRowCount(sheetName))
				int row = 1;
				for (int recordListMapIndex = 0; recordListMapIndex < recordListMap.size(); recordListMapIndex++)
				{
					...
					} else
					{
						...
						if (recordListMapIndex == 1)
						{
							...
							String percent = JavaHelpers
									.formatNumber(Double.valueOf(newMemberCount / Double.valueOf(count)), "0.0000%");
							excel.writeCell(sheetSummary, 3, majorityRow, percent);
							excel.writeCellFormula(sheetSummary, 3, majorityRow, "SUM(C" + (majorityRow + 1) + "/E$1)");
						}
						excel.writeCell(sheetName, column, row, recordMap.get(fieldName));
						column++;
	...
	private static String getQueryForPeriodField(String view, String fieldName)
	{
		...
		sqlStringBuilderSQL.append("(select [" + fieldName + "],count() as [Count] from [" + view + "] where ["
				+ fieldName + "] not null and [" + fieldName + "]!='' group by [" + fieldName + "]");
		sqlStringBuilderSQL.append(Constants.NEWLINE);
		sqlStringBuilderSQL.append("union all");
		sqlStringBuilderSQL.append(Constants.NEWLINE);
		sqlStringBuilderSQL.append("select '<EMPTY>' as [" + fieldName + "],(select count() from [" + view + "] where ["
				+ fieldName + "] is null or [" + fieldName + "]='') as [Count])");
		sqlStringBuilderSQL.append(Constants.NEWLINE);
	 ...
	 * @throws Throwable
	 */
	public static void createReportHTMLBillableHours(boolean sendEmail, boolean overideMonthDay) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList("overideMonthDay", overideMonthDay))));
		...
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + "["
				+ VivitViews.VIVIT_CONTRACTORS_BILLABLE_HOURS + "];";
		final List<Map<String, String>> reportListMap = jdbc.queryResultsString(sql, false);
		final List<String> fieldList = jdbc.getFieldNamesList(VivitViews.VIVIT_CONTRACTORS_BILLABLE_HOURS, true);
		...
		final String header = "Approved Billable Hours for the month of " + billableMonth + " (" + date + ")";
		final String footer = "Any additional hours need approval and requests for approval should be submitted to the President and Treasurer within a week of the end of the month the request is for.";
		...
		final String subject = "Vivit - Billable Hours (for the month of " + billableMonth + ") " + date;
		String body = report;// + Constants.NEWLINE
		body += VivitEnvironment.EMAIL_SIGNATURE;
		final String attachment = "";
		if (sendEmail)
		{
			Email.sendEmail(getEmailFrom(), getEmailPassword(), getEmailTo(QUERY_REPORT_BILLABLE_HOURS_EMAIL), "", "",
	 ...
	 * @throws Throwable
	 */
	public static void createReportHTMLTreasurer(boolean sendEmail, boolean overideMonthDay) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_SEND_EMAIL, sendEmail),
		...
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final List<String> fieldStandardTableList = jdbc.getFieldNamesList(VivitViews.VIVIT_CONTRACTOR_HOURS_BTA, true);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(JDBCConstants.SELECT + "[h].* ");
		stringBuilder.append(JDBCConstants.FROM + "[" + VivitViews.VIVIT_CONTRACTOR_HOURS_BTA_HIGHLEVEL + "] [h]");
		stringBuilder.append(JDBCConstants.INNER_JOIN + "[" + VivitViews.VIVIT_REPORT_TREASURER_YEARS + "] [y] ");
		stringBuilder.append(JDBCConstants.ON + "SUBSTR([h].[Year_Month],1,4)=[y].[Years]");
		List<Map<String, String>> reportListMap = jdbc.queryResultsString(stringBuilder.toString(), false);
		final HTML html = new HTML(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TREASURER, ColorsHEX.VIVIT_GRAY.getValue(),
				ColorsHEX.VIVIT_LIGHTBLUE.getValue());
		StringBuilder stringBuilderReport = new StringBuilder();
		stringBuilderReport.append(html.createStandardTableLogo(VivitEnvironment.URL_LOGIN, LOGO_SOURCE, 0));
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TREASURER, stringBuilderReport.toString(), false);
		stringBuilderReport.append(Constants.NEWLINE
				+ html.createStandardTable(Arrays.asList("Years"), jdbc.queryResultsString(JDBCConstants.SELECT_ALL
						+ JDBCConstants.FROM + "[" + VivitViews.VIVIT_REPORT_TREASURER_YEARS + "]", false)));
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TREASURER, stringBuilderReport.toString(), false);
		final List<String> fieldNumericList = Arrays
				.asList("Budgeted#;Actual#;Difference#".split(Constants.DELIMETER_LIST));
		final List<String> fieldCurrencyList = Arrays
				.asList("Budgeted$;Actual$;Difference$".split(Constants.DELIMETER_LIST));
		stringBuilderReport.append(Constants.NEWLINE
				+ html.createStandardTable(fieldStandardTableList, fieldNumericList, fieldCurrencyList, reportListMap));
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TREASURER, stringBuilderReport.toString(), false);
		// ****************************************************************************************************
		...
		for (final Map<String, String> tableMap : reportListMap)
		{
			...
			final String recordCount = jdbc.queryResults(stringBuilder.toString(), Constants.DELIMETER_LIST, false);
			stringBuilder = new StringBuilder();
		...
		for (final Map<String, String> mapVIEW : reportListMap)
		{
			...
			final String sql = JDBCConstants.SELECT + "[h].* " + JDBCConstants.FROM + "[" + viewName + "]" + " [h] "
					+ JDBCConstants.INNER_JOIN + "[" + VivitViews.VIVIT_REPORT_TREASURER_YEARS + "] [y] "
					+ JDBCConstants.ON + "SUBSTR([h].[Year_Month],1,4)=[y].[Years]";
			final String data = jdbc.queryResults(sql, Constants.DELIMETER_LIST, true);
			sysOut("data:[" + data + "]");
			if (!data.equals(""))
			{
						...
						if (!record.equals(""))
						{
							...
							for (int index = 0; index < values.length; index++)
							{
								...
								if (firstRecord)
								{
									...
									excel.writeCell(sheetName, column, row, value);
									excel.setFormatHeading(sheetName, column, row);
									excel.writeCell(sheetName, (column + 1), row,
		...
		if (sendEmail)
		{
			sendTreasurersReport(
					"Attached you will find the current Contractor Burn Rates for the following years:"
							+ Constants.NEWLINE + stringBuilderReport.toString() + Constants.NEWLINE,
	...
	{
		final List<String> fieldStandardTableList = jdbc
				.getFieldNamesList(VivitViews.VIVIT_CONTRACTOR_HOURS_BTA_VREPORT_TREASURER_YEARS, true);
		StringBuilder stringBuilder = new StringBuilder();
	...
	public static void createDatabaseBackup() throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		final String fileDateStamp = IExtension.SQLITE + "."
				+ DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_DATE_TIME_STAMP) + IExtension.BAK;
		final String databaseBackup = VivitFoldersFiles.DATABASE.replace(IExtension.SQLITE, fileDateStamp);
	...
	public static void formatReportBarCharts(String filePathName) throws Throwable
	{
		...
		String command = "cscript //S //T:" + TIMEOUT + " " + Constants.QUOTE_DOUBLE + VivitFoldersFiles.VBS
				+ Constants.QUOTE_DOUBLE + " " + Constants.QUOTE_DOUBLE + filePathName + Constants.QUOTE_DOUBLE + "";
		sysOut("command:[" + command + "]");
	...
	public static void getGoogleCommutes(Google google) throws Throwable
	{
		...
		List<Map<String, String>> agendaMapList = jdbc.queryResultsString(stringBuilderQuery.toString(), false);
		...
		List<Map<String, String>> flyingListMap = jdbc.queryResultsString(stringBuilderQuery.toString(), false);
		StringBuilder stringBuilderSQL = new StringBuilder();
		for (Map<String, String> flyingMap : flyingListMap)
		{
			String airportFrom = flyingMap.get("Airport");
			List<Flight> flightList = google.FlightsPage.getFlights(airportFrom, airportTo, dateDepartTo,
					dateDepartFrom);
		...
		List<Map<String, String>> driverListMap = jdbc.queryResultsString(stringBuilderQuery.toString(), false);
	...
	public static void updateGoogleCommutesXLS(JDBC jdbc) throws Throwable
	{
		...
		jdbc.exportDataFromTableView(VivitViews.VIVIT_COMMUTE_BDGET_WTOTALS, VivitFoldersFiles.DATA_FLIGHTS,
				sheetSummary, VivitData.DATABASE_DEFINITION, true);
		jdbc.exportDataFromTableView(VivitTables.DOM_VIVIT_FLIGHTS, VivitFoldersFiles.DATA_FLIGHTS, sheetFlights,
				VivitData.DATABASE_DEFINITION, false);
				...
				switch (columnName)
				{
					...
					case "Median (Actual)":
						if (airport.length() == 3)
						{
							...
							formula = "median(" + sheetFlights + "!" + columnLetter + rowStart + ":" + columnLetter
									+ rowEnd + ")";
		...
		for (int column = 0; column < columns; column++)
		{
			...
			{
				case "Traveler":
					formula = "counta(" + sheetSummary + "!" + columnLetter + "2" + ":" + columnLetter + rows + ")";
				...
				case "Average":
				case "Median (Actual)":
					formula = "sum(" + sheetSummary + "!" + columnLetter + "2" + ":" + columnLetter + rows + ")";
					excel.writeCell(sheetSummary, column, rows, "0");
					excel.writeCellFormula(sheetSummary, column, rows, formula);
					excel.setFormatHeading(sheetSummary, column, rows);
					break;
				case "Median (Treasurer)":
					...
					excel.setFormatHeading(sheetSummary, column, rows);
					excel.addComment(sheetSummary, column, rows,
							"As of " + DateHelpers.getCurrentDateTime("EEEE, MMM d, yyyy @ HH:mm:ss.SSS a"), false);
					break;
				default:
					excel.setFormatHeading(sheetSummary, column, rows);
					break;
	...
	private static String getFlightStartRow(XLS excel, String sheetName, String airport) throws Throwable
	{
	...
	public static void getGoogleFlights(Google google, String airportTo, String dateDepartTo, String dateDepartFrom)
			throws Throwable
	{
		JDBC jdbc = new JDBC("", VivitData.DATABASE_DEFINITION);
		StringBuilder stringBuilderSQL = new StringBuilder();
		...
		List<Map<String, String>> airportListMap = jdbc.queryResultsString(stringBuilderSQL.toString(), false);
		google.FlightsPage.getFlightsOld(VivitFoldersFiles.DATA_FLIGHTS, airportListMap, airportTo, dateDepartTo,
				dateDepartFrom);
	}
	...
	public static void getHTMLBrokenLinks() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + VivitViews.VIVIT_BROKEN_LINKS;
		final List<Map<Integer, String>> brokenLinksListMap = jdbc.queryResultsIndex(sql, true);
		jdbc.close();
		final StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append("<table class=" + Constants.QUOTE_DOUBLE + "scroll" + Constants.QUOTE_DOUBLE + " style="
				+ Constants.QUOTE_DOUBLE + STYLE_BORDER + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append("<tr>");
		stringBuilder.append("<th style=" + Constants.QUOTE_DOUBLE
				+ "border: 1px solid black; border-collapse: collapse; background-color: "
				+ ColorsHEX.VIVIT_GOLD.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
		...
		for (int brokenLinksListMapIndex = 0; brokenLinksListMapIndex < brokenLinksListMap
				.size(); brokenLinksListMapIndex++)
		{
			...
			{
				stringBuilder.append("<table class=" + Constants.QUOTE_DOUBLE + "scroll" + Constants.QUOTE_DOUBLE
						+ " style=" + Constants.QUOTE_DOUBLE + STYLE_BORDER + Constants.QUOTE_DOUBLE + ">");
			...
			} else
			{
				...
				for (final Entry entry : brokenLinkMap.entrySet())
				{
					...
					stringBuilder.append("<td style=" + Constants.QUOTE_DOUBLE
							+ "border: 1px solid black; border-collapse: collapse;" + Constants.QUOTE_DOUBLE + ">");
					final int key = (int) entry.getKey();
					final String value = (String) entry.getValue();
	...
	public static void getHTMLDatabaseChanges() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + VivitTables.VIVIT_DATABASE_CHANGES;
		...
		stringBuilder.append("</table>");
		final Map<String, Integer> fieldNameMap = new HashMap<>();
		for (int databaseChangesListMapIndex = 0; databaseChangesListMapIndex < databaseChangesListMap
				.size(); databaseChangesListMapIndex++)
		{
			final Map<Integer, String> brokenLinkMap = databaseChangesListMap.get(databaseChangesListMapIndex);
			sysOut("brokenLinkMap:" + brokenLinkMap.toString());
		...
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_DATABASE_CHANGES, htmlDatabaseChanges, false);
	}

	public static void createReportHTMLTestInformation(SeleniumWebDriver seleniumWebDriver) throws Throwable
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		List<Map<Integer, String>> testInformationListMap = getReportHTMLTestInformationListMap(seleniumWebDriver);
		final StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append("<table class=" + Constants.QUOTE_DOUBLE + "scroll" + Constants.QUOTE_DOUBLE + " style="
				+ Constants.QUOTE_DOUBLE + STYLE_BORDER + Constants.QUOTE_DOUBLE + ">");
		stringBuilder.append("<tr>");
		stringBuilder.append("<th style=" + Constants.QUOTE_DOUBLE
				+ "border: 1px solid black; border-collapse: collapse; background-color: "
				+ ColorsHEX.VIVIT_PURPLE.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
		// Need to account for the headings map.
		...
		stringBuilder.append("</table>");
		final Map<String, Integer> fieldNameMap = new HashMap<>();
		for (int testInformationListMapIndex = 0; testInformationListMapIndex < testInformationListMap
				.size(); testInformationListMapIndex++)
		{
			final Map<Integer, String> testInformationLinkMap = testInformationListMap.get(testInformationListMapIndex);
		...
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_TEST_INFORMATION, htmlTestInformation, false);
	}

	private static List<Map<Integer, String>> getReportHTMLTestInformationListMap(SeleniumWebDriver seleniumWebDriver)
			throws Throwable
	{
		...
		final EDriverProperties eDriverProperties = EDriverProperties
				.fromString(seleniumWebDriver.getCapabilities().getBrowserName().toUpperCase());
		...
		testInformationMap.put(2, DateHelpers.getCurrentDateTime("yyyy-MM-dd HH:mm:ss.SSS") + " ("
				+ DateHelpers.getTimeZoneDisplayName() + "-" + DateHelpers.getTimeZoneID() + ")");
		testInformationListMap.add(new HashMap<>(testInformationMap));
	...
	public static void getHTMLMemberChanges() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + VivitViews.VIVIT_MEMBER_CHANGES;
		...
		final Map<String, Integer> fieldNameMap = new HashMap<>();
		int memberChange = 0;
		for (int gtwEventsListMapIndex = 0; gtwEventsListMapIndex < gtwEventsListMap.size(); gtwEventsListMapIndex++)
		{
		...
		for (int gtwEventsListMapIndex = 0; gtwEventsListMapIndex < gtwEventsListMap.size(); gtwEventsListMapIndex++)
		{
			...
			} else
			{
				...
				for (final Entry entry : gtwEventsMap.entrySet())
				{
					...
					{
						href = "https://www.vivit-worldwide.org/events/EventDetails.aspx?id=" + value + "&group=";
						stringBuilder.append(HTML.link(href, value));
					} else
					{
						stringBuilder.append(value);
	...
	public static void getHTMLYMGTWEventAttendees() throws Throwable
	{
		...
		sysOut(ParameterHelper.getParameters(Arrays.asList()));
		JDBC jdbc = new JDBC("", DATABASE_DEFINITION);
		final String sql = JDBCConstants.SELECT_ALL + JDBCConstants.FROM + VivitViews.VIVIT_EVENT_ATTENDEES_CURRENT_GTW;
		...
		stringBuilder.append("<th style=" + Constants.QUOTE_DOUBLE
				...
				+ ColorsHEX.VIVIT_GOLD.getValue() + ";" + Constants.QUOTE_DOUBLE + ">");
		// Need to account for the headings map.
		stringBuilder.append("Current/Upcoming YM/GTW Event Attendees (" + (gtwEventAttendeesListMap.size() - 1) + ")");
		stringBuilder.append("</th>");
		stringBuilder.append("</tr>");
		stringBuilder.append("</table>");
		final Map<String, Integer> fieldNameMap = new HashMap<>();
		for (int gtwEventAttendeesListMapIndex = 0; gtwEventAttendeesListMapIndex < gtwEventAttendeesListMap
				.size(); gtwEventAttendeesListMapIndex++)
		{
			final Map<Integer, String> gtwEventAttendeesMap = gtwEventAttendeesListMap
					.get(gtwEventAttendeesListMapIndex);
		...
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_EVENT_ATTENDEES_YM, htmlGTWEventAttendeesLinks, false);
	}
	...
	public static void getHTMLYMGTWEventRegistration() throws Throwable
	{
		...
		final List<Map<Integer, String>> gtwEventRegistrationListMap = jdbc.queryResultsIndex(sql, true);
		jdbc.close();
		final StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append("<table class=" + Constants.QUOTE_DOUBLE + "scroll" + Constants.QUOTE_DOUBLE + " style="
				+ Constants.QUOTE_DOUBLE + STYLE_BORDER + Constants.QUOTE_DOUBLE + ">");
		...
		// Need to account for the headings map.
		stringBuilder.append(
				"Current/Upcoming YM/GTW Event Registrations (" + (gtwEventRegistrationListMap.size() - 1) + ")");
		stringBuilder.append("</th>");
		stringBuilder.append("</tr>");
		stringBuilder.append("</table>");
		final Map<String, Integer> fieldNameMap = new HashMap<>();
		for (int gtwEventRegistrationsListMapIndex = 0; gtwEventRegistrationsListMapIndex < gtwEventRegistrationListMap
				.size(); gtwEventRegistrationsListMapIndex++)
		{
			final Map<Integer, String> gtwEventRegistrationsMap = gtwEventRegistrationListMap
					.get(gtwEventRegistrationsListMapIndex);
		...
		FSO.fileWrite(VivitFoldersFiles.REPORT_HTM_AUTOMATION_EVENT_REGISTRATION_YM, htmlGTWEventRegistrationsLinks,
				false);
	}
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(LABEL_DATE_TIME_FROM, dateTimeFrom))));
		try
		{
			...
			sqlStringBuilder.append(JDBCConstants.WHERE + "[" + LABEL_WEB_SITE_MEMBER_ID + "]='10969660'");
	...
	public static void databaseInitialize(boolean initialize) throws Throwable
	{
		...
		FileUtils.copyFile(new File(VivitFoldersFiles.DATABASE), new File(VivitFoldersFiles.DATABASE_WORKING));
		...
		final int recordsUpdated = SQL.executeVivit("Initialization", JavaHelpers.getCurrentMethodName(),
				new StringBuilder(initializationQueries));
		sysOut("recordsUpdated:[" + recordsUpdated + "]");
		StringBuilder sqlStringBuilder = new StringBuilder();
		sqlStringBuilder.append(JDBCConstants.DELETE_FROM + "[" + VivitTables.VIVIT_DATABASE_CHANGES + "];");
		sqlStringBuilder.append(Constants.NEWLINE);
	...
	public void qbTest() throws QAException
	{
		...
		dbConnection.setName("C:" + Constants.DELIMETER_PATH + "Users" + Constants.DELIMETER_PATH + "Public"
				+ Constants.DELIMETER_PATH + "Documents" + Constants.DELIMETER_PATH + "Intuit"
				+ Constants.DELIMETER_PATH + "QuickBooks" + Constants.DELIMETER_PATH + "Company Files"
				+ Constants.DELIMETER_PATH + "Vivit.QBW");
		dbConnection.setPassword(EPasswords.QB_VIVIT.getValue());
		dbConnection.setConnectionString(dbConnection.getUrlPrefix() + dbConnection.getName() + Constants.DELIMETER_LIST
				+ "memory=" + dbConnection.getMemory());
		sysOut(dbConnection.toString());
		try
		{
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(classMethodType, "classMethodType"))));
	...
	{
		sysOut(Constants.CLASS_METHOD_DEBUG + JavaHelpers.getCurrentClassMethodDebugName() + "]");
		sysOut(ParameterHelper.getParameters(Arrays.asList(Arrays.asList(successStatus, "successStatus"),
	 ...
	 */
	public static void updateTableFromCurrentToPreviousAndInsert(List<StringBuilder> stringBuilderSQLList)
	{
		...
		for (String table : tableArray)
		{
			if (!table.equalsIgnoreCase("Calendars"))
			{
				...
				String queryDeletePrevious = JDBCConstants.DELETE_FROM + "[" + VivitTables.PREFIX + table
						+ LABEL_PREVIOUS + "];";
				sysOut(queryDeletePrevious);
				SQL.executeVivit("DeleteFrom", table + LABEL_PREVIOUS, new StringBuilder(queryDeletePrevious));
				String queryInsertPrevious = JDBCConstants.INSERT_INTO + "[" + VivitTables.PREFIX + table
						+ LABEL_PREVIOUS + "] " + JDBCConstants.SELECT_ALL_FROM + "[" + VivitTables.PREFIX + table
						+ LABEL_CURRENT + "];";
				sysOut(queryInsertPrevious);
				SQL.executeVivit("InsertInto", table + LABEL_PREVIOUS, new StringBuilder(queryInsertPrevious));
			}
			String queryDeleteCurrent = JDBCConstants.DELETE_FROM + "[" + VivitTables.PREFIX + table + LABEL_CURRENT
					+ "];";
			...
			for (int stringBiulderSQLIndex = 1; stringBiulderSQLIndex < stringBuilderSQLList
					.size(); stringBiulderSQLIndex++)
			{
				// String table = tableArray[(stringBiulderSQLIndex - 1)]
				String queryInsertCurrent = "Inserting data into [" + VivitTables.PREFIX + table + LABEL_CURRENT + "];";
