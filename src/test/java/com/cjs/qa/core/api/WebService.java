...
public class WebService
{
	...
	public static Map<String, String> getAPIJSONResponse(String requestMethod, String url, String apiRequest)
	{
		...
		{
			Environment.sysOut("getAPIJSONResponse Parameters:" + Constants.NL + Constants.TAB + "requestMethod:["
					+ requestMethod + "]" + Constants.NL + Constants.TAB + "url:[" + url + "]" + Constants.NL
					+ Constants.TAB + "apiRequest[" + apiRequest + "]");
		}
		int responseCode = -1;
		...
		try
		{
			...
			httpURLConnection.setRequestProperty("Content-Length", "" + Integer.toString(apiRequest.getBytes().length));
	...
	public static String getAPIJSONResponse(String string, String requestURL, String apiRequest, String ssoUserTokenId)
	{
	...
	public static String getAPIXMLRequest(Policy policy, SOAPMessage soapMessage)
	{
		try
		{
			...
			final String fileTemp = fileRequest.replace("request",
					policy.dateTimeStamp + "_" + Environment.getComputerName() + "_" + Environment.CURRENT_USER + "_"
							+ policy.policy + "_" + "request");
			...
			final String fileOut = fileRequest.replace("request", policy.dateTimeStamp + "_" + policy.computerName + "_"
					+ policy.userName + "_" + policy.policy + "_" + requestName + "_request");
	...
	public static String getAPIXMLResponse(Policy policy, String url, SOAPMessage soapMessage)
	{
		...
		try
		{
			...
			final String fileTemp = fileResponse.replace("response",
					policy.dateTimeStamp + "_" + Environment.getComputerName() + "_" + Environment.CURRENT_USER + "_"
							+ policy.policy + "_" + "response");
			...
			final String fileOut = fileResponse.replace("response",
					policy.dateTimeStamp + "_" + Environment.getComputerName() + "_" + Environment.CURRENT_USER + "_"
							+ policy.policy + "_" + responseName + "_response");
			FSO.fileWrite(fileOut, xml, false);
		} catch (final Exception e)
		{
			Environment.sysOut(e);
	...
	public static SOAPMessage createSOAPRequest(String url)
	{
		...
		try
		{
			...
			final SOAPElement soapElement1 = soapBody.addChildElement("getPolicyPremiumSummaryXML", "com", url);
			// soapElement1.setPrefix("com");
			final SOAPElement soapElement2 = soapElement1.addChildElement("basicXML", "com", url);
			final CDATASection cdata = soapElement2.getOwnerDocument().createCDATASection(
	...
	public static Map<String, String> getAPIXMLResponse(String requestMethod, String url, String apiRequest)
	{
		final Map<String, String> map = new HashMap<>();
		map.put("requestMethod", requestMethod);
		map.put("url", url);
		...
		if (Environment.isLogAPI())
		{
			Environment.sysOut("getAPIXMLResponse Parameters:" + Constants.NL + Constants.TAB + "requestMethod:["
					+ requestMethod + "]" + Constants.NL + Constants.TAB + "url:[" + url + "]" + Constants.NL
					+ Constants.TAB + "apiRequest[" + apiRequest + "]");
		}
		int responseCode = -1;
