...
import org.openqa.selenium.firefox.FirefoxOptions;import org.openqa.selenium.safari.SafariOptions;import org.openqa.selenium.ie.InternetExplorerOptions;import org.openqa.selenium.edge.EdgeOptions;import org.openqa.selenium.edge.EdgeDriver;
...
public interface ISelenium
{
	...
	public static final String	SELENIUM_GRID_HUB			= "http://" + SELENIUM_GRID_HUB_NAME + ":"
			+ String.valueOf(SELENIUM_GRID_HUB_NAMEPORT) + "/wd/hub";
	...
	public default void cukeAttachFile(Scenario scenarioObject, String fileName)
	{
		...
		try
		{
			scenarioObject.attach(Files.readAllBytes(Paths.get(fileName)), "application/vnd.ms-excel", fileName);
	...
	public default void embedScreenshot(Scenario scenarioObject, WebDriver webDriver)
	{
		if (Environment.isLogAll())
		{
			Environment.sysOut("scenarioObject.ID:[" + scenarioObject.getId() + ", scenarioObject.Name:["
					+ scenarioObject.getName() + "], webDriver:[" + webDriver.toString() + "]");
		}
		try
		{
			final byte[] screenshot = ((TakesScreenshot) webDriver).getScreenshotAs(OutputType.BYTES);
	 ...
	 * @return
	 */
	public static String getWebElementXPath(List<Integer> parentList, WebElement webElement, String byType)
			throws QAException
	{
		...
		if (parents != parentList.size())
			Assert.fail("The parentList [" + parentList.size() + "] does not match the number of parents [" + parents
					+ "]");
	...
	public static String getAbsoluteXPath(WebDriver webDriver, WebElement element)
	{
		...
		stringBuilder.append("for (curNode=element.previousSibling; curNode; curNode=curNode.previousSibling){");
		...
		stringBuilder.append("comp=comps[comps.length] ={};switch (element.nodeType){case Node.TEXT_NODE:");
		stringBuilder.append("comp.name='text()';break;case Node.ATTRIBUTE_NODE:comp.name='@' + element.nodeName;");
		stringBuilder.append("break;");
		stringBuilder.append("case Node.PROCESSING_INSTRUCTION_NODE:comp.name='processing-instruction()';");
		stringBuilder.append("break;");
		stringBuilder.append("case Node.COMMENT_NODE:comp.name='comment()';break;case Node.ELEMENT_NODE:");
		...
		return (String) ((JavascriptExecutor) webDriver).executeScript(stringBuilder.toString(), element);
	}
	...
	public static void killBrowserProcesses(String browser)
	{
		...
		switch (browser.toLowerCase())
		{
			case "chrome":
				...
				break;
			case "edge":
				processesRunning = "MicrosoftEdge.exe" + Constants.DELIMETER_LIST + "MicrosoftWebDriver.exe";
				break;
			case "firefox":
				break;
			case "ie":
	...
	public static DesiredCapabilities setDesiredCapabilities(String operatingSystem, String browser)
	{
		...
		switch (browser.toLowerCase())
		{
			...
			case "edge":
				...
				break;
			case "firefox":
				System.setProperty("webdriver.gecko.driver", Constants.PATH_DRIVERS_LOCAL + "geckodriver.exe");
	...
	public static WebDriver browserProfiling(String browser, Scenario scenario)
	{
		...
		try
		{
			...
			final DesiredCapabilities desiredCapabilities = setDesiredCapabilities(operatingSystem, browser);
	...
	public static WebDriver browserProfiling(WebDriver webDriver, String browser)
	{
		...
		try
		{
			...
			if (Environment.isLogAll())
			{
				sMessage = String.format("Browser:[%s], Default:[%s], Driver:[%s]", browser, browserDefault, webDriver);
				Environment.sysOut(sMessage);
			}
	...
	{
		String sMessage = null;
		final String screenshotCounter = DateHelpers.getCurrentDateTime(DateHelpers.FORMAT_DATE_TIME_STAMP);
	 ...
	 * @return
	 */
	public default boolean webPageGetObjects(WebDriver webDriver, WebElement elementParent, String functionName)
	{
		String sMessage = null;
		if (Environment.isLogAll())
		{
		...
		sb.append("elementID" + Constants.TAB + "elementName" + Constants.TAB + "elementHref" + Constants.TAB
				+ "elementText" + Constants.NL);
