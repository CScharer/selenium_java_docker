...
public class Page extends JavaHelpers
{
	...
	public final String						SCRIPT_GET_ELEMENT_BORDER		= "var element = arguments[0];"
			+ "if (element.currentStyle) {var style = element.currentStyle;"
			+ "var border = style['borderTopWidth'] + ' ' + style['borderTopStyle'] + ' ' + style['borderTopColor'] + ';' + style['borderRightWidth'] + ' ' + style['borderRightStyle'] + ' ' + style['borderRightColor'] + ';' + style['borderBottomWidth'] + ' ' + style['borderBottomStyle'] + ' ' + style['borderBottomColor'] + ';' + style['borderLeftWidth'] + ' ' + style['borderLeftStyle'] + ' ' + style['borderLeftColor'];"
			+ "} else if (window.getComputedStyle) {var style = document.defaultView.getComputedStyle(element);"
			+ "var border = style.getPropertyValue('border-top-width') + ' ' + style.getPropertyValue('border-top-style') + ' ' + style.getPropertyValue('border-top-color') + ';' + style.getPropertyValue('border-right-width') + ' ' + style.getPropertyValue('border-right-style') + ' ' + style.getPropertyValue('border-right-color') + ';' + style.getPropertyValue('border-bottom-width') + ' ' + style.getPropertyValue('border-bottom-style') + ' ' + style.getPropertyValue('border-bottom-color') + ';' + style.getPropertyValue('border-left-width') + ' ' + style.getPropertyValue('border-left-style') + ' ' + style.getPropertyValue('border-left-color');"
			+ "}element.style.border = '4px solid " + borderColor + "';" + "return border;";
	public final String						SCRIPT_UNHIGHLIGHT_ELEMENT		= "var element = arguments[0];"
			+ "var borders = arguments[1].split(';');" + "element.style.borderTop = borders[0];"
			+ "element.style.borderRight = borders[1];" + "element.style.borderBottom = borders[2];"
	...
	private SeleniumWebDriverEventListener	seleniumWebDriverEventListener	= new SeleniumWebDriverEventListener();
	...
	public Page(WebDriver webDriver)
	{
		...
		EventFiringDecorator<WebDriver> eventFiringWebDriver = new EventFiringDecorator<>(seleniumWebDriverEventListener);
	...
	{
		final List<String> lClassExclusions = Arrays.asList("java.lang.Thread", "com.cjs.framework.Core.Page",
				"com.cjs.framework.Core.AutGui", "com.cjs.framework.Core.AppCoreObject");
		String stack = null;
		...
		for (final StackTraceElement stackTraceElement : stackTraceElements)
		{
			...
			{
				stack = stackTraceElement.getFileName() + Constants.TAB + stackTraceElement.getMethodName()
						+ Constants.TAB + "Line (" + stackTraceElement.getLineNumber() + ")";
				break;
			}
		}
		return stack;
	...
	public boolean unhighlightPreviousElement()
	{
		...
		if (getPreviousWebElement() != null)
		{
			try
			{
				...
				{
					executeJavaScript(SCRIPT_UNHIGHLIGHT_ELEMENT, getPreviousWebElement(), getPreviousBorder());
					success = true;
				}
	...
	public void captureScreenshot()
	{
		...
		stringBuilder.append("_");
		stringBuilder.append(JavaHelpers.formatNumber(getScreenshotCounter(), FORMAT_SCREENSHOT_NUMBER));
	...
	{
		getWebDriver().findElement(By.xpath("//div[@class = 'popupContent']//div[@class = 'v-window-closebox']"))
				.click();
	}

	/**
	 * Closes a specified popup by switching the driver to the pop up and
	 * closing it
	 * 
	 * @param windowTitle
	 */
	...
	{
		final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), java.time.Duration.ofSeconds(timeOut));
	...
	{
		final String script = "var rect = arguments[0].getBoundingClientRect();"
	...
	{
		return getWebDriver().findElement(By.xpath("//*[text()='" + text + "']")).getCssValue("text-align");
	}
	...
	public String getTextColor(String text)
	{
		try
		{
			return getWebDriver().findElement(By.xpath("//*[text()='" + text + "']")).getCssValue("color");
		} catch (final Exception e)
		{
	...
	{
		final String script = "return document.documentElement.scrollWidth>document.documentElement.clientWidth;";
	...
	public boolean isListsEqual(List<String> stringList1, List<String> stringList2)
	{
		...
		{
			Environment.sysOut("The following items are not in the list: "
	...
	{
		final String script = "return document.documentElement.scrollHeight>document.documentElement.clientHeight;";
		return (Boolean) (executeJavaScript(script));
	}
	...
	{
		Environment.sysOut("({Field}" + fieldName + ", {tagName}" + tagName + ", {bounds}" + bounds + ");");
	}
	...
	protected boolean objectExists(By by, int seconds)
	{
		try
		{
			final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), java.time.Duration.ofSeconds(seconds));
	...
	public void scrollDown(int length)
	{
		if (isVerticalScrollbarPresent())
		{
			...
			String script = "scroll(" + getVerticalScrollBarPosition() + "," + (getVerticalScrollBarPosition() + length)
					+ ")";
			executeJavaScript(script);
		}
	}
	...
	public void scrollUp(int length)
	{
		if (isVerticalScrollbarPresent())
		{
			// String script = "window.scrollBy(" +
			// getVerticalScrollBarPosition() + ",-" + length + ")"
			String script = "scroll(" + getVerticalScrollBarPosition() + ",-"
					+ (getVerticalScrollBarPosition() + length) + ")";
			executeJavaScript(script);
		}
	}
	...
	protected void selectOption(By by, String value)
	{
		...
		final WebElement child = parent.findElement(By.xpath(".//label[contains(text(),'" + value + "')]"));
		child.click();
	}
	...
	protected void setCheckbox(By by, String value)
	{
		...
		if (!getCheckbox(by).equals(value))
		{
			Environment.sysOut(LABEL_THE_VALUE + value + "] was not selected for Checkbox [" + by.toString() + "].");
		}
	}
	...
	protected void setCheckboxes(List<WebElement> webElements, String value)
	{
		...
		for (final WebElement webElement : webElements)
		{
			...
			{
				Environment.sysOut(
						LABEL_THE_VALUE + value + "] was not selected for Checkbox [" + webElement.toString() + "].");
			}
		}
	}
	 ...
	 * @return
	 */
	public boolean softAssert(Scenario scenario, List<String> actual, List<String> expected, String message)
	{
	...
	{
		final WebDriverWait webDriverWait = new WebDriverWait(getWebDriver(), java.time.Duration.ofSeconds(TIMEOUT_ELEMENT));
	...
	{
		final Wait<WebDriver> webDriverWait = new WebDriverWait(getWebDriver(), java.time.Duration.ofSeconds(TIMEOUT_ELEMENT));
	...
	{
		final Wait<WebDriver> webDriverWait = new WebDriverWait(getWebDriver(), java.time.Duration.ofSeconds(timeout));
		final WebElement webElement = webDriverWait.until(new Function<WebDriver, WebElement>()
		{
			@Override
	...
	public boolean wildCardMatch(String text, String pattern)
	{
		...
		for (final String card : cards)
		{
			...
			if (idx == -1)
			{
				Environment.sysOut(Constants.NL + "card: " + card + Constants.NL + "Diff: " + Constants.NL
