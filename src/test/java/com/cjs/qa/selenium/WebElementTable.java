...
public class WebElementTable extends Page
{
	 ...
	 * @param xPathCells
	 */
	public WebElementTable(WebDriver webDriver, String xPathTable, String xpathHeadings, String xpathRows,
			String xPathCells)
	{
		super(webDriver);
	...
	{
		return getColumnRowValues(By.xpath(getXpathRows() + "[" + row + "]" + getxPathCells().replace(".", "")));
	}
	...
	public String prepareForCSV(String value)
	{
		...
		if (value.contains(",") || value.contains(Constants.CR) || value.contains(Constants.NL)
	...
	private void setHeadingMaps()
	{
		...
		final List<WebElement> headings = getWebElementTable().findElements(By.xpath(getXpathHeadings()));
