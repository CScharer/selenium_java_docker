...
public class WebDriverValidation
{
	...
	public void setMapURL(Map<String, String> mapURL)
	{
		...
		mapURL.put("edge", "https://developer.microsoft" + IExtension.COM + "/en-us/microsoft-edge/tools/webdriver/");
	...
	public void validate(String browser, SoftAssert softAssert) throws Throwable
	{
		...
		{
			case "chrome":
				String command = "cmd /C wmic datafile where name=\"C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe\" get Version /value";
				...
				//WebDriver has been updated from [ChromeDriver 74.0.3729.6][5.0.3770.90] to [ChromeDriver 75.0.3770.90][4.0.3729.6]			
				driverVersionExpected = "ChromeDriver 75.0.3770.90";
				// Supports Chrome version 74
				versionBy = By.xpath(
						".//*[@id='sites-canvas-main-content']//li[contains(text(),'Latest stable release:')]/a");
			...
			case "edge":
				...
				driverVersionExpected = "Version: 75.0.137.0 | Microsoft Edge version supported: 75 (x86, x64) | License terms | Notices";
				versionBy = By
						.xpath(".//*[@id='downloads']//li[@class='driver-download']/a[contains(text(),'Release')]/..");
				...
				for (int indexWebELement = 0; indexWebELement < webElements.size(); indexWebELement++)
				{
					...
					if (indexWebELement == 0)
					{
						...
						bodyStringBuilder
								.append("[" + JavaHelpers.difference(driverReleaseExpected, driverReleaseActual) + "]");
						bodyStringBuilder.append(" to ");
						bodyStringBuilder.append("[" + driverReleaseActual + "]");
						bodyStringBuilder
								.append("[" + JavaHelpers.difference(driverReleaseActual, driverReleaseExpected) + "]");
						softAssert.assertEquals(driverReleaseExpected, driverReleaseActual,
						...
						bodyStringBuilder
								.append("[" + JavaHelpers.difference(driverVersionExpected, driverVersionActual) + "]");
						bodyStringBuilder.append(" to ");
						bodyStringBuilder.append("[" + driverVersionActual + "]");
						bodyStringBuilder
								.append("[" + JavaHelpers.difference(driverVersionActual, driverVersionExpected) + "]");
						softAssert.assertEquals(driverVersionExpected, driverVersionActual,
								bodyStringBuilder.toString());
						driverVersionExpected = "Release 75" + Constants.NL
								+ "Version: 75.0.137.0 | Microsoft Edge version supported: 75 (x86, x64) | License terms | Notices";
						driverVersionActual = webElement.getText();
						Environment.sysOut("driverVersionActual:" + driverVersionActual);
						break;
					}
			...
			case "ie":
				...
				driverVersionExpected = "Download version 3.14.0 for (recommended) 32 bit Windows IE or 64 bit Windows IE"
						+ Constants.NL + "CHANGELOG";
		...
		bodyStringBuilder.append("[" + browser + "] WebDriver has been updated from ");
		bodyStringBuilder.append("[" + driverVersionExpected + "]");
		bodyStringBuilder.append("[" + JavaHelpers.difference(driverVersionExpected, driverVersionActual) + "]");
		bodyStringBuilder.append(" to ");
		bodyStringBuilder.append("[" + driverVersionActual + "]");
		bodyStringBuilder.append("[" + JavaHelpers.difference(driverVersionActual, driverVersionExpected) + "]");
		if (!driverVersionExpected.equals(driverVersionActual))
		{
			...
			Email.sendEmail(CJSConstants.EMAIL_ADDRESS_GMAIL, EPasswords.EMAIL_GMAIL.getValue(),
					CJSConstants.EMAIL_ADDRESS_GMAIL, "", "", subject, bodyStringBuilder.toString(), "");
		}
		softAssert.assertEquals(driverVersionExpected, driverVersionActual, bodyStringBuilder.toString());
