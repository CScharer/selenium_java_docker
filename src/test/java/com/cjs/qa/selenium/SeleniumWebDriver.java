...
import org.openqa.selenium.ie.InternetExplorerOptions;import org.openqa.selenium.edge.EdgeOptions;import org.openqa.selenium.edge.EdgeDriver;
...
public class SeleniumWebDriver
{
	 ...
	 * @throws Throwable
	 */
	public SeleniumWebDriver(String browser, boolean remote, String vendorURL, String operatingSystem, String version)
			throws Throwable
	{
		killBrowser();
		if (browser != null)
		...
		try
		{
			if (getWebDriver() != null)
			{
				...
				} else
				{
					...
					Environment.sysOut("windowHeight:[" + heightWindow + "], scrollHeight:[" + heightScroll + "]");
	...
	private String combineALLImages(String pathTemp) throws IOException
	{
		...
		final BufferedImage bufferedImageFinal = new BufferedImage(bufferedImageSample.getWidth() * columns,
				bufferedImageSample.getHeight() * rows, bufferedImageSample.getType());
		int index = 0;
		for (int indexRow = 0; indexRow < rows; indexRow++)
		{
			for (int indexColumn = 0; indexColumn < columns; indexColumn++)
			{
				...
				final String fileName = filePathName.substring((filePathName.lastIndexOf(Constants.DELIMETER_PATH) + 1),
						filePathName.length());
				Environment.sysOut("Adding Image File:[" + index + "], [" + fileName + "]");
				final BufferedImage bufferedImageTemp = ImageIO.read(listFile.get(index));
				bufferedImageFinal.createGraphics().drawImage(bufferedImageTemp,
						bufferedImageSample.getWidth() * indexColumn, bufferedImageSample.getHeight() * indexRow, null);
				index++;
			}
		}
	...
	public void getWebDriverInfo()
	{
		try
		{
			if (getWebDriver() != null)
			{
				...
				{
					Map<String, Object> chromeCapabilities = (Map<String, Object>) capabilities
							.getCapability(Browser.CHROME);
	...
	public void initializeWebDriver() throws Throwable
	{
		...
		final DesiredCapabilities desiredCapabilities = setDesiredCapabilities(getOperatingSystem(), getBrowser());
		final EDriverProperties eDriverProperties = EDriverProperties.fromString(getBrowser().toUpperCase());
		try
		{
				...
				switch (getBrowser().toLowerCase())
				{
					...
					case Browser.FIREFOX:
						...
						GeckoDriverService service = new GeckoDriverService.Builder()
								.usingDriverExecutable(new File("path to geckodriver")).usingAnyFreePort()
	...
	private ChromeOptions setChromeOptions(String pathFileExecutable, String pathFileBinary)
	{
		...
		final List<String> arguments = Arrays.asList("disable-popup-blocking", "test-type", "start-maximized",
	...
	private DesiredCapabilities setDesiredCapabilities(String operatingSystem, String browser)
	{
		...
		switch (browser.toLowerCase())
		{
			...
			case Browser.INTERNET_EXPLORER:
				...
				desiredCapabilities.setCapability(eDriverProperties.getWebDriverType(),
	...
	private void setLocalExecutables()
	{
		...
		Environment.sysOut(JavaHelpers.getCurrentMethodName() + "-webDriverType:["
				+ eDriverProperties.getWebDriverType() + "], PathDriver:[" + eDriverProperties.getPathDriver()
				+ "], , PathBinary:[" + eDriverProperties.getPathBinary() + "]");
	}
	...
	public void setProxy(String browser, boolean isRemote, DesiredCapabilities desiredCapabilities) throws Exception
	{
		if (!isRemote)
		{
	...
	public String toString()
	{
		...
		stringBuilder.append(c.nlTab(newLine, tab) + "{");
		tab = c.tabIncriment(tab, (newLine * 1));
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Grid Hub Name" + Constants.QUOTE_DOUBLE
				+ ": " + Constants.QUOTE_DOUBLE + getGridHubName() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Grid Hub Port" + Constants.QUOTE_DOUBLE
				+ ": " + Constants.QUOTE_DOUBLE + getGridHubPort() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Grid Hub" + Constants.QUOTE_DOUBLE + ": "
				+ Constants.QUOTE_DOUBLE + getGridHub() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder
				.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Operating System" + Constants.QUOTE_DOUBLE
						+ ": " + Constants.QUOTE_DOUBLE + getOperatingSystem() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Remote" + Constants.QUOTE_DOUBLE + ": "
				+ isRemote() + ",");
		final Scenario scenario = getScenario();
		if (scenario == null)
		{
			stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Scenario" + Constants.QUOTE_DOUBLE
					+ ": " + Constants.QUOTE_DOUBLE + scenario + Constants.QUOTE_DOUBLE + ",");
		} else
		{
			stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Scenario" + Constants.QUOTE_DOUBLE
					+ ": " + Constants.QUOTE_DOUBLE + getScenario().toString() + Constants.QUOTE_DOUBLE + ",");
		}
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Screenshot Count"
				+ Constants.QUOTE_DOUBLE + ": " + getScreenshots() + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Session ID" + Constants.QUOTE_DOUBLE
				+ ": " + Constants.QUOTE_DOUBLE + getSessionId() + Constants.QUOTE_DOUBLE + ",");
		final WebDriver webDriver = getWebDriver();
		if (webDriver == null)
		{
			stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Web Driver" + Constants.QUOTE_DOUBLE
					+ ": " + Constants.QUOTE_DOUBLE + webDriver + Constants.QUOTE_DOUBLE + ",");
		} else
		{
			stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Web Driver" + Constants.QUOTE_DOUBLE
					+ ": " + Constants.QUOTE_DOUBLE + getWebDriver().toString() + Constants.QUOTE_DOUBLE + ",");
		}
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Vendor URL" + Constants.QUOTE_DOUBLE
				+ ": " + Constants.QUOTE_DOUBLE + getVendorURL() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Version" + Constants.QUOTE_DOUBLE + ": "
				+ Constants.QUOTE_DOUBLE + getVersion() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder
				.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Capabilities" + Constants.QUOTE_DOUBLE + ":");
		stringBuilder.append(c.nlTab(newLine, tab) + "{");
		tab = c.tabIncriment(tab, (newLine * 1));
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Browser" + Constants.QUOTE_DOUBLE + ": "
				+ Constants.QUOTE_DOUBLE + getCapabilities().getBrowserName() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Platform" + Constants.QUOTE_DOUBLE + ": "
				+ Constants.QUOTE_DOUBLE + getCapabilities().getPlatformName() + Constants.QUOTE_DOUBLE + ",");
		stringBuilder.append(c.nlTab(newLine, tab) + Constants.QUOTE_DOUBLE + "Version" + Constants.QUOTE_DOUBLE + ": "
				+ Constants.QUOTE_DOUBLE + getCapabilities().getBrowserVersion() + Constants.QUOTE_DOUBLE + "");
		tab = c.tabIncriment(tab, (newLine * -1));
		stringBuilder.append(c.nlTab(newLine, tab) + "}");
		tab = c.tabIncriment(tab, (newLine * -1));
		stringBuilder.append(c.nlTab(newLine, tab) + "}");
